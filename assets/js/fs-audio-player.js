...
class FSAudioPlayer {
    ...
    _initVisualizer() {
        ...
        try {
            ...
            try {
                ...
                this._vuAnalyserL.smoothingTimeConstant = 0;
                this._vuAnalyserR.smoothingTimeConstant = 0;
                this._mediaSource.connect(this._vuSplitter);
                this._vuSplitter.connect(this._vuAnalyserL, 0);
                this._vuSplitter.connect(this._vuAnalyserR, 1);
    ...
    _drawVU(ctx, w, h) {
        ...
        // Piecewise interpolation — 22nd rev: 균일 간격, 0VU=+12°
        const vuToAngle = (v) => {
            if (v <= -20) return -25;
            if (v >= 3) return 24;
            if (v <= -10) return -25 + ((v + 20) / 10) * 5;
            if (v <= -7)  return -20 + ((v + 10) / 3) * 5;
            if (v <= -5)  return -15 + ((v + 7)  / 2) * 5;
            if (v <= -3)  return -10 + ((v + 5)  / 2) * 5;
            if (v <= -2)  return -5  + ((v + 3)  / 1) * 5;
            if (v <= -1)  return  0  + ((v + 2)  / 1) * 6;
            if (v <= 0)   return  6  + ((v + 1) / 1) * 6;
            if (v <= 1)   return 12  + (v / 1) * 4;
            if (v <= 2)   return 16  + ((v - 1) / 1) * 4;
            if (v <= 3)   return 20  + ((v - 2) / 1) * 4;
            return 24;
        };
    ...
    _createVUMeterElement(label) {
        ...
        // 5) 메인 dB 눈금 + 숫자 — 22nd rev: 균일 간격, 0VU=+12°
        const ticks = [
            { vu: -20, a: -25 },
            { vu: -10, a: -20 },
            { vu: -7,  a: -15 },
        ...
        // 12) 회전축 점 — 22nd rev: 보이지 않게 (호환용 dummy)
