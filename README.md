...
![version](https://img.shields.io/badge/version-v5.8.3e-blue)
...
**현재 버전**: v5.8.3e (rhwp 0.8.4 기반)

### 주요 변경 이력

#### v5.8.3e (2026-08-17) ⭐ 현재

- **[문서] rhwp 업그레이드 가이드 v6.6 → v6.7 개정 (2026-08-20)**
...
- **[수정] HWP 문서 열기가 매번 30초 걸리던 문제 — WASM 대기 상한 30초 → 2초 (2026-08-20, 계측으로 원인 확정)**
...
- **[진단] HWP 문서 로딩 단계별 계측 추가 (2026-08-20)**
...
- **[수정] rhwp 에디터 상단 서식 도구 모음 깨짐 — style-bar 마크업을 0.8.4 구조로 교체 (2026-08-20)**
  ...
  - **별건 유지**: 에디터 첫 로딩이 느린 것은 이번 세션 버전업(5.8.3d→5.8.3e)으로 studio 자산 39MB 를 다시 받은 영향으로 보인다(캐시버스팅이 `APP_VERSION` 기반). 두 번째 열기부터는 빨라져야 정상이며, 계속 느리면 별도로 봐야 한다.

- **[수정] 영상 1개에 임시폴더·ffmpeg 가 2개 생기는 버그 — 세션 재사용 판정 오류 (2026-08-20, 로그로 원인 확정)**
...
- **[진단] HW 인코딩·중복 세션 원인 추적 로그 3종 추가 (2026-08-20)**
...
- **[수정] HW 인코더 감지가 ffmpeg 신버전에서 항상 실패하던 문제 — 테스트 방식 교체 (2026-08-20)**
...
- **[진단] iOS 잠금화면 썸네일 유실 — 연속 스킵 구간 계측 추가 (2026-08-17, 펜닐님 제보)**
  ...
  - **[수정] 잠금화면 썸네일 — '커버 확정 전 기본 아이콘 노출' 차단 (2026-08-20, 로그로 원인 특정)**
  - **로그 판정(2026-08-20)**: ㉠**빠른 스킵 증상은 개선된 것으로 보인다** — `cover_load_timeout` **0건**, `track_skip_fast` **0건**, `artwork_lost` **0건**, blob 역조회 `found` **210건 전부 true**, `ms_set`의 `art`는 `url` 208 / `default` **3**. 커버 확정이 사실상 전부 성공했다. ㉡**남은 증상**(다른 사이트에서 동영상 재생 후 웹하드로 돌아와 다시 재생하면 썸네일 없음)은 그 `default` 3건이며, **셋 다 완전히 같은 형태**였다.
  ...
  - **[추가 수정] 세션 복원이 누락되는 경로 차단 — 이번 증상의 진짜 원인 (2026-08-20 2차 로그)**
    ...
    - **[치명적 버그 수정] 토큰 초기화 누락으로 복원이 아예 실행되지 않았다 (2026-08-20, 펜닐 "소스는 있는데 로그가 안 찍힌다")**: 위 수정에서 `const _reacqToken = ++this._reacqSeq;`로 썼는데 `this._reacqSeq`가 초기화되지 않아 **`++undefined = NaN`** 이 된다. 그러면 가드의 `this._reacqSeq !== _reacqToken`이 **`NaN !== NaN` → 항상 true**라 `restore()`가 **매번 조기 종료**됐다. 즉 로그가 안 찍힌 게 아니라 **복원 코드가 한 번도 실행되지 않았고**, 세션 복원 자체가 이전보다 **더 나빠진 상태**였다. → `const _reacqToken = (this._reacqSeq = (this._reacqSeq || 0) + 1);`로 수정.
    ...
    - **[후속] 실기기 로그로 동작 확인 + 죽은 변수 정리 (2026-08-20)**
    ...
    - **[3차 시도] 세션 재획득 간격 확대 + 커버 URL 캐시버스팅 (2026-08-20)**
      ...
      - **[회귀 수정 2 — 진짜 원인] 250ms 간격 자체가 곡 전환마다 빈 썸네일을 만들고 있었다 (2026-08-20)**
      ...
      - **[마무리] 곡 전환 시 남아 있던 순간 깜빡임 제거 — 세션을 아예 놓지 않도록 (2026-08-20)**
      ...
      - **[최종] 남은 순간 깜빡임의 진짜 원인 = 재생 중 1초마다의 무조건 metadata 재설정 (2026-08-20)**
...
*최종 업데이트: 2026-08-17 (v5.8.3e, rhwp 0.8.4 기준)*
