...
- **[개선] ffmpeg/ffprobe 경로 설정 안내 문구 보강 (2026-08-24, 한/영)**
  - **배경**: 자동 탐색을 두 파일 모두에 적용했지만, 환경에 따라(예: 웹서버 계정이 `/var/packages/` 를 읽지 못하는 경우) 여전히 못 잡을 수 있다. 그때 사용자가 스스로 복구할 수 있게 **설정 화면에서 바로 안내**한다.
  - **추가 내용**: ①평소에는 자동 감지되므로 **비워 두어도 된다** ②동영상 스트리밍 재생·썸네일 추출 등 ffmpeg 기반 기능이 동작하지 않으면 **전체 경로를 직접 입력하고 [테스트] 버튼**으로 확인 ③시놀로지는 보통 `/var/packages/ffmpegN/target/bin/` 아래에 설치된다(N은 버전, 예: ffmpeg5·ffmpeg7) — 배포 사용자 실측 경로를 그대로 예시로 넣었다.
  - **범위**: `index.php` 의 해당 설명 문자열 한/영 2줄만 확장. 로직·마크업 변경 없음, 문자열에 이스케이프가 필요한 따옴표 없음, PHP 문법 통과, 추가·삭제 파일 0개.
