...
![version](https://img.shields.io/badge/version-v5.8.3f-blue)
...
![rhwp](https://img.shields.io/badge/rhwp-0.8.4-9cf)
...
| 기능 | 설명 |
|---|---|
| 📄 **HWP/HWPX 뷰어 + 편집기** | rhwp 0.8.4 통합 — **자체호스팅 NAS 중 글로벌 유일** |
...
| 🎵 **MP3 플레이어** | 셔플/반복/구간반복(A-B)/볼륨 영구저장, 비주얼라이저, iOS MediaSession |
...
| **한글** | **HWP, HWPX (rhwp 0.8.4 전용 뷰어 + 편집기)** |
...
- **구간 반복(A-B)** — 버튼 한 개로 A 지정 → B 지정 → 해제, 시크바에 구간 표시 (곡 단위 임시 상태)
- 볼륨 영구저장
- 캔버스 비주얼라이저
...
- **rhwp 0.8.4** — HWP/HWPX 뷰어 + 편집기 (Rust+WASM)
...
버전: 0.8.4
파일: assets/rhwp/
업그레이드: rhwp_업그레이드_가이드_v6_7.md 참조
```
...
**현재 버전**: v5.8.3f (rhwp 0.8.4 기반)

### 주요 변경 이력

#### v5.8.3f (2026-08-24) ⭐ 현재

- **[수정] ESC 한 번에 모달 두 개가 함께 닫히던 문제 (2026-08-25, 펜닐 제보)**
  - **증상**: 이미지 미리보기에서 EXIF 창(confirm 모달)을 열고 ESC 를 누르면 **EXIF 와 미리보기가 동시에 닫혔다**. 다른 모달들은 ESC 한 번에 하나씩 닫힌다.
  - **원인 — keydown/keyup 타이밍 어긋남**: 미리보기 모달은 **keyup** 에서 닫힌다(브라우저 비디오 컨트롤이 keydown 을 삼키기 때문에 의도적으로 그렇게 되어 있음). 반면 confirm 모달은 **keydown 캡처**에서 자신을 닫는다. 그래서 keyup 시점에는 `confirm-modal-overlay` 가 **이미 사라져 있어**, keyup 핸들러의 *"confirm 이 떠 있으면 미리보기를 닫지 않는다"* 는 방어(기존 코드)가 **무력화**됐다. 같은 ESC 한 번이 두 모달을 차례로 닫은 것이다.
  - **수정**: confirm 모달의 ESC 분기에서 `App._escConsumedByConfirm = true` 로 **소비 표시**를 남기고, 바로 뒤따르는 keyup 이 그 플래그를 보면 미리보기를 닫지 않고 **플래그만 지우고 반환**한다(1회용). 기존 오버레이 검사 방어는 그대로 두었다 — 두 방어가 서로 다른 타이밍을 담당한다.
  - **플래그를 ESC 분기에서만 세우는 이유**: 버튼 클릭으로 confirm 을 닫을 때는 ESC 가 없으므로 플래그가 서면 안 된다. `confirmModalCancel()` 자체가 아니라 **ESC 처리 지점에서만** 표시하도록 했다(설정 지점 1곳 확인).
  - **검증**: ESC 시퀀스를 시뮬레이션해 3경우 확인 — ①EXIF 열린 상태에서 ESC 두 번 → **첫 번째는 EXIF 만, 두 번째에 미리보기** ②미리보기만 열린 상태 → ESC 한 번에 닫힘(회귀 없음) ③confirm 닫은 뒤 다음 ESC → **플래그가 남아 삼켜지지 않음**.
  - **감사**: app.js **사라진 줄 0**(순수 추가), 추가·삭제 파일 0개, JS·PHP 문법 통과.
  - **확인 필요**: 실제로 ①EXIF 창에서 ESC → EXIF 만 닫히는지 ②한 번 더 ESC → 미리보기 닫히는지 ③다른 모달(가사·확인창 등)에서 회귀가 없는지.

- **[기능] 이미지 미리보기에 EXIF 정보 보기 추가 (2026-08-25, 펜닐 요청)**
  - **위치·범위**: 미리보기 모달 **줌 바의 ⓘ 버튼**(회전·전체화면 옆), **전체 EXIF 태그**를 섹션(FILE/COMPUTED/IFD0/EXIF/GPS 등)별로 표에 표시(펜닐 선택).
  - **서버**: `FileManager::getImageExif()` 신설 + `image_exif` 액션. 권한 검사는 `media_info` 와 **동일한 패턴**(로그인 + 폴더 권한), 경로 해석도 **검증된 기존 방식**(`getRealPath` → `buildPath` → `isPathSafe`)을 그대로 따랐다. 진짜 원격 스토리지(FTP/SFTP/WebDAV/S3)는 로컬 경로가 없어 `exif_read_data` 를 쓸 수 없으므로 `supported:false` 로 응답(SMB 는 마운트되어 있어 진행).
  - **안전장치**: `exif_read_data` 확장 유무 확인 · 확장자를 **JPEG/TIFF 로 제한**(다른 형식은 EXIF 대상이 아님) · 값 길이 300자 제한 · 썸네일 바이너리 등 비표시 값 제외 · 제조사 문자열이 CP949 인 경우 UTF-8 변환 · `try/catch(Throwable)`.
  - **작업 중 잡은 실수 2가지**: ①처음에 `getFullPath()`·`readFileChunk()` 를 호출했는데 **둘 다 존재하지 않는 메서드**였다 → `getMediaInfo()` 가 실제로 쓰는 경로 해석 패턴을 확인해 교체. ②`this.alert(html, title, true)` 로 3번째 인자를 넘겼는데 **`alert(message, title)` 은 인자가 2개**였다(3번째는 무시됨) → 제거. 내부에서 message 를 HTML 로 삽입하므로 표는 그대로 렌더된다. 사용한 JS 헬퍼 7개(`api`·`toast`·`alert`·`escapeHtml`·`currentPreviewPath`·`currentPreviewStorageId`·`currentStorage`)가 모두 실존하는지도 확인했다.
  - **검증**: EXIF 가 든 최소 JPEG 를 직접 만들어 파싱 확인 — `IFD0: Make=TestCam, Model=ModelX`, `EXIF: ISOSpeedRatings=400` 이 섹션별로 정상 추출됨.
  - **감사**: 언어 키 한/영 각 4개 추가(사라진 키 0), 추가·삭제 파일 0개, PHP·JS 문법 통과.
  - **[개선] 읽을 수 있는 요약 + 전체 태그 토글 (2026-08-25, 펜닐 지적)**: 전체 태그를 그대로 보여주니 `FNumber 8/5`·`FocalLength 6249513/1048576`·`UndefinedTag:0xA432`·`MakerNote` 처럼 **사람이 해석할 수 없는 값**이 대부분이었다. → 서버에서 `buildExifSummary()` 로 **요약을 만들어 위에 두고, 전체 태그는 `<details>` 로 접어** 필요할 때만 펼치게 했다.
    - **변환 내용**: 유리수 → 실수(`8/5` → `f/1.6`), 셔터는 1초 미만이면 분수 표기(`1/120초`), 초점거리는 35mm 환산 병기(`6mm (35mm 환산 26mm)`), 촬영일시 구분자 정규화(`2026:08:24` → `2026-08-24`), **GPS 도분초 → 십진수**(`37/1, 23/1, 5372/100` → `37.398256`)로 바꾸고 **지도 링크**를 붙였다. 렌즈명은 표준 태그가 아니라 `UndefinedTag:0xA434`(LensModel)에서 읽는다.
    - **해석 불가 값은 줄 자체를 넣지 않는다** — 빈 항목으로 지저분해지지 않도록. 유리수 파싱은 **0 나눗셈·비숫자·빈 문자열을 모두 null 로 처리**해 안전하다(`1/0`·`abc`·`x/y`·`''` 검증 완료).
    - **검증**: 펜닐이 제공한 **실제 iPhone 16 Plus EXIF** 로 요약 생성 확인 — 카메라 `Apple iPhone 16 Plus` · 렌즈 `iPhone 16 Plus back dual wide camera 5.96mm f/1.6` · 촬영일시 `2026-08-24 15:13:50` · 해상도 `5712 × 4284` · 조리개 `f/1.6` · 셔터 `1/120초` · ISO `50` · 초점거리 `6mm (35mm 환산 26mm)` · GPS `37.398256, 126.987992`.
  - **[정리] 전체 태그에서 불필요 항목 제거 + 알려진 태그 이름 부여 (2026-08-25, 펜닐 지적)**: 전체 태그에 내부 오프셋·바이너리·코드값이 섞여 있어 읽기 어려웠다.
    - **제거**: `THUMBNAIL` **섹션 전체**(내장 썸네일의 압축·오프셋), `FileType`(코드값 — MimeType 이 이미 있음), `html`(`<img>` 태그 조각), `ByteOrderMotorola`, `Thumbnail.*`, `Exif_IFD_Pointer`·`GPS_IFD_Pointer`(내부 오프셋), `MakerNote`(제조사 바이너리), `ComponentsConfiguration`·`SceneType`(표시 불가), `FlashPixVersion`·`ExifVersion`(규격 버전).
    - **이름 부여 후 유지**: 이름만 없을 뿐 의미 있는 태그는 남긴다 — `0xA434`→**LensModel**, `0xA433`→LensMake, `0x9010~0x9012`→OffsetTime 계열(촬영 시각의 시간대), `0xA435`→LensSerialNumber 등. **그 외 `UndefinedTag:*` 는 제거**한다.
    - **작업 중 잡은 연쇄 문제**: 필터가 `UndefinedTag:0xA434` 를 `LensModel` 로 **개명**하는데, 요약 생성 함수는 여전히 원래 키를 먼저 찾고 있었다. 폴백(`?? $exif['LensModel']`)이 있어 우연히 동작했지만 **의도가 어긋난 상태**여서 우선순위를 뒤집었다(LensModel 우선, 원본 키는 폴백으로 유지 — 필터를 거치지 않는 경로 대비).
    - **검증**: 펜닐이 제공한 실제 iPhone EXIF 로 **태그 50 → 32개**로 감소, 렌즈·시간대는 이름을 얻어 유지됨을 확인. 필터 적용 후 **요약 9개 항목이 모두 정상 생성**되는 것까지 통합 확인(카메라·렌즈·촬영일시·해상도·조리개·셔터·ISO·초점거리·GPS).
  - **PNG 등은 그대로 안내 유지**: EXIF 는 JPEG/TIFF 규격의 일부라 PNG·GIF·WebP 는 **애초에 대상이 아니다**. 문구 세분화나 버튼 숨김도 검토했으나 **현 동작이 틀리지 않아 그대로 두기로 결정**(펜닐 판단).
  - **확인 필요**: 실제 사진으로 ①ⓘ 버튼이 줌 바에 보이는지 ②요약이 정상 표시되고 GPS 지도 링크가 열리는지 ③[전체 태그 보기] 토글이 펼쳐지는지.

- **[개선] 시스템 설정 섹션 제목 변경 — 「썸네일 설정」 → 「미디어 · 썸네일 설정」 (2026-08-25, 펜닐 지적)**
  - **이유**: 이 섹션에는 썸네일 관련(사용 여부·크기·캐시) 외에 **ffmpeg/ffprobe 경로 · 하드웨어 가속 · PDF 도구 경로 · 서버 환경**이 함께 있고, 특히 **동영상 트랜스코딩(HLS 스트리밍)이 여기 설정을 그대로 사용**한다. 「썸네일 설정」이라는 이름은 범위를 잘못 알린다.
  - **변경**: `🖼️ 썸네일 설정` → **`🎬 미디어 · 썸네일 설정`** (영문 `🖼️ Thumbnail Settings` → **`🎬 Media & Thumbnails`**). 아이콘도 미디어를 나타내도록 바꿨다.
  - **다른 문구는 그대로 둠**: 섹션 내 나머지 "썸네일" 표현(썸네일 사용·썸네일 크기·PDF 썸네일·썸네일 캐시 삭제)은 **실제로 썸네일만 가리키므로 정확한 표현**이다. ffmpeg 설명은 앞선 작업에서 이미 "동영상 스트리밍 재생(변환)과 썸네일 생성"으로 고쳐져 있다.
  - **범위**: `index.php` 제목 1줄만 교체(같은 문구가 쓰이는 다른 곳 없음 — 전수 확인). 추가·삭제 파일 0개, PHP 문법 통과.

- **[개선] 하드웨어 정보 표시 + 설치 필요 패키지 안내 (2026-08-25, 펜닐 요청)**
  - **배경**: 진단 버튼이 실패 사유는 알려주지만 **"그래서 뭘 설치해야 하는지"** 가 없었다. 배포 사용자는 시놀로지에서 **FFmpeg · SynoCli Video Drivers · SynoCli Video Driver Tools** 3종을 **스스로 찾아 설치**해 해결했는데, 그 정보가 화면 어디에도 없었다. 또 인텔 5세대 이상 리눅스에서 QSV 를 쓰려면 별도 런타임이 필요한데 그것도 안내가 없었다.
  - **① CPU·GPU 모델명 표시**: 리눅스는 `/proc/cpuinfo` 의 `model name`, GPU 는 DRM 노드(`/sys/class/drm/card*/device/`)의 vendor/device ID 로 수집. 윈도우는 `wmic` 사용.
    - **GPU 는 ID 만으로는 무슨 장치인지 알 수 없어**(펜닐 지적: `Intel (0x0412)` 로는 이해 불가) **모델명 조회를 3단계로** 시도한다 — ①`lspci -nn` 출력에서 `[vendor:device]` 매칭 ②`pci.ids` 데이터베이스 조회(`/usr/share/hwdata/`·`/usr/share/misc/`·`/usr/share/` 순으로 탐색) ③둘 다 없으면 종전처럼 벤더명만. 최종 표기는 `<모델명> [8086:0412]` 형태로 **ID 도 함께** 남겨 검색에 쓸 수 있게 했다.
    - **검증**: 실제 형식의 `pci.ids`·`lspci` 출력으로 파싱 확인 — `8086:0412` → *Xeon E3-1200 v3/4th Gen Core Processor Integrated Graphics Controller*(배포 사용자 GPU), `8086:4680` → *AlderLake-S GT1*, `10de:1c03` → *GP106 [GeForce GTX 1060 6GB]*, `1002:67df` → *Ellesmere [Radeon RX 470/480]*, 없는 ID → 벤더만 표시로 폴백.
    - **보안·성능**: `lspci` 는 **`command -v` 로 존재를 확인한 뒤 고정 인자로만** 호출(변수 삽입 없음), `pci.ids` 는 읽기 전용으로 열고 GPU 개수만큼만 조회한다. 진단 버튼을 눌렀을 때만 실행돼 **상시 부하가 없다**. **세대 판별표는 코드에 넣지 않았다** — 표가 계속 낡고 오판 시 잘못된 안내를 하게 되므로 **모델명만 보여주고 판단은 사용자에게** 맡긴다. 값을 못 구하면 해당 줄을 생략한다.
  - **② 실패 종류·환경별 패키지 안내**: 후보별 실패 사유에 따라 서로 다른 안내를 붙인다.
    - **빌드에 인코더 없음** → 시놀로지: 패키지 센터에서 FFmpeg 설치(SynoCommunity 저장소 필요) / 리눅스·윈도우: 해당 인코더 포함 빌드로 교체
    - **QSV 인코딩 실패** → 시놀로지: **SynoCli Video Drivers · SynoCli Video Driver Tools** / 리눅스: `intel-media-va-driver-non-free`, `libmfx-gen1.2`(또는 `libmfx1`)
    - **VA-API 인코딩 실패** → 시놀로지: 위와 동일 / 리눅스: `intel-media-va-driver`(구세대는 `i965-va-driver`), 확인은 `vainfo`
  - **검증**: 배포 사용자 환경(시놀로지 · i3-4150 · 권한 없음)을 재현해 화면 출력을 확인 — CPU/GPU 모델명, 후보별 실패 사유, **그가 직접 찾았던 3개 패키지 안내**, 권한 가능성 문구가 모두 표시된다.
  - **보안**: 새 액션 경로에 **사용자 입력이 전혀 들어가지 않고**(`$_GET`/`$_POST`/`$input` 미사용), `wmic` 는 윈도우 분기 안에서만 호출, 파일·glob·exec 는 모두 오류 억제 처리.
  - **감사**: 언어 키 한/영 각 7개 추가(사라진 키 0, ko 2698·en 2703), 추가·삭제 파일 0개, PHP·JS 문법 통과.
  - **확인 필요**: 실제 화면에서 ①CPU·GPU 줄이 정상 표시되는지(윈도우는 `wmic` 결과) ②실패 항목에 패키지 안내가 붙는지.

- **[정리] 하드웨어 가속 작업 중 발생한 미사용 언어 키 6개 제거 (2026-08-24)**
  - **배경**: 같은 영역(권한 안내)을 여러 차례 개선하면서 **이전 문안의 키가 잔재로 남았다**. `--memberadd` 도입과 분기 재구성으로 쓰이지 않게 된 것들이다.
  - **제거 대상**: `hw_perm_hint`(옛 안내 — 위험한 `usermod -aG videodriver http` 예시 포함) · `hw_perm_restart`(옛 안내) · `hw_syno_overwrite` · `hw_syno_get_first` · `hw_syno_then` · `hw_existing_members`. **6개 모두 이번 세션에 추가된 것으로, 원본(v5.8.3d)에는 없던 키**임을 확인했다.
  - **삭제 전 안전 확인**: 프로젝트 전체(`*.php`·`*.js`·`*.html`)에서 **참조 파일 0개**임을 검색으로 확인. 삭제 후 ①원본 키가 하나도 사라지지 않았는지 ②JSON 유효성 ③**역방향 — 코드에서 사용 중인 `hw_*`/`api_hw_*`/`api_err_*` 키 87개가 모두 언어 파일에 존재하는지**(ko·en 누락 0)까지 검증했다.
  - **결과**: ko 2697 → 2691, en 2702 → 2696. 추가·삭제 파일 0개, PHP·JS 문법 통과.

- **[개선] 하드웨어 가속 권한 문제 — 실제 값으로 채운 조치 명령 제공 (2026-08-24)**
  - **배경**: 진단 버튼이 배포 사용자 환경에서 `renderD128 권한 없음` 을 정확히 짚어냈지만(실화면 확인), 안내가 `usermod -aG videodriver http` 라는 **예시값**이라 실제 웹서버 계정명·그룹명이 다르면 사용자가 직접 알아내야 했다. 원인은 알려주는데 **조치까지 이어지지 않는** 상태였다.
  - **개선 ① 조치 명령 자동 생성**: 서버가 **웹서버 계정**(`posix_getpwuid(posix_geteuid())`, 실패 시 `get_current_user()`)과 **렌더 노드 그룹**(`filegroup()` + `posix_getgrgid()`)을 조회해 그대로 복사해 쓸 수 있는 명령을 만든다. **시놀로지는 `synogroup`, 일반 리눅스는 `usermod`** 로 구분(`/etc/synoinfo.conf`·`/var/packages` 존재로 판정). 화면에 계정·그룹을 함께 보여주고 **[복사] 버튼**을 제공한다. 값을 못 구하면 일반 안내로 대체돼 **안내 자체가 깨지지 않는다**.
    - 검증: `http` + `videodriver` + 시놀로지 → `sudo synogroup --member videodriver http` (배포 사용자 실제 값), `www-data` + `video` + 일반 리눅스 → `sudo usermod -aG video www-data`, 계정·그룹 미확보 시 → 일반 안내로 대체.
  - **개선 ② QSV/NVENC 실패에도 권한 가능성 연결**: 리눅스의 QSV·NVENC 도 내부적으로 `/dev/dri` 렌더 노드를 통해 GPU 에 접근하므로 **권한이 없으면 VA-API 뿐 아니라 이들도 실패**한다. 그런데 ffmpeg 메시지가 `Invalid argument` 처럼 모호해 권한 문제인지 알기 어렵다. → 렌더 노드가 있는데 접근이 막힌 상황이면 각 실패 항목에 *"렌더 노드 권한이 없어 실패했을 수 있습니다"* 를 덧붙인다. **단정하지 않고 '가능성'으로 표시** — 세대 미달·런타임 미설치 등 다른 원인일 수 있어 오진단을 피하기 위함이다.
  - **작업 중 잡은 실수**: 복사 완료 알림에 `this.showToast()` 를 썼는데 **이 프로젝트에 없는 함수**였다(전체에서 내 호출 1건뿐). 실제 알림 함수는 `this.toast()`(532회 사용)이며, 기존 클립보드 구현(앱 비밀번호 복사)의 패턴을 확인해 맞췄다. 사용한 헬퍼 3개(`escapeHtml`·`api`·`toast`)가 모두 실존하는지도 함께 확인했다.
  - **보안**: `posix_*` 는 확장이 없을 수 있어 **전부 `function_exists` 로 감싸고**(6곳) 오류 억제, 생성 명령에 **사용자 입력이 들어가지 않으며**(서버 조회값만), 화면 출력은 `escapeHtml()` 처리. 클립보드는 HTTPS/제스처 제약으로 실패할 수 있어 `execCommand` 폴백을 뒀다.
  - **감사**: 언어 키 한/영 각 6개 추가(사라진 키 0, ko 2683·en 2688), 추가·삭제 파일 0개, PHP·JS 문법 통과. 기존 `hw_perm_hint` 키는 미사용이 됐으나 **제거하지 않고 남겨 두었다**(다른 곳에서 쓰일 여지가 있고 삭제가 더 위험).
  - **[중요 수정] synogroup 덮어쓰기 사고 방지 (2026-08-24, 배포 사용자 실사례)**: 시놀로지의 `synogroup --member` 는 **나열한 계정만 남기고 나머지를 그룹에서 제거하는 덮어쓰기** 명령이다. 종전 안내는 `sudo synogroup --member videodriver http` 형태여서, **그대로 실행하면 기존 멤버가 전부 삭제**된다. 실제 배포 사용자 NAS 의 `videodriver` 그룹에는 `sc-synocli-videodriver`·`sc-synocli-videodriver-tools`·`sc-ffmpeg5`·`sc-ffmpeg7`·`sc-ffmpeg8` 이 들어 있었고, 사용자가 **스스로 `--get` 으로 먼저 확인해** 사고를 피했다. 우리가 위험한 명령을 제공한 셈이라 수정했다.
    - **수정**: 서버가 `posix_getgrgid()['members']` 로 **기존 멤버를 읽어 전부 나열한 뒤 웹서버 계정을 덧붙인** 완성 명령을 만든다. 화면에도 *"synogroup은 나열한 계정만 남깁니다 — 아래 명령에는 기존 멤버가 모두 포함되어 있으니 그대로 실행하세요"* 경고를 함께 표시.
    - **멤버를 못 읽은 경우**: 파괴적 명령을 **아예 만들지 않고** `sudo synogroup --get <그룹>` 조회부터 하도록 안내한다(빈 그룹과 '읽기 실패'를 `$membersKnown` 으로 구분).
    - **이미 멤버인 경우**: *"이미 그룹에 속해 있습니다. 웹서버를 재시작하면 적용됩니다"* 로 안내 — 불필요한 명령 실행을 막는다.
    - **일반 리눅스는 무관**: `usermod -aG` 는 **추가(append)** 방식이라 기존 멤버에 영향이 없다.
    - **보안 필터가 만들 뻔한 2차 손실도 차단(감사에서 발견)**: 멤버명은 표시 명령에 들어가므로 `^[A-Za-z0-9._-]+$` 만 통과시키는데, **걸러진 계정이 명령에서 빠지면 그 계정이 그룹에서 제거된다**(synogroup 이 덮어쓰기이므로). 즉 보안 필터가 오히려 데이터 손실을 만들 수 있었다. → 필터에 걸린 멤버가 **하나라도 있으면 `$membersKnown` 을 false 로 되돌려 완성 명령을 만들지 않고 '조회 먼저' 경로**로 보낸다. 검증 — 정상 멤버는 완성 명령, 이상 멤버 포함 시 조회 안내, 빈 그룹은 완성 명령(덮어쓸 대상이 없어 손실 없음).
    - **검증**: 배포 사용자 실제 그룹 구성으로 생성한 명령이 **사용자가 손으로 만든 것과 완전히 일치**함을 확인. 멤버 미확보·이미 멤버·일반 리눅스 4경우 모두 의도대로 동작.
  - **[재개선] `--memberadd` 를 1순위로 (2026-08-24, 웹 검증 후)**: 시놀로지 문서·커뮤니티를 확인한 결과 ①`synogroup --member` 가 **완전한 덮어쓰기**라는 점이 다수 출처에서 확인됐고(우리 판단이 맞았음) ②최신 DSM 에는 **`--memberadd`** 가 있어 **기존 멤버를 나열할 필요 없이 추가만** 한다는 것을 알았다. → **`sudo synogroup --memberadd <그룹> <계정>` 을 기본으로 제시**하고, 구버전 DSM 대비로 `--member` 완성 명령(기존 멤버 전부 + 계정)을 **대안으로 함께** 보여준다. 대안까지 만들 수 없으면(멤버 미확보) `--get` 조회 명령을 안내한다. 이로써 **덮어쓰기 실수 여지가 구조적으로 사라졌다** — 기본 명령에는 나열이 아예 없다.
  - **[감사에서 발견·수정] 도달 불가 분기**: `--memberadd` 도입으로 시놀로지에서 `fix_cmd` 가 **항상 채워지게** 되면서, 기존의 `else if (res.get_cmd)` 분기가 **영원히 실행되지 않게** 됐다(멤버를 못 읽어도 조회 안내가 표시되지 않는 상태). → 조회 안내를 `alt_cmd` 유무 분기 아래로 옮겨 정상 표시되도록 고쳤고, 죽은 분기는 제거했다. 복사 버튼도 대상 id 를 인자로 받도록 확장(기본값은 기존 동작 유지)해 기본·대안 두 명령 모두 복사 가능하다.
    - 검증: 시놀로지·멤버 읽음 → 기본(`--memberadd`) + 대안(`--member` 전체 나열), 시놀로지·멤버 못 읽음 → 기본 + `--get` 조회 안내, 일반 리눅스 → `usermod -aG` 단독. 분기 구조(`fix_cmd` → `alt_cmd` / `get_cmd` → 일반 안내)와 두 복사 버튼 id 가 `else if` 로 배타 렌더되는 것까지 확인.
  - **개선 ③ 환경별 실행 절차 안내(2026-08-24, 펜닐 지적)**: 종전에는 *"SSH에서 위 명령을 실행한 뒤 웹서버를 재시작해야 반영됩니다"* 한 줄뿐이라 **SSH 켜는 법**과 **웹서버 재시작 방법**이 빠져 있었다. 서버가 이미 판정해 내려주던 `is_synology` 값이 화면에서 **쓰이지 않고 있었으므로**, 이를 이용해 환경별 번호 절차를 표시한다.
    - **시놀로지**: ①제어판 → 터미널 및 SNMP → SSH 활성화 ②SSH 접속해 명령 실행(sudo 계정) ③패키지 센터 → Web Station → 중지 후 시작(확실하게 하려면 NAS 재부팅) ④[테스트] 재실행해 "접근 가능" 확인 ⑤`hw_encoder_cache.json` 삭제 후 재생
    - **일반 리눅스**: ①SSH 접속해 명령 실행 ②`systemctl restart apache2`(또는 nginx·php-fpm) ③④는 동일
    - 캐시 삭제 안내가 **절차 안과 하단에 중복**되던 것도 정리 — 권한 절차가 표시될 때는 하단 안내를 생략한다.
  - **확인 필요**: 배포 사용자 화면에서 ①계정·그룹이 실제 값으로 표시되는지 ②[복사] 버튼 동작 ③QSV 실패 항목에 권한 가능성 문구가 붙는지 ④환경에 맞는 절차(시놀로지용)가 표시되는지.

- **[기능] 하드웨어 가속 진단 버튼 — 어느 단계에서 막혔는지까지 표시 (2026-08-24)**
  - **배경**: 종전에는 **동영상을 실제로 재생해봐야** HW 사용 여부를 알 수 있었고, 실패해도 원인(빌드에 인코더가 없음 / 렌더 노드 권한 / 드라이버)을 구분할 수 없었다. 배포 사용자 환경 확인 과정에서 이 왕복이 반복돼, **버튼 한 번으로 판정과 조치까지** 알 수 있게 했다.
  - **동작**: 시스템 설정 → 하드웨어 가속 옆 **[테스트]** 버튼. 후보(NVENC·QSV·AMF·VA-API)마다 ①**ffmpeg 빌드 포함 여부** ②**실제 1프레임 인코딩**을 확인한다. 실제로 인코딩을 돌리므로 *"목록에는 있는데 막상 안 되는"* 경우도 걸러진다.
  - **표시 내용**: OS·사용 중인 ffmpeg 경로 / `/dev/dri` 렌더 노드 목록과 **접근 가능 여부** / 후보별 ✅❌와 실패 사유 / 실패 시 ffmpeg 출력 마지막 부분(최대 400자). 하나도 안 되면 *"소프트웨어(libx264)로 재생됩니다"*, 렌더 노드 권한이 없으면 **조치 방법**(`usermod -aG videodriver http` 후 웹서버 재시작)을 함께 안내한다. 설정 변경 후 `hw_encoder_cache.json` 삭제 안내도 항상 표시.
  - **실제 사례로 검증된 필요성**: 배포 사용자 `ls -al /dev/dri` 결과가 `crw-rw---- root videodriver renderD128` — **기타 사용자 권한이 없어** 웹서버 계정이 `videodriver` 그룹에 없으면 접근 불가다. 이 경우 코드는 조용히 SW 로 떨어지므로(오류 없음) 진단 버튼이 없으면 원인을 알 수 없다. 버튼은 이를 `권한 없음` 으로 명확히 표시한다.
  - **보안**: `requireRealAdmin()` 필수, **사용자 입력을 일절 받지 않음**(고정된 테스트 명령만 실행), 모든 경로 `escapeshellarg()`, 실패 출력은 `shellOutToUtf8()` 로 인코딩 변환 후 400자로 자름.
  - **작업 중 잡은 실수**: 새 함수를 클래스 메서드처럼 `}` 로 끝냈는데 이 파일의 해당 영역은 **객체 리터럴**이라 `},` 여야 했다. `node --check` 가 다음 메서드에서 `Unexpected token 'async'` 로 잡아줘 **배포 전에 발견**했다 — 문법 검사를 매번 돌리는 이유가 이런 경우다.
  - **감사**: 언어 키 한/영 각 10개 추가(사라진 키 0, ko 2677·en 2682), 추가·삭제 파일 0개, PHP·JS 문법 통과.

- **[기능] 하드웨어 가속 인코더 선택 + VA-API 지원 (2026-08-24)**
  - **배경**: 배포 사용자 환경(시놀로지 네이티브, i3-4150 + Intel HD Graphics 4400)에서 SW 인코딩만 된다는 제보. 하드웨어는 Quick Sync 를 갖췄지만 인텔의 **리눅스용 QSV 런타임(MediaSDK/oneVPL)이 Broadwell(5세대) 이상만 지원**해 `h264_qsv` 를 쓸 수 없다(HD 4400 은 Haswell = 4세대). 반면 **VA-API 는 구세대 인텔 GPU 도 지원**한다. 윈도우는 DXVA/D3D11 경로라 **세대 제한이 없어** 이 문제와 무관하다.
  - **동작 비교표**

    | 상황 | 시도 순서 |
    |---|---|
    | 윈도우 · 자동 | NVENC → QSV → AMF → SW |
    | 윈도우 · QSV 지정 | QSV → SW |
    | 윈도우 · VA-API 지정 | **자동으로 되돌림** → NVENC → QSV → AMF → SW |
    | 리눅스(렌더노드 有) · 자동 | NVENC → QSV → AMF → **VA-API** → SW |
    | 리눅스 · VA-API 지정 | VA-API → SW |
    | 리눅스 · NVENC 지정 | NVENC → SW |
    | 리눅스(렌더노드 無) · 자동 | NVENC → QSV → AMF → SW |
    | 잘못된 설정값 | **자동으로 되돌림** |

  - **설계 원칙 3가지**: ①**VA-API 후보는 리눅스 + 렌더 노드가 있을 때만** 생성된다(`vaapiDevicePath()` 가 윈도우에서 빈 값 반환) ②**특정 인코더를 지정하면 그것만** 시도해 감지 절차가 짧아지고 의도가 명시된다 ③**허용값이 아니거나 해당 후보가 없으면 자동으로 되돌린다** — 잘못된 설정 하나로 하드웨어 가속이 통째로 막히지 않게 하기 위함이다. 어느 경우든 실패하면 **SW 폴백**이 받쳐준다.
  - **속도 관련(오해 방지)**: 감지는 **24시간에 한 번**만 실행되고 `hw_encoder_cache.json` 에 캐시된다. 후보가 하나 늘어도 하루 1초 남짓 차이이며 **재생 속도에는 영향이 없다**. 특정 인코더 지정의 실익은 속도보다 **확실성과 문제 추적**에 있다.
  - **구현 — QSV 와 다른 점**: NVENC/QSV/AMF 는 `-c:v` 만 바꾸면 되지만 VA-API 는 ①입력 **앞**에 `-vaapi_device` ②필터 체인 **맨 끝**에 `format=nv12,hwupload` 가 필요하다. 그래서 명령 조립 함수 **4곳**(메인 파이프·메인 HLS·공유 파이프·공유 HLS)이 헬퍼 `vaapiDeviceArg()`·`vaapiVf()` 를 호출한다. **VA-API 가 아니면 두 헬퍼가 빈 문자열/입력 원본을 돌려주어 기존 명령이 한 글자도 바뀌지 않는다.** 감지 **테스트 명령도 실제 사용과 같은 형태**로 따로 만들어, 테스트만 통과하고 실사용에서 실패하는 일을 막았다.
  - **작업 중 잡은 함정 3가지**: ①**설정 저장소 불일치** — 처음에 `db->load('settings')` 를 읽게 했으나 미디어 설정은 `data/site_settings.json` 에 저장되고 `loadSiteSettingsOnce()` 가 그것을 읽는다. 그대로 뒀으면 **설정을 켜도 반영되지 않았을 것**. ②메인 파이프의 `scaleFilter` 는 `' -vf "..."'` 처럼 **옵션까지 포함한 문자열**이라 표현식만 뽑아 재조립하도록 분기(VA-API 가 아니면 원본 그대로 사용). ③선택 방식으로 확장하며 초기 구현의 `vaapiEnabled()` 가 **죽은 메서드**가 되어 제거 — 그 안에 있던 배경 설명은 헬퍼 묶음 위 주석으로 옮겨 보존했다.
  - **검증**: 위 비교표 8경우를 실제 PHP 로 실행해 전부 기대와 일치함을 확인. 명령 조립도 대조 — **자동/미설정 상태에서 libx264·h264_qsv·필터 유무 모든 조합이 종전과 동일**, VA-API 선택 시 `-vaapi_device '...' -i ... -vf "scale=1280:-2,format=nv12,hwupload"` 로 올바르게 조립됨. 헬퍼 4개 모두 사용 중(죽은 코드 없음), 클로저 내 `$this` 바인딩 확인, 추가·삭제 파일 0개, 언어 키 사라짐 0개, PHP·JS 문법 통과.
  - **[감사에서 발견·수정] 동영상 변환 경로 누락**: VA-API 를 트랜스코딩 조립부 4곳(메인 파이프·메인 HLS·공유 파이프·공유 HLS)에만 적용했는데, **동영상 변환(파일 저장) 기능의 `buildConvCmd` 를 놓쳤다**. 이 경로도 `detectHwEncoder()` 결과를 그대로 쓰므로 VA-API 가 선택되면 **장치 인자·업로드 필터 없이 실행돼 실패**했을 것이다. `detectHwEncoder()` 를 쓰는 경로와 ffmpeg 명령 조립 클로저를 **전수 조사**해 확인했고(총 5곳, `runConv` 는 `buildConvCmd` 를 호출하는 래퍼라 별도 처리 불필요), 같은 방식으로 적용했다. 이 빌더에는 기존 `-vf` 가 없어 업로드 필터를 단독으로 넣는다. 검증 — SW·QSV 는 명령이 종전과 동일, VA-API 만 `-vaapi_device '...' -i ... -vf "format=nv12,hwupload" -c:v h264_vaapi` 로 조립됨.
  - **⚠️ VA-API 실동작 미검증**: 개발 환경에 VA-API 장치가 없어 **실제 인코딩은 확인하지 못했다**(NVENC/QSV/AMF/자동 경로는 종전과 동일하므로 영향 없음). 배포 사용자(i3-4150 시놀로지) 확인 순서 — ①시스템 설정 → 하드웨어 가속에서 **자동** 그대로 두고 `data/hw_encoder_cache.json` 삭제 → 동영상 재생 → `data/hw_detect.log` 에 `[h264_vaapi] OK` 가 뜨는지 ②안 되면 **VA-API 를 직접 지정**해 같은 절차 반복 ③재생·화질 정상 여부. 실패 시 로그의 ffmpeg 원문을 확인하며, 설정을 자동으로 되돌리면 즉시 종전 상태가 된다.

- **[수정] PDF 도구 경로 [테스트] 버튼 오류 2건 (2026-08-24, 펜닐 제보)**
  - **증상**: 시스템 설정 → PDF 도구 경로에서 [테스트]를 누르면 `실행 실패: "'imagick'"…` 이 뜨고 **메시지 글자가 깨져** 판독이 불가했다. 정작 서버 환경 카드에는 `✅ Imagick` 으로 정상 표시되는 상태.
  - **원인 ① — Imagick 은 CLI 명령이 아니다**: 드롭다운에 `imagick` 옵션이 있는데, 테스트 로직은 선택값을 그대로 `exec()` 로 실행한다. Imagick 은 **PHP 확장**이라 실행 파일이 없어 셸이 "내부 또는 외부 명령이 아니다"를 뱉는다. **도구가 없는 것이 아니라 애초에 실행할 수 없는 대상을 exec 한 것**이다.
    - **수정**: `imagick` 선택 + 경로 미입력이면 exec 대신 **확장 검사**로 분기 — `extension_loaded('imagick')` 확인 후 `Imagick::queryFormats('PDF')` 로 **PDF 델리게이트(Ghostscript) 유무까지** 판정하고 `Imagick::getVersion()` 을 버전으로 돌려준다. 확장은 있으나 PDF 미지원인 상태를 따로 구분해 안내(신규 언어 키 `api_err_imagick_missing`·`api_err_imagick_no_pdf`, 한/영). 경로를 함께 입력했다면 CLI 도구를 의도한 것으로 보고 기존 분기로 넘긴다. 호출은 `try/catch(\Throwable)` 로 감쌌다.
  - **원인 ② — 셸 출력 인코딩**: 한국어 윈도우 명령 프롬프트는 오류 문구를 **CP949** 로 내보내는데, 그 바이트를 변환 없이 JSON(UTF-8)으로 응답해 깨졌다.
    - **수정**: 공용 헬퍼 `shellOutToUtf8()` 신설 — **이 프로젝트에 이미 있는 7-zip 출력 처리와 같은 방식**(UTF-8 로 해석되지 않을 때만 CP949 로 보고 변환). 적용 지점은 **3곳**(ffmpeg 테스트·ffprobe 테스트·pdftool 테스트) 으로, 같은 결함이 세 곳에 있었다.
    - **가드**: 이 함수는 **오류 메시지를 만드는 경로**에서 호출되므로, mbstring 이 없는 환경에서 여기서 치명적 오류가 나면 정작 보여줘야 할 실패 사유가 가려진다. → `function_exists()` 로 확인하고 없으면 원문을 그대로 반환한다. **mbstring 미탑재 컨테이너에서 실행해 치명적 오류 없이 원문 보존됨을 확인**(빈 문자열·ASCII·UTF-8 한글·CP949 바이트 4경우).
  - **감사**: api.php **실제 삭제 0줄**(대조에서 잡힌 3줄은 정규식 파싱 아티팩트 — 원본·현재 개수 동일 확인), 언어 키는 추가만(ko 2665→2667, en 2670→2672, 사라진 키 0), PHP 문법·JSON 유효성 통과, 추가·삭제 파일 0개.
  - **확인 필요**: 드롭다운에서 ①**Imagick** 선택 → 버전과 `PDF: PDF` 형태로 성공 표시되는지 ②**pdftoppm/mutool/gs** 를 없는 경로로 지정 → 실패 메시지가 **한글로 깨지지 않고** 보이는지 ③ffmpeg/ffprobe 테스트도 잘못된 경로로 눌러 메시지가 정상 표시되는지.

> **이 버전의 성격**: 설정 화면 안내 문구만 보강한 소규모 버전. 기능·로직 변경 없음.
> 직전 v5.8.3e 에서 ffmpeg 경로 자동 탐색(표시용·트랜스코딩용 양쪽)을 적용했으나, 환경에 따라(예: 웹서버 계정이 `/var/packages/` 를 읽지 못하는 경우) 자동 탐색이 실패할 수 있어 **사용자가 스스로 복구할 수 있도록 설정 화면에서 안내**한다.
>
> **업그레이드 시 참고**: 캐시버스팅이 `APP_VERSION` 기반이라 버전이 바뀌면 **studio 자산 39MB 를 브라우저가 다시 받는다**(현재 `?v=APP_VERSION` 사용 지점 8곳). hwp 에디터 **첫 열기 1회만 느리고 두 번째부터 정상**이면 이 영향이며 결함이 아니다.

- **[개선] ffmpeg/ffprobe 경로 설정 안내 문구 정비 (2026-08-24, 한/영)**
  - **배경**: 자동 탐색을 표시용·트랜스코딩용 양쪽에 적용했지만, 환경에 따라(예: 웹서버 계정이 `/var/packages/` 를 읽지 못하는 경우) 여전히 못 잡을 수 있다. 그때 사용자가 스스로 복구할 수 있도록 설정 화면에서 바로 안내한다.
  - **4차 정정 — 서버환경 카드 문구도 동일 오해(펜닐 지적, 화면 캡처)**: 시스템 설정 → 서버 환경 카드에도 `ffmpeg … 동영상 썸네일 생성` 으로만 표시돼 같은 오해를 준다. 이 문구는 `lang/ko.json`·`lang/en.json` 의 **`ffmpeg_desc`·`ffprobe_desc`·`ffmpeg_missing_desc`·`ffprobe_missing_desc` 4개 키**이며, `app.js` 의 `t('키','기본값')` 폴백에도 같은 문자열이 박혀 있어 **양쪽을 함께** 고쳤다(언어 파일이 실제 표시값, 폴백은 키 누락 시 대비).
    | 키 | 이전 | 이후 |
    |---|---|---|
    | `ffmpeg_desc` | 동영상 썸네일 생성 | **동영상 스트리밍 변환 · 썸네일 생성** |
    | `ffprobe_desc` | 동영상 메타데이터 읽기 | **동영상 길이 · 코덱 정보 읽기** |
    | `ffmpeg_missing_desc` | 동영상 썸네일 생성 불가… | **동영상 스트리밍 재생과 썸네일 생성 불가…** |
    | `ffprobe_missing_desc` | 동영상 메타데이터 읽기 불가… | **동영상 길이 · 코덱 정보 읽기 불가…** |
    영문 4개도 같은 취지로 수정(`Video thumbnails` → `Video transcoding & thumbnails` 등). **언어 파일 값과 app.js 폴백이 일치하는지 대조 확인**했고, 키 개수는 ko 2665·en 2670 그대로(사라진 키 0).
  - **3차 정정(펜닐 지적)**: *"동영상에서 프레임을 추출해 썸네일을 만듭니다"* 만 적으면 **ffmpeg 가 썸네일 전용인 것처럼 오해**된다. 실제로는 `findFfmpeg()` 호출부가 **①`transcodeShareStream()`(HLS 스트리밍 변환) ②`generateVideoThumbnail()` ③`getHwEncoderInfo()`** 세 곳이며, **스트리밍 재생이 주 용도**다. → "동영상 스트리밍 재생(변환)과 썸네일 생성에 쓰입니다"로 고치고, 실패 문구도 "동영상이 재생되지 않거나 썸네일이 안 보이면"으로 두 경우를 모두 담았다. ffprobe 설명도 "길이·메타데이터" → "길이·코덱 등 정보"로 구체화.
  - **1차 문안의 문제 2가지(펜닐 지적)**: ①한 문단에 다 몰아넣어 **너무 길고 읽기 어려웠다** ②*"평소에는 자동으로 감지하므로 비워 두어도 됩니다"* 가 **윈도우도 자동 감지되는 것처럼 오해**를 준다 — 실제 코드상 윈도우 탐색 목록은 `ffmpeg`(PATH)·`C:\ffmpeg\bin\ffmpeg.exe`·`C:\Program Files\ffmpeg\bin\ffmpeg.exe` **3곳뿐**이라 다른 위치에 설치하면 찾지 못한다. 버전별 패키지 자동 탐색(`findVersionedBins`)은 `PHP_OS_FAMILY === 'Windows'` 에서 즉시 빈 배열을 반환하므로 **리눅스·시놀로지 전용**이다.
  - **수정**: "자동 감지" 문구를 **삭제**하고, `<br>` 로 4줄로 나눠 OS별로 안내한다.
    ```
    ffmpeg: 동영상에서 프레임을 추출해 썸네일을 만듭니다. ffprobe: 동영상 길이·메타데이터를 읽습니다(ffmpeg에 함께 포함).
    동영상 재생이나 썸네일이 안 되면 전체 경로를 입력하고 [테스트]를 눌러 확인하세요.
    · 윈도우: ffmpeg.exe / ffprobe.exe 의 전체 경로
    · 시놀로지: 보통 /var/packages/ffmpegN/target/bin/ (N은 버전, 예: ffmpeg5)
    ```
    시놀로지 경로 예시는 배포 사용자 실측값(`ffmpeg5`)을 그대로 썼다. 영문도 같은 구조로 정리.
  - **5차 정정 — 리눅스 줄 추가(펜닐 지적)**: 처음에는 윈도우·시놀로지 2줄만 두었는데, `/var/packages/`는 **시놀로지 고유 구조**여서 일반 리눅스 사용자가 참고할 예시가 없었다. "리눅스(시놀로지)"로 묶는 대안도 검토했으나 경로가 서로 달라 혼동을 주므로 **별도 줄로 분리**했다 — `· 리눅스: 보통 /usr/bin/ffmpeg (which ffmpeg 로 확인)`. 이제 윈도우·리눅스·시놀로지 3줄이라 사용자가 자기 환경 줄만 보면 된다.
  - **주의(기록)**: 이 `<p class="setting-desc">` 는 `echo` 로 **이스케이프 없이 출력**되므로 `<br>`·`<code>` 태그가 그대로 렌더된다(`\n` 은 HTML 에서 공백이 되어 줄바꿈이 안 되므로 `<br>` 를 써야 함).
  - **범위**: `index.php` 설명 문자열 한/영 2줄만 교체. 로직·마크업 구조 변경 없음, PHP 문법 통과, 추가·삭제 파일 0개.

#### v5.8.3e (2026-08-17~08-24)

- **[수정] 트랜스코딩 경로도 버전별 ffmpeg 패키지를 찾도록 — 시놀로지 mkv·mpg 재생 실패 해결 (2026-08-24, 배포 사용자 로그로 확정)**
  - **경과**: 앞선 수정으로 **서버환경 화면은 정상화**됐다(사용자 캡처: `ffmpeg v5.1.10-7 (/var/packages/ffmpeg5/target/bin/ffmpeg)`, `ffprobe v5.1.10-7` — glob 탐색이 `ffmpeg5` 를 정확히 찾음). **그런데 재생은 여전히 실패**했다.
  - **진짜 원인(hls_timing.log 로 확정)**:
    ```
    Unrecognized option 'readrate'.
    Error splitting the argument list: Option not found
    libavcodec 58.35.100   ← ffmpeg 4.x 계열
    ```
    `-readrate` 는 **ffmpeg 5.0 에서 추가된 옵션**인데, 실제 트랜스코딩은 여전히 **DSM 내장 4.1.9** 를 쓰고 있었다. 즉 **화면 표시용 탐색(api.php)과 실제 트랜스코딩용 탐색(api/FileManager.php)이 서로 다른 코드**였고, 앞선 수정은 전자만 고친 것이었다. `hw_detect.log` 가 0바이트였던 것도 같은 맥락(트랜스코딩 경로가 별도).
  - **수정**: `FileManager` 에 자체 메서드 `findVersionedBins()` 신설 후 **`findFfmpeg()`·`findProbeBin()` 두 곳**에 적용 — 버전이 붙은 패키지를 PATH 보다 먼저 시도한다. **api.php 의 동명 전역 함수에 의존하지 않는다** — 이 클래스는 단독 require 될 때를 대비해 자체 보유하는 것이 기존 방침이기 때문(파일 상단 주석과 동일 원칙).
  - **검증**: 배포 사용자의 **실제 경로를 포함해** 버전 추출 확인 — `/var/packages/ffmpeg5/…`→5, `ffmpeg7`→7, `/volume1/@appstore/ffmpeg8/…`→8, 버전 없는 `ffmpeg`·`MediaServer`→0(전부 기대값 일치). 설정 경로 우선순위 보존을 두 함수 모두에서 확인(설정 검사가 glob 보다 앞). 실행 권한 없는 파일 제외·인자 화이트리스트 동일 적용. 시놀로지가 아닌 환경에서는 glob 이 빈 배열이라 무영향(컨테이너 0건 확인).
  - **사용자 측 해결 확인**: 사용자가 **설정에 절대경로를 직접 입력**하자 즉시 정상 재생됐다(캡처: ffmpeg/ffprobe 모두 `정상 작동 (v5.1.10-7)`). 설정값은 모든 경로에서 최우선이므로, 이번 수정은 **경로를 몰라도(패키지센터로 설치해 경로를 알 수 없는 경우) 자동으로 잡히게** 하는 것이다.
  - **감사**: FileManager 실제 삭제 0줄(대조에서 잡힌 1줄은 알려진 정규식 파싱 아티팩트), 추가·삭제 파일 0개, PHP 문법 통과.
  - **✅ 실효 검증 완료 (2026-08-24, 배포 사용자 확인)**: **설정 경로를 비운 상태**에서 **리눅스·시놀로지 양쪽 모두** ffmpeg 를 자동으로 찾아 동영상이 정상 재생됨을 확인했다. 즉 패키지센터로 설치해 **경로를 모르는 사용자도 별도 설정 없이 동작**한다. 우려했던 웹서버 계정의 `/var/packages/` 접근 권한 문제도 실제로는 발생하지 않았다. 수동 입력은 이제 비표준 위치에 설치한 경우의 예비 수단이다.

- **[해결/교훈] HW(QSV) 인코딩 복구 — 원인은 "그래픽 드라이버 업데이트 후 재부팅 미실시" (2026-08-24)**
  - **결론**: 인텔 그래픽 드라이버를 32.0.101.7088 로 올린 뒤 **서버를 재부팅하지 않아** MFX 런타임이 새 드라이버와 맞물리지 않은 상태였다. **재부팅 후 QSV 정상 동작** — `hw_encoder_cache.json` 삭제 후 재감지하니 `"encoder":"h264_qsv"` 로 기록됨(펜닐 확인).
  - **⚠️ 다음에 같은 증상이 나오면 가장 먼저 확인할 것**: `MFX session: -9` / `Failed to find d3d11va adapter` / `Cannot load nvcuda.dll` 류가 뜨면 **드라이버·런타임을 바꾼 뒤 재부팅했는지부터 확인**한다. 로그의 기술적 단서만 쫓다가 이 기본 항목을 놓쳤고, 그 결과 아래 네 가지를 차례로 잘못 짚었다.
  - **이번에 틀린 진단들(기록용 — 같은 길을 다시 가지 않기 위해)**:
    - ❌ **드라이버 롤백 필요** → 아니었음. 최신 드라이버로도 정상 동작한다.
    - ❌ **Oray Display Mirror Driver(원격제어 가상 디스플레이)가 D3D11 어댑터 열거를 방해** → 비활성화해도 증상 동일, 무관했다.
    - ❌ **Apache 가 Session 0(Windows 서비스)이라 GPU 접근 차단** → Jellyfin 공식 문서가 **Windows 헤드리스 서버 QSV 지원을 명시**하며, README 기록상 예전에도 같은 서비스 구성에서 `⚡ HLS HW : Intel` 로 잘 동작했다. 자동 로그인 같은 운영 부담을 질 필요가 없었다.
    - ❌ **ffmpeg 버전 제약(jellyfin-ffmpeg 로 교체 필요 / 7.1 로 다운그레이드)** → 무관. 현재 **ffmpeg 9.0(git master) + QSV 정상 동작** 확인. "최신 ffmpeg 를 못 쓴다"는 제약은 존재하지 않았다.
  - **RDP 관련 주의(별개로 유효)**: 원격 데스크톱 세션에서 ffmpeg 명령을 직접 돌리면 물리 GPU 대신 `Microsoft Basic Render Driver` 가 잡혀 **결과가 실제와 다를 수 있다**. HW 판정은 반드시 **웹서버 프로세스가 남긴 `data/hw_detect.log`**(`php_sapi: apache2handler` 로 확인)를 기준으로 한다.
  - **그럼에도 유효한 이번 수정들**: ①`-f mp4 <NUL>` 감지 테스트는 ffmpeg 신버전에서 **항상 오탐**하던 실제 버그로, 재부팅과 무관하게 고쳐야 했다 ②`hw_detect.log` 가 없었으면 원인 후보를 좁히지도 못했고, 지금 QSV 복구 확인도 이 로그로 한다 ③중복 세션 수정은 `force_sw` 요청 경로에서 **HW 가 살아난 뒤에도 여전히 유효**하다.
  - **진단 로그 정리 안내**: `data/hls_timing.log`·`data/hwp_timing.log`·`data/debug_logs/` 는 확인이 끝나면 삭제해 기록을 멈춘다. **`data/hw_detect.log` 는 남겨두길 권장** — 매번 덮어써 커지지 않고, 드라이버·ffmpeg 를 바꿨을 때 `hw_encoder_cache.json` 만 지우면 바로 결과를 확인할 수 있다.

- **[수정] 시놀로지에서 설치한 ffmpeg 를 못 찾고 DSM 내장 4.1.9 로 떨어지던 문제 (2026-08-24, 배포 사용자 제보)**
  - **증상**: 사용자가 ffmpeg 를 5.x/7.x/8.x 로 재설치했는데 **시스템 설정 → 서버환경에 계속 v4.1.9** 로 표시되고 **ffprobe 는 "미설치"**. 함께 **mkv·mpg 재생 실패**(브라우저 `SRC_NOT_SUPPORT`, "mp4만 가능"). 이전 화면에는 ffmpeg v5.1.6-6 · ffprobe v7.1.3-6 으로 정상 표시됐었다(`-6` 은 시놀로지 패키지 빌드 번호).
  - **원인**: 시놀로지 패키지센터의 ffmpeg 는 **버전이 이름에 붙어** `/var/packages/ffmpeg7/target/bin/ffmpeg` 처럼 설치되는데, 탐색 목록에는 **버전 없는 `ffmpeg`** 만 있었다(`/var/packages/ffmpeg/target/bin/ffmpeg` 등). 그래서 어느 경로도 맞지 않고 **1순위인 PATH 의 `ffmpeg`(= DSM 내장 4.1.9)** 로 떨어졌다. DSM 내장에는 ffprobe 가 없어 "미설치"로 뜨고, 축소 빌드(`encoder_list_len` 8,845 — 개발 서버 13,156 대비)라 mkv·mpg 트랜스코딩도 실패한다. **버전 오표시와 재생 실패가 같은 뿌리**다.
  - **수정**: 공용 헬퍼 `findVersionedMediaBins($bin)` 신설 — `/var/packages/*/target/bin/<bin>` 과 `/volume*/@appstore/*/bin/<bin>` 을 glob 으로 훑어 **실행 가능한 것만** 모으고, 패키지명의 숫자 기준 **내림차순**(ffmpeg8 > 7 > 6 > 5 > 무번호)으로 정렬해 반환한다. 이를 **PATH 보다 앞**에 배치한 곳은 3군데(서버환경의 ffmpeg·ffprobe, 별도 액션의 ffmpeg). **사용자가 설정에 직접 넣은 경로는 여전히 최우선**이다(호출부에서 먼저 push).
  - **작업 중 자작 버그 발견·수정**: 초기 구현은 `dirname()` **2회**로 패키지명을 뽑았는데, 경로 형태에 따라 **폴더 깊이가 달라**(`/var/packages/X/target/bin/` 은 4단계, `/volume1/@appstore/X/bin/` 은 3단계) 둘 다 `target`/`bin` 이 나와 **정렬이 통째로 무력화**됐다. 가짜 트리 검증에서 ffmpeg7·6·무번호가 전부 `target` 으로 출력돼 발견. → 경로 전체를 `/` 로 쪼개 `ffmpeg<숫자>` 형태 조각을 찾는 방식으로 교체.
  - **검증**: 두 경로 형태를 섞은 가짜 트리로 실행 — `ffmpeg8(@appstore) → ffmpeg7(var/packages) → ffmpeg6 → ffmpeg5(@appstore) → 무번호` 순으로 정확히 정렬됐고, **실행 권한 없는 파일은 제외**됐다. 잘못된 인자(`'rm'` 등)는 빈 배열 반환(경로 조작 방지). 시놀로지 경로가 없는 환경에서는 glob 이 빈 배열이라 **기존 동작 무영향**(컨테이너 실행 결과 0건 확인).
  - **표시 개선은 불필요로 판단**: 감지된 경로는 **이미 화면에 표시되고 있었다** — 사용자 화면의 `ffmpeg v4.1.9 … (ffmpeg)` 에서 괄호 안이 감지 경로이며, 절대경로가 아닌 `ffmpeg` 라는 것 자체가 "PATH 의 것을 쓰고 있다"는 신호였다. 별도 UI 추가 없이 그대로 둔다.
  - **감사**: api.php 실제 삭제 0줄(대조에서 잡힌 2줄은 정규식 파싱 아티팩트 — 원본·현재 개수 동일 확인), 추가·삭제 파일 0개, PHP 문법 통과.
  - **확인 필요**: 배포 후 서버환경에서 ①ffmpeg 버전이 설치한 것으로 표시되는지 ②ffprobe 가 인식되는지 ③괄호 안 경로가 `/var/packages/ffmpegN/...` 형태인지 ④mkv·mpg 재생 정상화 여부. 그래도 안 잡히면 사용자가 **설정에 절대경로를 직접 입력**하면 최우선으로 사용된다.

- **[문서] rhwp 업그레이드 가이드 v6.6 → v6.7 개정 (2026-08-24)**
  - **개정 이유**: 0.8.4 배포본에서 **가이드가 잡아내지 못한 결함 2건**이 실사용에서 드러났다. 둘 다 "자산은 정상인데 FileStation 래퍼가 studio 의 변화를 따라가지 못한" 유형이라, 기존 STEP 10 검증(자산 md5·ABI·패치 재적용)으로는 **원리적으로 검출 불가**했다.
  - **구조적 배경(가이드 STEP 0 신설)**: FileStation 은 studio 의 `index.html` 을 **배포하지 않고**, `rhwp_editor.php` 가 자체 HTML 을 만들어 studio 의 JS·CSS 만 로드한다. 따라서 studio 가 `index.html` 안에서 바꾼 것(툴바 마크업·초기화 시점)은 **자동으로 따라오지 않는다** — 이것이 결함 2건의 공통 뿌리다.
  - **STEP 4-b 신설 — studio `index.html` 대조**: 빌드 산출물 배치 후 `sb-*` 클래스 집합과 요소 id 집합을 `rhwp_editor.php` 와 `diff` 로 비교. 클래스만 다르면 레이아웃 구조 변경(마크업 교체), id 가 다르면 JS 바인딩 영향까지 확인. 교체 시 **교체 전후 id 집합 동일**을 필수 확인 항목으로 못박음.
  - **검증 19 신설 — 마크업 동기화**: CSS 에 존재하는 `.sb-*` 클래스가 마크업에서 **실제로 쓰이는지 역방향 확인**하는 스크립트 포함. 결함 A 를 잡는 핵심으로, 자산 md5 대조는 통과하지만 마크업 불일치는 못 잡으므로 별도 수행한다.
  - **검증 20 신설 — 문서 로딩 시간 실측**: `data/hwp_timing.log` 로 구간별 소요를 측정하고 정상 기준표(`wasm_ready` 수백 ms·`ok:true`, 총 3초 안팎)와 대조. 결함 B 처럼 **기능은 정상인데(`loaded:true`) 매번 30초가 낭비되는** 형태는 기능 체크리스트로 절대 안 잡히므로 정량 확인이 유일한 방어선이다.
  - **부록 A 신설 — 일반 원칙 4가지**: ①자산 검증과 통합 검증은 다르다 ②"기능이 된다"와 "제대로 된다"는 다르다 ③**콘솔 로그 의존 감지는 취약하다**(현재 3개: `__rhwpWasmReady`·`__rhwpDocLoaded` 2종 — 업그레이드 때 문구 존재를 확인하는 스크립트 포함, 단 **문자열이 있어도 호출 시점이 바뀌면 무의미**하므로 최종 판정은 검증 20 의 실측으로) ④추측하지 말고 계측한다.
  - **결과물**: `rhwp_업그레이드_가이드_v6_7.md`. README 상단 참조도 v6.6 → v6.7 로 갱신.

- **[수정] HWP 문서 열기가 매번 30초 걸리던 문제 — WASM 대기 상한 30초 → 2초 (2026-08-24, 계측으로 원인 확정)**
  - **계측 실측**(`data/hwp_timing.log`, 956KB 문서):
    ```
    start              +98ms
    input_ready         +0ms   (누적 98ms)
    wasm_ready      +30751ms   {"ok":false}     ← 30초를 꽉 채우고 타임아웃
    fixed_wait_500ms  +502ms
    file_fetched       +21ms   {"bytes":979456}
    inject_done         +1ms
    doc_rendered      +502ms   {"loaded":true}  ← 실제 파싱·렌더는 0.5초
    총 31,875ms
    ```
    **총 31.9초 중 96%가 헛기다림**이었다. 파일 크기·렌더 성능과 무관하며, 문서 자체는 정상적으로 열린다(`loaded:true`).
  - **원인 — 교착**: 이 대기는 studio 가 찍는 `[WasmBridge] WASM 초기화 완료` 콘솔 로그를 후킹해 플래그를 세우는 방식인데, 0.8.x studio 의 해당 코드는 `async initialize(){ if(this.initialized) return; … console.log('[WasmBridge] WASM 초기화 완료') }` 처럼 **문서를 열 때 비로소 호출되는 지연 초기화(lazy init)** 다. 즉 **우리는 "초기화 로그"를 기다리고 studio 는 "파일 주입"을 기다리는** 상태라 플래그가 영원히 서지 않고 상한까지 간다. 펜닐 기억대로 이전 rhwp 버전에서는 즉시 열렸던 이유가 이것으로 설명된다(그때는 초기화 시점이 달랐다).
  - **수정 방침 — 상한 값만 조정**: 대기 자체를 없애지 않았다. 파일 입력 요소나 studio 앱이 아직 준비되지 않은 환경에서 곧바로 주입하면 실패할 수 있어 **짧은 여유(2초)는 남긴다**. 그리고 실패하더라도 **기존 재시도 4회(1.0s·1.5s·2.0s 간격)가 그대로 받쳐주므로 최악의 경우에도 종전과 동일하게 동작**하고, 정상 경로에서는 28초가 사라진다. 플래그가 실제로 서는 환경에서는 종전처럼 즉시 통과한다.
  - **`fixed_wait_500ms`는 유지**: WASM 대기가 짧아진 지금 이 500ms 가 studio 이벤트 바인딩을 기다리는 **유일한 안정화 장치**다. 없앴다가 주입이 실패하면 재시도로 오히려 더 느려지므로 그대로 둔다.
  - **안전성 검증**: `waitForWasmReady()` 를 **주석 제외·상한 값 정규화 후 비교한 결과 종전과 완전히 동일** — 판정 조건(플래그·DOM fallback)·폴링 간격(100ms)·재시도 횟수와 간격·주입 절차 모두 미변경. 실제로 사라진 줄은 `const maxAttempts = 300;` **1줄뿐**이다. 인라인 JS `node --check` 통과, PHP 문법 통과, 추가·삭제 파일 0개, studio 자산 변경 0건.
  - **계측 전송 경로 수정(같은 날)**: 서버 로그가 생성되지 않던 문제 — POST URL 을 상대경로 `'rhwp_editor.php?...'` 로 썼던 것이 원인. hwp 는 **새 탭**으로 열리므로 주소 형태에 따라 어긋날 수 있다. 이미 정상 동작이 확인된 `STREAM_URL` 과 동일하게 **서버가 계산한 스크립트 경로**(`TIMING_URL`)로 바꾸고, 전송 결과(`status=204` 등)를 콘솔에 남겨 다음에 바로 진단되도록 했다.
  - **확인 필요**: 배포 후 hwp 파일을 열어 ①체감상 즉시 열리는지 ②`hwp_timing.log` 의 `wasm_ready` 가 2초 내로 끝나고 총 소요가 3초 안팎인지 ③문서가 정상 렌더되는지(`loaded:true`, 재시도 없이 `ok_first_try`). 진단이 끝나면 `data/hwp_timing.log` 를 지우면 기록이 멈춘다.

- **[진단] HWP 문서 로딩 단계별 계측 추가 (2026-08-24)**
  - **증상**: 에디터로 hwp 파일을 열면 **매번 느리다**(첫 회만이 아님). 미리보기(뷰어)는 빠르다. 펜닐 기억으로는 rhwp 이전 버전에서는 클릭하자마자 열렸다.
  - **왜 계측이 필요한가**: 뷰어와 에디터는 경로가 다르다 — 뷰어는 `rhwp.js` + `rhwp_bg.wasm`(7.7MB)만 쓰지만, 에디터는 studio 번들(1.2MB)과 **CanvasKit(6.8MB)** 까지 거쳐 Skia로 렌더한다. 게다가 파일 주입 절차에 **문서 크기와 무관한 고정 지연**이 있다(WASM 준비 후 `setTimeout 500ms`, 완료 감지는 100ms 폴링이라 실제 완료보다 최대 100ms 늦게 잡힘). 어느 구간이 지배적인지 **실측 없이는 알 수 없어** 추측 수정을 피하려고 계측을 먼저 넣었다.
  - **의심 후보(미확정)**: rhwp **0.8.3** 에 *"CanvasKit이 정확한 glyph run을 재생하도록 개선(#3741)"* 이 있었고 당시 기록에도 **"FileStation 에디터 렌더 경로에 직접 관련"** 이라고 남겨 두었다. 정확도를 높이면서 렌더 비용이 늘었을 수 있다. **0.8.4는 배포 채널 정리 전용이라 엔진 무변경**이므로, 느려진 시점은 0.8.3 쪽으로 좁혀진다. 다만 이는 아직 가설이며 계측 결과로 확인해야 한다.
  - **계측 구간(6개)**: `start` → `input_ready` → `wasm_ready`(ok 여부) → `fixed_wait_500ms` → `inject_done` → `doc_rendered`(loaded·retry). 여기에 파일 수신을 따로 재는 `file_fetched`(바이트 수 + fetch 소요)를 더해 **네트워크·WASM 준비·고정 대기·파싱/렌더**를 분리한다. 재시도 경로(2~4차)도 동일하게 기록된다.
  - **기록 위치**: 브라우저 콘솔(`[hwp_timing] ...`)과 서버 `data/hwp_timing.log`. **서버 기록은 그 파일이 있을 때만** 동작하며(없으면 완전히 꺼짐), 클라이언트는 로딩 완료 시 **한 번만** POST 한다(`keepalive`, 실패해도 무시 — 로딩에 영향 없음).
  - **보안**: 수신 엔드포인트는 기존 `can_read` 권한 검사를 **통과한 뒤**에 위치. 본문 20KB 상한, 파일명은 `basename()`, 수치는 정수 캐스팅, UA는 120자로 자르고 개행 제거(로그 인젝션 차단), 쓰기는 `LOCK_EX`. 저장 내용은 파일명·구간별 소요·UA 앞부분뿐이다.
  - **안전성**: 순수 관측 추가로 **기존 로딩 로직은 한 줄도 바꾸지 않았다**(재시도 횟수·대기 시간·판정 조건 모두 그대로). 인라인 스크립트 `node --check` 통과, PHP 문법 통과, 추가·삭제 파일 0개.
  - **사용법**: `data/hwp_timing.log` 빈 파일 생성 → hwp 파일 열기 → 그 로그와 브라우저 콘솔 확인. 진단이 끝나면 파일을 지우면 기록이 멈춘다.

- **[수정] rhwp 에디터 상단 서식 도구 모음 깨짐 — style-bar 마크업을 0.8.4 구조로 교체 (2026-08-24)**
  - **증상**: 에디터(그냥 열기)의 상단 툴바가 **2줄로 밀리고 그룹 라벨(글꼴 및 간격·글자 모양·색·문단)이 사라짐**. 스타일 드롭다운도 비어 보임. 미리보기(viewer)는 정상.
  - **원인 — 자산이 아니라 마크업 불일치**: 0.8.4 studio 는 툴바를 **`rhwp-studio/index.html` 에 하드코딩**해 두는데, FileStation 은 그 파일을 쓰지 않고 **`rhwp_editor.php` 가 자체 마크업을 만들어** studio 의 JS·CSS 만 로드한다(배포 studio 폴더에 `index.html` 자체가 없음). 그래서 CSS 는 0.8.4 의 리본 구조(`sb-ribbon-group`/`sb-field-grid`/`sb-command-band`/`sb-ribbon-label`)를 전제하는데 **PHP 의 마크업은 옛 구조 그대로**여서 스타일이 걸리지 않았다. 실제로 배포 CSS 에는 `.sb-ribbon-label` 규칙이 있는데 **JS·PHP 어디에서도 그 클래스를 쓰지 않는** 상태였다.
  - **자산은 정상 확인**: `npm pack @rhwp/core@0.8.4` 로 원본을 받아 대조 — `rhwp.js`(404,678B)·`rhwp_bg.wasm`(8,038,570B) **md5 완전 일치**. 즉 studio 재빌드는 불필요했다.
  - **수정**: GitHub `edwardkim/rhwp` **v0.8.4 태그**의 `rhwp-studio/index.html` 에서 style-bar 블록을 그대로 가져와 `rhwp_editor.php` 의 해당 블록(5,018자)을 교체. 그 블록은 순수 HTML 이라(PHP 태그·onclick·data-cmd 0건) 로직 영향이 없다.
  - **안전성 검증**: 교체 전후 **요소 id 32개가 완전히 동일**(사라짐 0 · 신규 0) — JS 바인딩이 깨질 여지가 없다. 태그 균형도 확인(div 14/14, select 4/4, button 22/22, span 41/41, option 22/22). 그룹 라벨 4개(`글꼴 및 간격`·`글자 모양`·`색`·`문단`)가 원본 화면과 일치하게 복원됨.
  - **변경 범위**: `rhwp_editor.php` **1파일뿐** — studio 자산(JS·CSS·wasm·폰트)은 **일절 건드리지 않음**. 추가·삭제 파일 0개.
  - **미확인**: 실기기(브라우저) 렌더링 확인 필요. 확인 순서 — ①상단 툴바가 1줄로 정렬되고 그룹 라벨 4개가 보이는지 ②스타일/글꼴/크기/줄간격 드롭다운 동작 ③굵게·기울임·밑줄·취소선 등 버튼 동작 ④글자색·형광펜 팔레트 ⑤정렬 버튼 ⑥모바일 레이아웃.
  - **별건 유지**: 에디터 첫 로딩이 느린 것은 버전업(작업 당시 5.8.3d→5.8.3e, 이후 5.8.3f)으로 studio 자산 39MB 를 다시 받은 영향으로 보인다(캐시버스팅이 `APP_VERSION` 기반). **버전을 올릴 때마다 첫 열기 1회는 느리고 두 번째부터 정상**이면 이 영향이며 결함이 아니다.

- **[수정] 영상 1개에 임시폴더·ffmpeg 가 2개 생기는 버그 — 세션 재사용 판정 오류 (2026-08-24, 로그로 원인 확정)**
  - **증상**: 동영상 하나를 재생했는데 `data/hls_sessions/` 폴더와 ffmpeg 프로세스가 **2개** 생긴다. 예전에는 없던 현상.
  - **확정 근거(신규 REUSE_SKIP 로그)**: `REUSE_SKIP mismatch=force_sw(1 vs 0) existEncoder=libx264` — 기존 세션을 재사용하지 못한 이유가 **`force_sw` 불일치** 하나로 특정됐고, 곧바로 두 번째 `START ... existing_dirs=2` 가 찍혔다.
  - **원인**: 재사용 판정이 **의미가 다른 두 값을 비교**하고 있었다. 기존 세션 쪽은 `current_encoder` 에 `libx264` 가 들어있는지로 "SW 세션인가"를 **추정**했는데, 그것은 *실제 사용 인코더*이지 *요청 의도*가 아니다. HW 감지가 실패해 **모든 세션이 처음부터 libx264 로 시작**하면 그 값이 항상 `'1'`이 되고, `force_sw` 없는 다음 요청(`'0'`)과 영원히 어긋나 **매번 새 세션이 생긴다**.
  - **왜 지금 드러났나**: HW 가 동작하던 시절에는 세션이 `h264_qsv` 로 시작해 `'0'` 이었고 일반 요청과 일치해 정상 재사용됐다. **HW 가 죽으면서 잠재돼 있던 결함이 표면화**된 것이며, 최근 기능 추가로 생긴 회귀가 아니다.
  - **수정**: 요청 의도(`force_sw`)를 meta.json 에 **그대로 저장**하고, 재사용 판정을 **의도끼리** 비교하도록 바꿨다. 적용 지점 6곳 — 메인/공유 각각의 ①신규 세션 저장 ②재사용 시 meta 재기록에 **보존**(빠지면 다음 판정이 옛 방식으로 되돌아가 재발) ③판정식. 필드가 없는 **기존 meta.json 은 종전 방식으로 판정**해 배포 직후 살아 있는 세션이 갑자기 재사용 불가가 되지 않게 했다(하위호환).
  - **검증**: 판정식을 실제 PHP 로 6경우 실행 — ①HW 시절 세션+일반요청 재사용 ②**HW 실패로 SW 시작+일반요청 → 재사용(버그 해소)** ③force_sw 세션+force_sw 요청 재사용 ④**force_sw 세션+일반요청 → 새 세션(HW→SW 폴백 분리 의도 유지)** ⑤⑥구 meta 하위호환 동작 확인.
  - **남은 것**: 정리 단계의 `taskkill: 액세스가 거부되었습니다 → total_killed=0` 과 `file_delete_failed` 는 별개 문제로 남아 있다(중복이 사라지면 빈도는 줄어든다).

- **[진단] HW 인코딩·중복 세션 원인 추적 로그 3종 추가 (2026-08-24)**
  - **배경**: ffmpeg(8.x/git master)와 인텔 그래픽 드라이버를 올린 뒤 ①HW 가속이 안 되고 SW만 쓰이며 ②동영상 1개 재생에 **임시폴더·ffmpeg 프로세스가 2개** 생기는 문제가 나타났다. 예전에는 없던 증상이다. 그런데 **원인을 좁힐 로그가 없었다** — 감지는 결과만 캐시에 남기고 과정을 기록하지 않았고, 세션이 왜 재사용되지 않고 새로 생겼는지도 기록되지 않았다. 원격 접속(RDP) 세션에서 명령을 직접 돌리면 물리 GPU 대신 `Microsoft Basic Render Driver`가 잡혀 결과가 달라지므로, **웹서버 프로세스가 스스로 남긴 기록**이라야 신뢰할 수 있다.
  - **① `data/hw_detect.log` (신규, 항상 기록)** — `detectHwEncoder()`의 전 과정. ffmpeg 경로, `PHP_SAPI`·OS·`exec` 가용 여부, `-encoders` 조회 성공 여부(길이), **인코더별 빌드 포함 여부**, 후보별 **성공/실패와 실패 시 ffmpeg 출력 마지막 1200자**, 최종 선택 결과. 감지는 24시간에 한 번만 돌고 파일은 **매번 덮어써** 커지지 않는다. `MFX session: -9`(장치 못 잡음)·`Failed to find d3d11va adapter`(어댑터 열거 실패) 같은 실제 원인이 그대로 남는다.
  - **② `REUSE_SKIP` (기존 `hls_timing.log`에 추가)** — 기존 세션을 재사용하지 못하고 새로 만들 때 **어느 조건이 어긋났는지**(storage_id / path / audio / force_sw / client_session / quality / seek)를 값과 함께 남긴다. 특히 `current_encoder`가 SW 폴백으로 `libx264`가 되면 내부 판정값이 바뀌어 다음 요청과 어긋나 **새 세션이 생기는 경로**가 있어 그 확인용이다. 기존 게이트(로그 파일이 있을 때만 기록)를 그대로 따른다.
  - **③ `START` 줄 확장** — 새 세션 생성 시 그 요청의 조건(`force_sw`·`audio`·`quality`·`seek`·`client_session`)과 **당시 세션 폴더 개수**를 함께 남긴다. ②와 짝을 이뤄 "왜 재사용 못 하고 새로 만들었는지"가 완성되고, 폴더가 2개 생길 때 두 `START` 줄을 비교하면 무엇이 달랐는지 바로 보인다.
  - **검증**: ①의 로그 생성 로직을 실제 PHP로 실행해 확인 — 빌드 포함 여부(`build_has_h264_nvenc: YES` 등), 후보별 실패와 ffmpeg 원문, `RESULT: libx264 (CPU)`까지 의도대로 기록됨. PHP 5파일·JS 2파일 문법 통과, 추가·삭제 파일 0개.
  - **감사 주의(기록)**: 실행문 카운터 방식 대조가 **80줄 삭제**로 보고했으나, 같은 줄이 파일 여러 곳에 존재해 생긴 **오탐**이었다. 원본의 각 실행문을 현재 파일에서 **문자열 포함으로 재확인**한 결과 실제로 사라진 것은 **8줄뿐이며 전부 의도한 교체분**(HW 감지 테스트 6줄 + START 로그 1줄 + 파싱 아티팩트 1줄)이었다. 다중 위치에 같은 코드가 있는 파일에서는 카운터 대조가 오탐을 낼 수 있다.

- **[수정] HW 인코더 감지가 ffmpeg 신버전에서 항상 실패하던 문제 — 테스트 방식 교체 (2026-08-24)**
  - **증상**: 동영상 스트리밍이 **SW(libx264)로만** 처리되고 HW 가속이 안 걸린다. `data/hw_encoder_cache.json`을 지워도 재감지 후 다시 `"encoder":"libx264 (CPU)"`로 저장된다(`time` 값은 갱신되므로 감지 자체는 실행됨). 서버 ffmpeg는 **8.1.1**.
  - **원인**: `detectHwEncoder()`의 동작 테스트가 `-f mp4 <NUL>`로 인코딩해 보는 방식이었는데, **mp4 muxer는 seek 가능한 출력이 필요**하고 `NUL`/`/dev/null`은 seek이 안 된다. 그 결과 출력에 `Error opening output file`이 찍히고, 판정이 `['Error','error','unsupported','failed','Cannot','No such','not found']`라는 **지나치게 넓은 키워드 검사**여서 이를 잡아 **HW가 정상이어도 전부 실패로 판정**했다. 구버전 ffmpeg에서는 드러나지 않던 경로다.
  - **컨테이너 ffmpeg 6.1.1로 실측 대조**: 구방식(`-f mp4 /dev/null`)은 **종료코드 0인데 출력에 `Error opening output file`** — 오탐이 그대로 재현됐다. 신방식(`-f null -`)은 정상 인코더에서 **종료코드 0 · `error` 문자열 0건**, 미지원 인코더(`h264_nvenc`)에서는 **종료코드 255**로 실패를 정확히 잡는다(양방향 확인).
  - **수정 3가지**: ㉠출력 muxer를 **`-f null -`** 로 교체 — seek이 필요 없어 **모든 ffmpeg 버전에서 동일하게 동작**하고 인코더는 그대로 돌므로 검증 목적은 유지된다. ㉡판정을 **종료 코드(`exec`의 `$rc === 0`)** 로 변경, `exec`이 막힌 환경에서는 `shell_exec` + **인코더 고유 실패 문구만**(`Error while opening encoder`·`No capable devices found`·`MFX session` 등) 검사하도록 좁혔다. ㉢`-hide_banner`(configuration 문자열 오탐 제거)·`-frames:v 1`·**`-nostdin`** 추가 — 실측 중 `-nostdin` 없이 ffmpeg가 **입력 대기로 멈추는 것**을 확인해 감지가 통째로 지연되는 위험도 함께 막았다.
  - **범위**: `api/FileManager.php` 단일 함수. 삭제된 실행문 6줄은 전부 이 교체분이며 `$devNull`은 `-encoders` 조회에 여전히 쓰여 유지. 다른 곳에 같은 `-f mp4 <NUL>` 패턴 없음(전수 확인).
  - **[사후 정정 2026-08-24]** 이 수정 후에도 한동안 `libx264 (CPU)`가 나왔는데, 그 원인은 이 항목과 무관한 **드라이버 업데이트 후 재부팅 미실시**였다(위 [해결/교훈] 항목 참조). 재부팅 후 `h264_qsv` 로 정상 감지됨. 즉 **이 감지 방식 교체는 옳은 수정이었고**(`-f mp4 <NUL>` 오탐은 실제 버그), 남아 있던 실패는 별개 원인이었다.
  - **적용 후**: `data/hw_encoder_cache.json`을 한 번 더 삭제해야 재감지된다(24시간 캐시). 그래도 `libx264 (CPU)`면 **ffmpeg 빌드에 HW 인코더가 없는 것** — `ffmpeg -encoders | findstr /i "nvenc qsv amf"`로 확인.

- **[조사 종료] 모바일 하단 상태바가 복귀 시 커 보이는 증상 — 크롬 렌더링 버그로 확정, 페이지 코드로 우회 불가 (2026-08-18)**
  - **증상**: 이미 열려 있던 웹하드 탭으로 복귀하면(크롬이 탭을 폐기했다 되살리므로 `init_start`가 찍힌다) 하단 상태바가 커 보인다. **화면을 한 번 탭하거나 새로고침하면 즉시 정상화**된다. 펜닐님 확인으로 **크롬 업데이트 이후 나타난 증상**이다.
  - **결정적 근거 — 탭 전후 값이 완전히 동일**: 증상이 보이는 상태에서 잰 값이 전부 정상이었고(`footer h=32 · bottom=745 · pos=fixed · just=flex-start · pad=4px 10px`, `containerH 745 = realH 745 = innerH 745`, `isMobile:true`), **정상으로 돌아오는 순간(탭 직후)의 값도 완전히 같았다**(`tap_before == tap_after`; 이후 '값이 달라졌을 때만' 기록하도록 바꾼 `tap_change`는 **0건**). 즉 크롬은 **올바른 값을 이미 갖고 있으면서 화면에 반영만 하지 않는다** → JS가 고칠 대상 자체가 없다.
  - **실패한 시도 3가지 (다시 반복하지 말 것)**
    1. **`--app-vh`로 컨테이너 높이 보정** — 높이는 원래부터 정확했다(`diff:0`). 원인이 아니었다.
    2. **강제 리페인트 2단계**(`transform: translateZ(0)` · `height` 1px 넛지) — `repaint_attempt` 로그로 **두 단계 모두 실행된 것이 확인**됐으나 증상 유지. 페이지 안에서 다시 그리게 만드는 방식은 통하지 않는다.
    3. **`position: fixed` → `sticky` 전환** — 원래 증상은 그대로면서, `sticky`는 문서 흐름에 있어 내용이 화면보다 길면 **평소에 푸터가 안 보이고 스크롤 끝에서만 나타나는 새 문제**까지 발생. `#main-screen`이 flex 컨테이너가 아니라 sticky가 하단 고정을 대체할 조건이 아니었다 → **원복**.
  - **계측 코드 전량 제거**: 조사용으로 넣었던 `--app-vh` 보정 · 복귀 재레이아웃 · `restore_measure` / `tap_before`·`tap_after` / `tap_change` · 강제 리페인트를 모두 제거하고, **결론과 실패한 시도를 소스 주석으로 남겼다**(`app.js` 복귀 처리 위치). CSS의 `height: var(--app-vh, 100dvh)` 참조 2곳도 정리 — 대조 결과 `#main-screen` 높이 규칙이 **원본과 완전히 일치**(`100vh` → `100dvh`)한다.
  - **선택: 크롬 업데이트 대기.** 복귀 시 자동 새로고침으로 덮을 수는 있으나 매 복귀마다 전체 재로딩·스크롤 초기화·업로드 중단 위험이 있어 보류했다. **탭 한 번이면 정상화되는 무해한 표시 문제**이고, 렌더링 버그라 크롬 쪽에서 고쳐질 가능성이 있다.

<!-- (원문 보존) 아래는 조사 과정 기록 -->
- **[수정] 모바일에서 브라우저 닫았다 복귀 시 하단 상태바가 화면 중간에 뜨는 증상 (2026-08-17, 펜닐님 제보 — 이전 세션들에서 여러 차례 시도 후 보류했던 건)**
  - **증상**: 브라우저를 닫았다가 웹하드 탭으로 복귀하면 하단 상태바("N개 항목 / Copyright")가 화면 중간에 붙고 그 아래가 빈 공간으로 남는다.
  - **먼저 확인한 것(중요)**: 모바일 상태바는 **이미 `position: fixed; bottom: 0`** 이고, `fixed`의 기준을 깨는 조상(`transform`·`filter`·`will-change`·`contain`)도 **없다**. 즉 "하단에 고정"은 이미 되어 있으며, `bottom:0`을 더 강하게 걸어도 소용이 없다 — **브라우저가 들고 있는 뷰포트 값 자체가 실제 보이는 화면과 어긋난 상태**이기 때문이다(복귀 직후 `vh`/`dvh`가 재계산되지 않는 iOS 동작).
  - **이전 시도가 실패한 이유**: 2026-08-13에 `--real-vh` 변수를 만들었다가 *"CSS 어디에서도 안 쓴다"*는 이유로 제거한 흔적이 있다. **접근이 틀린 게 아니라 변수를 만들고 CSS에 연결하지 않아 효과가 없었던 것**이다(설정 1곳 / 사용 0곳).
  - **수정**: `visualViewport`로 **실제 보이는 높이**를 재서 `--app-vh`에 넣고, **CSS가 그 값을 실제로 쓰게** 연결했다(이번엔 설정 2곳 / **사용 2곳**).
    - JS: `_applyAppVh()`는 `body.is-mobile`일 때만 동작하고, 값이 이전과 같으면 다시 쓰지 않는다(불필요한 스타일 무효화 방지). iOS는 복귀 직후 값이 한 박자 늦게 확정되는 경우가 있어 **0·150·400ms 세 번** 나눠 재는 `_applyAppVhBurst()`를 쓴다. 갱신 시점은 **증상이 나는 순간들** — `pageshow`(bfcache 복귀)·`visibilitychange`(탭 복귀)·`orientationchange`·`visualViewport.resize`.
    - CSS: `body.is-mobile #main-screen`과 `@media (max-width:1024px) #main-screen`에 `height: var(--app-vh, 100dvh)`를 **기존 `100vh` → `100dvh` 폴백 뒤에** 추가. **변수가 없으면(구형 브라우저·JS 미실행) 기존 `100dvh`가 그대로 쓰여 동작이 변하지 않는다.**
  - **검증**: 보정 로직을 파일에서 추출해 실행 — 초기 3연속 측정은 값이 같아 **1회만 기록**(중복 억제 동작), `pageshow` 후 뷰포트가 달라지면 `--app-vh=640px`로 갱신, **PC(`is-mobile` 아님)에서는 기록 0건**(무영향), 등록 이벤트 4종 확인. CSS는 `height` 선언이 **하나도 사라지지 않고 순수 추가**임을 대조로 확인.
  - **[후속] 키보드 부작용 차단 (2026-08-17 자체 감사)**: 처음에는 `visualViewport.resize`도 구독했는데, **소프트 키보드가 올라올 때도 이 이벤트가 발생하고 `height`가 크게 줄어든다**. 그 값을 그대로 쓰면 검색창·이름 변경 등 **입력할 때마다 화면 전체가 키보드 위로 눌린다**(기존 `100dvh`는 키보드에 반응하지 않아 크기가 유지됐다). 이 프로젝트에는 포커스 시 레이아웃을 보정하는 기존 코드가 없어 완충 장치도 없었다. → ㉠**`visualViewport.resize` 구독 제거** — 원래 목적(복귀 시 어긋남 보정)은 `pageshow`·`visibilitychange`·`orientationchange` 세 시점으로 충분하고, `resize`는 위험만 늘린다. ㉡**키보드 가드 추가**(이중 안전장치) — 어떤 경로로 불리든 `window.innerHeight - visualViewport.height > 150`이면 키보드가 떠 있는 것으로 보고 갱신을 건너뛴다(키보드가 뜬 상태에서 `pageshow`/`visibilitychange`가 발생하는 경우 대비). 검증 — 초기 844 기록, 주소창 표시 780 **기록**, 키보드 480·430 **무시**, 키보드 내려감 844 **기록**, 등록 이벤트 3종에 `visualViewport` 구독 **없음**.
  - **[재진단·수정] 진짜 차이는 '목록 재렌더 유무'였다 (2026-08-17, 펜닐님 추가 관찰 "탭 복귀는 정상, 브라우저 종료 후 복귀만 이상")**
    - **앞선 진단 정정**: 처음에는 뷰포트 값 문제로 보고 `--app-vh`로 컨테이너 높이를 맞췄다. 그런데 **탭 복귀는 정상**이라는 관찰이 결정적이었다. 두 경로의 핸들러를 괄호 균형으로 정확히 뽑아 비교하니 — **탭 복귀(`visibilitychange`)는 `loadStorages` + `loadFiles`** 를 호출해 목록을 다시 그리지만, **브라우저 종료 후 복귀(`pageshow`)는 `loadStorages` + 배지만** 호출한다. 즉 차이는 뷰포트가 아니라 **레이아웃 재계산 유무**였다. (※ 중간에 `updateDeviceClass` 차이라고 잘못 짚었는데, 스캐너가 핸들러 경계를 넘어 `resize` 핸들러 내용을 읽은 오진이었다 — 괄호 균형 파싱으로 정정.)
    - **추가 사실**: 기존 `pageshow` 핸들러는 첫 줄이 `if (!event.persisted) return;` 인데, 이전 세션 기록에 **"이 환경에서는 `persisted`가 항상 false"** 라고 남아 있다. 즉 그 핸들러는 사실상 아무것도 하지 않는다. → **기존 핸들러는 건드리지 않고 별도 리스너**를 추가했다.
    - **수정(2단 구조)**: ①**1단계** — 네트워크 없이 `_applyAppVh()` + `_updateStickyTops()`로 sticky 오프셋을 재계산(가볍고 부작용 적음). ②**2단계** — 450ms 뒤 `#main-screen`의 실제 높이와 `visualViewport` 높이를 비교해 **40px 넘게 어긋난 경우에만** `_reloadFilesKeepScroll()`로 목록을 다시 그린다(스크롤 위치 유지). 매 복귀마다 무조건 재로딩하면 네트워크 요청이 늘고 스크롤이 튀므로 **판정을 넣어 어긋난 경우에만** 수행한다. 판정 결과는 `restore_relayout`(컨테이너 높이·실제 높이·diff·수행 여부)으로 로그에 남는다.
    - **검증**: 재레이아웃 로직을 파일에서 추출해 7경우 실행 — 정상(diff 0) 재렌더 **안 함**, 크게 어긋남(diff 100) 재렌더 **1회**, 경계값(diff 40) 안 함, 미세차(diff 10) 안 함, **게시판 화면·비로그인·PC는 재렌더 안 함**(PC는 sticky 재계산도 안 함). 기존 `pageshow` 핸들러의 `persisted` 가드는 **원본 그대로**임을 대조 확인.
  - **[로그 분석·계측 재설계] 2026-08-18 — 컨테이너 높이는 원인이 아님이 확정됐다**
    - **로그가 밝힌 것 3가지**: ㉠`restore_relayout` 2건 모두 `containerH:745 / realH:745 / diff:0` — **`#main-screen` 높이는 실제 화면과 정확히 일치**한다. 즉 `--app-vh`로 컨테이너 높이를 맞추던 방향은 **원인이 아니었다**. ㉡`pageshow` **20회 중 계측은 2회만** 기록됐다 — `pageshow`가 `checkAuth`보다 먼저 발생해 그 시점 `hasUser:false`이고(20회 전부), 판정 안의 `if (!this.user) return;`에 걸려 **정작 필요한 순간에 로그가 유실**됐다. ㉢`persisted`는 20회 전부 `false`이고 `init_start`도 20회 — **브라우저 종료 후 복귀는 bfcache가 아니라 완전한 새 로드**다. 페이지를 처음부터 다시 그리는데도 증상이 나온다는 뜻이라 "복귀 시 레이아웃이 굳었다"는 가설과 맞지 않는다.
    - **계측 재설계(동작 수정 없음)**: ①`is-mobile` 가드를 **측정에는 걸지 않고** 보정 동작(`_applyAppVh`·`_updateStickyTops`)에만 적용 — 클래스가 아직 없는 시점에 로그가 통째로 사라지던 문제 제거. ②`user` 가드보다 **먼저** 기록하도록 순서 변경. ③**푸터를 직접 측정** — `#status-bar`의 `getBoundingClientRect()`(높이·top·bottom)와 computed `position`·`justifyContent`·`padding`, 그리고 `isMobile`·`hasUser`·`containerH`·`scrollHeight`·`realH`·`innerH`를 함께 남긴다. ④**t0(100ms)·t1(800ms)·t2(2000ms) 세 시점**을 비교해 언제 어긋나는지 보이게 했다.
    - **근거 없는 재렌더 제거**: 이전 턴의 "diff > 40이면 목록 재렌더"는 `diff`가 항상 0으로 확인되어 **발동할 수 없는 코드**였다. 네트워크·스크롤 부작용만 남기므로 제거하고, 주석도 실제 동작과 맞게 정정했다(재렌더 호출이 남아 있다는 잘못된 설명 방지).
    - **판별 방법**: 증상 시 `restore_measure`의 `footer.pos`가 `fixed`가 아니거나 `justify`가 `space-between`(=PC 규칙)이면 **`body.is-mobile`이 적용되지 않은 것**이고, `pos:fixed`인데도 `bottom`이 화면 높이와 다르면 **브라우저가 fixed 기준을 잘못 잡은 것**이다. 두 경우의 대응이 완전히 다르다.
    - **검증**: 계측 블록을 추출해 실행 — 로그인 전(`hasUser:false`·`isMobile:false`)에도 t0·t1·t2 **3회 모두 기록**, 증상 상태 재현 시 `footer:{h:120, pos:"static", justify:"space-between", pad:"6px 15px"}`로 PC 규칙 적용 여부까지 판별됨을 확인.
  - **[우회책] 하단 상태바 강제 리페인트 2단 구조 (2026-08-18)**
    - **근거**: 증상이 **보이는 상태에서 잰 값이 6건 전부 정상**이었다 — `footer h=32 · top=713 · bottom=745 · pos=fixed · just=flex-start · pad=4px 10px`, `container 745 = real 745 = inner 745`, `isMobile:true`. 즉 크롬이 값은 맞게 계산해두고 **화면에 반영만 안 한 상태**다. 화면을 한 번 탭하면 낫는 것도 "다시 그리면 해결된다"는 뜻이며, 펜닐님 확인대로 **크롬 브라우저 쪽 문제**다. JS로는 '실제로 그려진 픽셀'을 읽을 방법이 없으므로 추가 재현 로그로는 더 나올 게 없고, **사용자가 탭할 때 일어나는 일을 코드로 대신**하는 것이 남은 방법이다.
    - **구현**: 모바일 복귀 시에만 `#status-bar`를 대상으로 ①**250ms — `transform: translateZ(0)`를 한 프레임만 줬다 되돌림**(합성 레이어만 갱신, 가장 가벼움) ②**1200ms — 높이를 1px 늘렸다 다음 프레임에 되돌림**(크롬이 '바뀐 게 없다'고 무시하는 경우 대비해 리플로우까지 유발). 값을 고치는 게 아니라 그리기를 유발하는 것이라 **레이아웃·스크롤·데이터에 영향이 없다**.
    - **효과 판별용 로그**: 각 단계에서 `repaint_attempt {step, method}`를 남긴다. 다음 로그에서 **1단계만 찍혔는데 정상 → transform으로 해결**, **2단계까지 갔는데 정상 → 리플로우가 필요했던 것**, **둘 다 찍혔는데 여전히 이상 → 페이지 내부 조작으로는 불가**(복귀 시 자동 새로고침 등 다른 접근 필요)로 갈린다. 즉 **우회책 투입 자체가 다음 진단**이다.
    - **검증**: 블록을 추출해 실행 — 두 단계가 순서대로 실행되고 스타일 변경이 `transform="translateZ(0)"` → `transform=""` → `height="33px"` → `height=""` 순으로 일어나 **최종 상태에 흔적이 남지 않음**(둘 다 빈 문자열) 확인. **PC(`is-mobile` 아님)에서는 측정만 하고 `#status-bar`를 전혀 건드리지 않음**도 확인.
    - **결과: 효과 없음 → 코드 제거**. 실기기 로그에서 `repaint_attempt` **두 단계 모두 실행된 것이 확인**됐는데도 증상은 그대로였다. 즉 **페이지 안에서 다시 그리게 만드는 방식으로는 해결되지 않는다**. 효과 없는 코드를 남기면 매 복귀마다 헛돌고 원인 판단도 흐려지므로 제거하고, 왜 제거했는지를 주석으로 남겼다.

  - **[수정] 최종 해법 — 모바일 하단 상태바를 `position: fixed` → `sticky` 로 전환 (2026-08-18)**
    - **논리**: 리페인트로 못 고친다면 **문제가 되는 경로 자체를 안 타게** 하는 수밖에 없다. 크롬이 잘못 다루는 것은 **뷰포트 기준의 `fixed` 요소**이므로, 기준을 **스크롤 컨테이너**로 바꾸는 `sticky` 로 전환하면 그 버그의 영향권을 벗어난다.
    - **구조상 가능한 이유(확인함)**: `#status-bar`는 `#main-screen`의 **직계 마지막 자식**이고, 모바일에서 `#main-screen`은 `overflow-y: auto` 스크롤 컨테이너다. 이 조건에서 `position: sticky; bottom: 0`은 **스크롤 끝에서 하단에 붙는 동작이 `fixed`와 동일**하다.
    - **함께 조정한 것**: ㉠`sticky`는 문서 흐름에 남으므로 뒤 배경이 비치지 않도록 `background`를 명시했다(`fixed`일 때는 불필요했음). ㉡`margin-top: auto`로 내용이 짧을 때도 하단에 밀착. ㉢`fixed` 전제로 넣었던 `body.is-mobile .file-area { padding-bottom: 36px }`는 푸터가 흐름에 들어오면서 **중복 여백**이 되므로 8px로 줄였다(완전히 없애면 마지막 항목이 푸터에 바짝 붙는다). ㉣`right: 0`은 `sticky`에서 불필요해 제거.
    - **사라진 선언 4건 전부 의도한 것**: `position: fixed` · `right: 0` · `padding-bottom: 36px`(전환분 3건) + 이전 턴의 `display: none !important` 1건.
    - **결과: 실패 → 원복.** 실기기 확인 결과 ㉠**원래 증상은 그대로**였고 ㉡**새 문제가 생겼다** — `sticky`는 문서 흐름 안에 있어 내용이 화면보다 길면 **평소에 푸터가 안 보이고 스크롤 끝에서만 나타나며**, `margin-top:auto` 때문에 스크롤 끝에서 늘었다 줄어드는 현상까지 발생했다. `#main-screen`이 flex 컨테이너가 아니고 내용이 화면보다 길어 **`sticky`가 하단 고정을 대체할 수 있는 조건이 아니었다** — "구조상 동일하게 동작한다"는 판단이 틀렸다. → `position: fixed` + `.file-area { padding-bottom: 36px }`로 **원복**(푸터 규칙이 원본과 문자 단위로 일치함을 대조 확인).

  - **[진단] 탭 전후 값 비교 계측 (2026-08-18) — 마지막 진단 카드**
    - **증상 정정(펜닐님)**: "브라우저 종료 후 복귀"는 **이미 열려 있던 탭으로 돌아오는 것**이다(크롬이 탭을 폐기했다 되살리므로 로그에는 `init_start`가 찍힌다). 탭하거나 새로고침하면 즉시 정상화된다.
    - **지금까지 무효였던 시도 3가지**: 강제 리페인트(`transform`·`height` 넛지) / `--app-vh`로 컨테이너 높이 보정 / `fixed → sticky` 전환. 값은 항상 정상(`h=32 · bottom=745 · container 745 = real 745`)인데 화면만 틀리다.
    - **빠져 있던 데이터**: 복귀 직후(100~2000ms)만 쟀을 뿐 **정상으로 돌아오는 순간(탭 직후)의 값을 한 번도 재지 않았다**. → `touchend`에 `tap_before` / `tap_after`(350ms 뒤) 측정을 붙였다. **복귀 후 첫 터치 1회만 기록하고 리스너를 즉시 해제**한다(로그 오염·부하 방지).
    - **판별**: 두 값이 **다르면** 무엇이 바뀌며 고쳐지는지 특정되어 복귀 시 그 값을 강제로 맞추면 되고, **같은데 화면만 달라지면** 페이지 안에서는 해결 불가가 확정된다(그때는 복귀 시 새로고침 등 더 파괴적인 수단만 남는다).
    - **자동 새로고침을 아직 쓰지 않는 이유**: 복귀마다 전체 재로딩(느려짐·데이터), 스크롤·선택 상태 초기화, 판정 오류 시 무한 새로고침 위험, 업로드 중 중단 가능성. 증상 대비 대가가 크다.
    - **[수정] 계측이 안 걸리던 구멍 2개 (2026-08-18, 펜닐님 "로그가 제대로 기록이 안 되는 것 같다")**: 실제로 `tap_*` 기록이 **0건**이었고 원인이 둘이었다.
      - **㉠ 복귀 경로의 절반을 놓치고 있었다**: `_relayoutAfterRestore`가 **`pageshow`에서만** 등록됐는데, 로그상 복귀는 `pageshow` 45회 · `visibility_visible` 25회로 갈린다. **탭 복귀로 돌아온 경우엔 계측이 아예 안 걸렸다**. → `visibilitychange`에도 등록.
      - **㉡ '첫 터치 1회만' 제한이 역효과**: 복귀 직후 스크롤 등 다른 조작을 먼저 하면 그게 첫 터치로 소모돼 **정작 증상이 보일 때의 탭을 놓친다**. → 1회 제한을 없애고 리스너는 앱 수명 동안 한 번만 등록(`_tapProbeBound` 가드), 대신 **탭 전후 값이 실제로 달라진 경우에만** `tap_change`로 기록해 로그 오염을 막는다.
    - **검증**: `pageshow`·`visibilitychange` 양쪽 등록 확인, 값이 안 바뀌는 탭은 **기록 0건**(오염 없음), 값이 바뀌는 탭은 `tap_change {before, after}` 기록, **탭 복귀로도 `restore_measure` 3건 정상 기록**, 반복 복귀에도 **`touchend` 리스너 1개 유지**(중복 없음).
  - **한계(솔직히)**: 이 영역은 이전 세션들에서 여러 번 실패한 곳이다. `visualViewport` 값 자체가 복귀 직후 늦게 확정되면 3회 측정으로도 못 잡을 수 있다. `#main-screen` 높이는 모바일 전 화면에 영향을 주므로 **세로·가로·모달·스크롤 위치**를 함께 확인해야 한다. 되돌리려면 CSS의 `height: var(--app-vh, 100dvh)` 두 줄만 지우면 즉시 이전 동작으로 복귀한다.

- **[진단] iOS 잠금화면 썸네일 유실 — 연속 스킵 구간 계측 추가 (2026-08-17, 펜닐님 제보)**
  - **증상**: 잠금화면에서 목록을 끝까지 스크롤하며 곡을 빠르게 넘기는 중 썸네일이 안 나온다. 여러 차례 대응했는데도 재발.
  - **기존 계측으로 잡히는 것**: `ms_set`이 아트워크 종류(`blob`/`url`/`default`)를 이미 남기고, `artwork_lost`가 metadata·artwork가 **완전히 사라진** 경우를 잡는다. 하지만 **기본 아이콘으로 대체된 경우**는 `artwork.length > 0`이라 `artwork_lost`에 안 걸리고, 왜 커버를 못 찾았는지도 남지 않았다.
  - **유력 지점**: 화면 표시용 커버가 `blob:` URL일 때 잠금화면용으로는 쓸 수 없어(OS가 문서 스코프 blob을 해석 못 함) **캐시를 역조회해 원본 http(s) URL을 찾는데, 못 찾으면 `null`이 되어 기본 음표 아이콘으로 떨어진다**. 곡을 빠르게 넘기면 새 트랙의 커버가 아직 캐시에 등록되기 전이라 이 경로에 걸릴 수 있다.
  - **추가한 계측 3가지**
    - `art_blob_lookup` — blob 역조회 성공 여부(`found`)와 캐시 크기(`cacheSize`). **기본 아이콘으로 떨어진 직접 원인**을 가린다.
    - `ms_set` 확장 — 기존 `art`에 더해 `activeCover`(화면 표시용 커버가 확정됐는지), `trackHasCover`(트랙 자체에 커버 정보가 있는지), `name`(곡명 40자).
    - `track_skip_fast` — 직전 전환과의 간격(`gapMs`)·연속 횟수(`run`)·`from`/`to`. **"얼마나 빨리 넘겼을 때 깨지는가"** 를 로그만으로 판별하기 위한 재현 조건이다.
  - **로그 오염 방지**: `track_skip_fast`는 **1.2초 이내 연속 2회 이상**일 때만 남긴다. 검증 — 2초 간격 전환 3회는 **0건**, 200ms 간격 5회는 `run`이 2→6으로 누적되며 5건 기록.
  - **적용 범위**: 본체(`app.js`)만. 공유 페이지(`fs-audio-player.js`)에는 원래 `_msLog`·`_debugLog`가 없어(진단은 본체 전용) 그대로 두었다.
  - **[수정] 잠금화면 썸네일 — '커버 확정 전 기본 아이콘 노출' 차단 (2026-08-24, 로그로 원인 특정)**
  - **로그 판정(2026-08-24)**: ㉠**빠른 스킵 증상은 개선된 것으로 보인다** — `cover_load_timeout` **0건**, `track_skip_fast` **0건**, `artwork_lost` **0건**, blob 역조회 `found` **210건 전부 true**, `ms_set`의 `art`는 `url` 208 / `default` **3**. 커버 확정이 사실상 전부 성공했다. ㉡**남은 증상**(다른 사이트에서 동영상 재생 후 웹하드로 돌아와 다시 재생하면 썸네일 없음)은 그 `default` 3건이며, **셋 다 완전히 같은 형태**였다.
  - **확정된 원인**: 커버가 확정되기 전에 `_updateMediaSession`이 먼저 한 번 돌아 **'커버 없음' 상태를 잠금화면에 내보낸다**. 로그가 그대로 보여준다 — `ms_set {art:"default", activeCover:null}` → `ms_art_blob_lookup {found:true}` → `ms_set {art:"url", activeCover:"blob"}`. **코드상으로는 즉시 회복되는데 iOS가 두 번째 설정을 반영하지 않는다.** 다음 곡으로 넘기면 정상인 것도 그때는 커버가 처음부터 확정되기 때문이다. 폴더 이미지(`this.cover`)조차 없는 폴더에서 이 구간이 생긴다.
  - **수정**: 마지막으로 성공한 artwork(`_lastGoodArtworkUrl`)를 기억해 두었다가, **커버가 아직 확정되지 않았을 때 그 자리를 메운다**. 즉 '커버 없음'을 한 번도 내보내지 않는다.
    - **폴백 범위를 좁혔다**: `_activeCoverUrl`이 **`undefined`(아직 확정 전)일 때만** 적용하고, 명시적 `null`(= 이 곡은 커버가 없다고 확정)일 때는 적용하지 않는다. 안 그러면 **커버 없는 곡에 이전 곡 커버가 뜨는 부작용**이 생긴다(검증에서 실제로 확인해 범위를 축소했다).
    - **blob은 기억하지 않는다** — 잠금화면이 blob:을 해석하지 못하므로, 역조회로 원본 http(s) URL이 된 것만 저장한다.
    - `ms_set`에 `usedLastGood` 필드를 추가해 **수정이 실제로 동작했는지 로그로 판별**할 수 있게 했다.
  - **룰4**: `app.js`와 공유 페이지 `fs-audio-player.js`가 같은 구조여서 **양쪽 동일 적용**(폴백 1 · 기억 1 · blob 제외 1, 대조 확인).
  - **[추가 수정] 세션 복원이 누락되는 경로 차단 — 이번 증상의 진짜 원인 (2026-08-24 2차 로그)**
    - **1차 수정 후에도 증상 유지**. 새 로그(`usedLastGood` 필드로 배포 반영 확인)를 보면 **첫 재생 1건을 빼고 모든 `ms_set`이 `art:"url"` · `activeCover:"blob"`** 으로 정상이었다 — JS는 매번 artwork를 올바르게 설정하고 있었다.
    - **결정적 단서**: `ms_reacquire_release` **74건** vs `ms_reacquire_done` **73건** — **복원이 한 번 실행되지 않았다**. `_reacquireMediaSession`은 `metadata = null` + `playbackState = 'none'`으로 세션을 놓은 뒤 **`setTimeout(0)`으로 되돌리는데**, 그 사이 화면이 잠기거나 탭이 백그라운드로 가면 **타이머가 스로틀되어 복원이 실행되지 않는다**. 그러면 잠금화면 메타데이터가 **빈 채로 남는다** — "다른 사이트 동영상 보고 와서 재생을 누르면 썸네일이 안 나오고, 다음 곡으로 넘기면 나온다"(그때는 `_loadTrack`이 메타데이터를 새로 설정)와 정확히 맞물린다.
    - **수정**: 복원을 함수로 분리하고 **세 경로에서 시도**한다 — ㉠`setTimeout(0)` ㉡**`requestAnimationFrame`**(백그라운드 타이머가 막혀도 화면이 그려지는 순간 실행) ㉢**복귀(`visibilitychange`) 시 미복원 상태면 즉시 복원**. 토큰(`_reacqSeq`)과 실행 가드(`_reacqRestored`)로 **중복 실행과 오래된 복원을 차단**하고, `reacquire_done`에 `via` 필드를 남겨 어느 경로로 복원됐는지 로그로 판별할 수 있게 했다.
    - **공유 페이지는 대상 아님**: `fs-audio-player.js`에는 `_reacquireMediaSession` 자체가 없다(본체 전용) — 룰4 비대상임을 확인.
    - **[치명적 버그 수정] 토큰 초기화 누락으로 복원이 아예 실행되지 않았다 (2026-08-24, 펜닐 "소스는 있는데 로그가 안 찍힌다")**: 위 수정에서 `const _reacqToken = ++this._reacqSeq;`로 썼는데 `this._reacqSeq`가 초기화되지 않아 **`++undefined = NaN`** 이 된다. 그러면 가드의 `this._reacqSeq !== _reacqToken`이 **`NaN !== NaN` → 항상 true**라 `restore()`가 **매번 조기 종료**됐다. 즉 로그가 안 찍힌 게 아니라 **복원 코드가 한 번도 실행되지 않았고**, 세션 복원 자체가 이전보다 **더 나빠진 상태**였다. → `const _reacqToken = (this._reacqSeq = (this._reacqSeq || 0) + 1);`로 수정.
      - **테스트가 이 버그를 놓친 이유(중요)**: 검증 하네스가 객체 리터럴에 **`_reacqSeq: 0`을 미리 넣어둬** 실제 조건(초기화 없음)을 재현하지 못했다. **하네스가 초기화한 필드가 실제로도 초기화되는지 확인해야 한다.** 하네스에서 그 초기화를 빼고 다시 돌리자 수정 전 코드는 **4경우 전부 `metadata=null` · `done` 0건**으로 버그가 그대로 재현됐고, 수정 후에는 전부 정상 복원됐다.
    - **[후속] 실기기 로그로 동작 확인 + 죽은 변수 정리 (2026-08-24)**
      - **작동 확인**: 새 빌드 로그에 `ms_reacquire_done`의 `via`가 찍히기 시작했고, 그중 **`via:"raf"`가 실제로 나왔다** — 타이머가 스로틀되어 `setTimeout` 복원이 실패했지만 **rAF 경로가 구제한 실사례**다(이전 버전이었다면 그대로 빈 세션으로 남았을 상황). 새 빌드 구간의 `release`/`done`은 **2건/2건으로 정확히 짝이 맞고**, 모든 시점에 `hasMeta:true · artCount:6`으로 메타데이터가 정상 유지된다. (전체 로그의 78 vs 75 차이는 NaN 버그가 있던 옛 빌드 구간이 섞인 것.)
      - **오해 정정**: `artCount:6`을 "iOS가 첫 번째만 시도하는데 6개나 넣었다"고 우려했으나, **6개 모두 같은 URL에 `sizes`만 다른 표준 패턴**이라 문제가 아니다. 코드 주석의 '첫 번째만 시도' 경고는 *서로 다른 URL*을 넣는 경우에 대한 것이다.
      - **죽은 변수 제거**: 복원 보장 수정에 넣었던 `_reacqPendingTrack`이 **쓰기 2곳·읽기 0곳**이었다(직전에 정리한 `_docTitleName`과 같은 패턴). 동적 접근까지 확인 후 2줄 제거. `_reacqRestore`(복귀 시 재시도용, 1629줄에서 읽음)·`_reacqSeq`·`_reacqRestored`는 정상 사용이라 유지.
      - **같은 버그 재발 방지 점검**: `_reacquireMediaSession` 안에서 쓰는 인스턴스 필드를 전수 조사해 **산술(`++`, `+`)에 쓰이면서 초기화가 없는 필드**를 찾았다 — `_reacqSeq` 하나뿐이고 이미 초기화되어 있다. 나머지는 메서드이거나 비교만 하는 값이라 `undefined`여도 안전. 하네스가 미리 초기화한 필드도 `_destroyed` 하나뿐이며 실제 코드에도 초기화가 있어 **이번엔 가려진 버그가 없음**을 확인했다.
    - **[3차 시도] 세션 재획득 간격 확대 + 커버 URL 캐시버스팅 (2026-08-24)**
      - **상황**: 2차 수정 후 로그는 완전히 정상이다 — `release 2 / done 2` 짝 일치, `via` 분포 `raf`/`timeout`, 재생·백그라운드·복귀 전 구간에서 `hasMeta:true · artCount:6`. 재현 시나리오(음악 재생 → 일시정지 → 다른 사이트 동영상 → 복귀 → 재생)가 로그상 정확히 재현되고 메타데이터도 정상 복원되는데 **실기기 잠금화면에는 여전히 썸네일이 안 나온다**(펜닐 확인). 즉 **iOS가 올바른 메타데이터를 받고도 표시하지 않는다**.
      - **가설 2가지에 대응**: ㉠`release` → `done`이 **같은 초에 끝나** iOS가 '동일 세션의 연속 갱신'으로 보고 아트워크를 다시 읽지 않는다 → 복원을 **250ms 뒤**로 미뤄 별개 세션으로 인식될 여지를 준다. ㉡iOS가 **이미 받아둔 이미지**로 보고 갱신을 건너뛴다 → 재획득 때만 커버 URL에 `_ms=<timestamp>`를 붙인다(평상시에는 원본 URL 그대로라 캐시 효율 유지, 서버 `audio_cover`는 이 파라미터를 읽지 않아 무해).
      - **작업 중 잡은 충돌**: rAF는 다음 프레임(약 16ms)에 실행되므로 그대로 두면 **250ms 간격보다 먼저 복원해 ㉠의 의도가 무효**가 된다 → rAF가 간격이 찰 때까지 스스로 다음 프레임으로 미루도록 고쳤다. 검증: 타이머 정상 → `via=timeout` **250ms 후**, 타이머 스로틀 → `via=raf` **259ms 후**(둘 다 간격 준수, 안전망도 유지).
      - **[계측 결함 수정] `art0`가 70자에서 잘려 배포 확인이 불가능했다 (2026-08-24)**: `_msLog`가 artwork URL을 `src.slice(0, 70)`으로 남기는데, 실제 커버 API URL(`api.php?action=audio_cover&storage_id=…&path=…`)이 그보다 길어 **끝에 붙는 `_ms=` 파라미터가 통째로 잘렸다**. 그 결과 로그만으로는 **3차 수정(캐시버스팅)이 배포됐는지 판별할 수 없었다** — "그대로다"는 보고가 새 빌드를 테스트한 것인지 확인이 안 되는 상태였다. → 130자 이하는 전문, 그보다 길면 **앞 70자 + `…` + 뒤 60자**로 남겨 URL 끝을 볼 수 있게 했다. 검증: blob → `blob:`, 짧은 URL → 전문, 긴 URL → 끝의 `_ms=…`까지 표시.
      - **재현 절차 정정(펜닐)**: 정확한 순서는 **음악 재생 → 잠금화면 → 잠금화면에서 일시정지 → 다른 사이트 동영상 → 복귀 → 재생 → 잠금화면**이다. 이전에 이해하던 "웹하드 화면에서 일시정지"와 달리 **일시정지가 잠금화면(백그라운드)에서 일어난다**. 로그상 그 시점은 `ms_pause {vis:"hidden"}`으로 기록되며 `pause_external`이 아닌 자체 정지로 판정된다(잠금화면 버튼 → MediaSession 액션 핸들러 → 우리 togglePlay 경로라 정상).
      - **[해결 확인 2026-08-24]** 실기기에서 재현 절차대로 테스트해 **썸네일 정상 표시 확인**(펜닐). 그 세션 로그 — `art0`에 `_ms=1787191037145` 등이 **URL 끝까지 보여 3차 수정 배포가 확정**됐고, `release/done` **6/6**(복원 누락 0), `via`는 **전부 `timeout`**(250ms 간격이 지켜져 rAF 안전망을 쓸 일이 없었다는 뜻), `ms_set art`는 `url` 14 / `default` 1(첫 재생만). 단 **250ms 간격과 캐시버스팅을 함께 넣었으므로 둘 중 무엇이 결정적이었는지는 확정되지 않는다** — 잘 동작하는 지금 나눠서 실험할 이유는 없다.
      - **[회귀 수정 2026-08-24] 곡 전환 때 빈 썸네일이 잠깐 뜨는 문제 — 캐시버스팅을 재획득 1회로 한정**
        - **증상(펜닐 제보 + 스크린샷)**: 잠금화면에서 다음 트랙으로 넘어가면 **빈 썸네일이 떴다가** 커버가 채워진다. 이전에는 없던 현상 = **이번 수정이 만든 회귀**.
        - **원인**: `_msArtBust`를 해제하지 않아 첫 재획득 이후 **모든 메타데이터 갱신에 `_ms`가 붙었다**. 곡이 바뀌면 커버 URL 자체가 달라지는데 거기에 `_ms`까지 붙어 **매 곡마다 HTTP 캐시를 우회해 새로 받고**, iOS가 이미지를 받는 동안 잠금화면이 비어 보인다. 직전 감사에서 *"값이 재획득 사이에 바뀌지 않으므로 캐시된다"* 고 판단했는데, **곡이 바뀌면 URL의 나머지 부분이 달라진다는 점을 놓친 오진**이었다.
        - **수정**: 재획득 복원 직후 `this._msArtBust = null`로 **즉시 해제**. 캐시버스팅은 **세션 재획득 1회에만** 적용되고, 평상시 갱신(곡 전환·커버 확정·복귀 재동기화)은 **원본 URL 그대로**라 캐시가 적중한다.
        - **검증**: 재획득 전 곡 전환 → 원본 / 재획득 복원 → `_ms` 붙음(의도) / **재획득 후 곡 전환 → 원본**(캐시 적중, 회귀 해소). 재획득 복원 4경로도 재확인 — 실제 시간이 흐르는 하네스에서 `via=timeout` 251ms, `via=raf` 260ms로 정상(간격 준수).
        - **하네스 주의(교훈 추가)**: rAF를 '즉시 실행'으로 스텁한 옛 하네스에서는 250ms 대기 로직이 들어간 뒤 ③rAF 경로가 실패로 보였다 — **스텁이 실제 타이밍을 재현하지 못하면 멀쩡한 코드가 실패로 보인다**. 시간 의존 로직은 실제 시간이 흐르는 하네스로 검증할 것.
      - **[회귀 수정 2 — 진짜 원인] 250ms 간격 자체가 곡 전환마다 빈 썸네일을 만들고 있었다 (2026-08-24)**
        - **1차 회귀 수정(캐시버스팅 1회 한정)으로는 해결되지 않았다**(펜닐: "여전히 빈 썸네일이 뜬다, 매 곡마다"). 다만 ②(다른 사이트 갔다 와서 재생)는 정상이라 캐시버스팅은 제 역할 중이었다.
        - **진짜 원인**: 재획득은 `_reacqDoneFor !== this.currentIndex` 조건이라 **곡이 바뀔 때마다 실행된다**. 그 안에서 `metadata = null`로 세션을 놓고 **250ms 뒤** 복원하므로, **곡 전환마다 잠금화면이 250ms 동안 비었다**. 원래(0ms, 다음 틱)에는 눈에 안 띄던 구간을 내가 250ms로 늘려 **보이게 만든 것** — 명백한 자작 회귀다.
        - **수정**: 간격·캐시버스팅이 실제로 필요한 상황은 **외부에 세션을 뺏긴 뒤 복귀**뿐이므로 그때만 '강한 재획득'을 한다. 플래그 `_needsHardReacq`를 ㉠`pause_external` ㉡**페이지 복귀(`visibilitychange` → visible)** 두 시점에 세우고, 재획득 시 소비(즉시 해제)한다. **복귀를 트리거로 넣은 것이 중요** — 재현 절차의 정지는 잠금화면에서 일어나 **자체 정지로 판정되어 `pause_external`이 뜨지 않기** 때문이다. 잠금화면에서 곡만 자동으로 넘어가는 동안에는 페이지가 계속 hidden이라 플래그가 서지 않는다.
        - **검증**: ①잠금화면 곡 자동 전환 → 복원 **1ms** · `_ms` **미적용**(빈 썸네일 없음) ②복귀 후 재생 → 복원 **251ms** · `_ms` **적용 후 해제**(②기능 유지) ③강한 재획득 직후 다음 곡 → 복원 **1ms**(플래그 소비 확인, 연속 전환에 지연 없음).
      - **[마무리] 곡 전환 시 남아 있던 순간 깜빡임 제거 — 세션을 아예 놓지 않도록 (2026-08-24)**
        - 위 수정으로 ①②는 해결됐으나 **곡 전환 때 빈 카드가 순식간에 한 번 스쳤다**(펜닐: "창 크기가 순간 보인다"). 간격을 0으로 줄여도 **`metadata = null` 자체는 실행되고** 복원은 `setTimeout(…, 0)`이라 빨라야 다음 틱이라, 그 한 프레임 동안 잠금화면 카드가 비었다.
        - **수정**: '강한 재획득' 판정을 **세션을 놓기 전으로 옮기고**, 평범한 곡 전환에서는 **세션을 놓지 않고 메타데이터만 덮어쓴다**. 곡 전환은 어차피 제목·커버가 모두 바뀌므로 갱신이 그대로 반영되며, 놓았다 잡을 이유가 없다 — 세션 해제는 **외부에 뺏긴 뒤 되찾아야 하는 경우**에만 필요하다. `reacquire_release` 로그에 `hard` 필드를 추가해 어느 쪽이었는지 남긴다.
        - **검증**: ①곡 전환 → 스타일 변경 추적상 `metadata=null` **구간 없음**(복원 2ms) ②복귀 후 재생 → `metadata=null` → 250ms 후 복원(의도대로 세션 해제 발생). 복원 3경로도 재확인 — 곡 전환 1ms/`_ms` 미적용, 복귀 250ms/`_ms` 적용 후 해제, 연속 곡 전환 1ms.
      - **[최종] 남은 순간 깜빡임의 진짜 원인 = 재생 중 1초마다의 무조건 metadata 재설정 (2026-08-24)**
        - 세션 미해제로 고친 뒤에도 **곡 전환 때 아주 짧게 한 번 깜빡였다**(펜닐: "2번에서 1번으로 줄었다").
        - **원인**: `_startArtworkMaintenance`가 **재생 중 매초** `_checkAndRestoreArtwork`를 돌리고, 거기서 유실이 없어도 **1초마다 무조건 `_forceRefreshArtwork()`** 를 호출해 **새 MediaMetadata를 설정**하고 있었다. iOS는 설정할 때마다 잠금화면 카드를 다시 그린다. 코드 주석에 *"같은 artwork URL이라 iOS가 보통 다시 그리지 않음"* 이라는 **전제**와 *"실기기 확인 포인트: 1초 재설정 시 제어센터 깜빡임 여부(미검증) — 거슬리면 주기 상향"* 이라는 **미검증 항목**이 나란히 적혀 있었는데, 이번에 그 전제가 **틀렸음이 확인**됐다.
        - **수정 2가지**: ㉠**무조건 갱신 제거** — 실제 유실이 감지된 경우(`needsRestore`)에만 복구한다. 유실 감지는 여전히 매초 수행하므로 **복구 지연은 종전과 동일(최대 1초)** 이고 불필요한 재설정만 사라진다. ㉡**`_updateMediaSession`에 중복 방지** — 제목·아티스트·아트워크가 직전과 완전히 같으면 새 MediaMetadata를 만들지 않는다(세션이 비어 있으면 비교와 무관하게 반드시 설정하므로 복원이 건너뛰어질 일은 없다). 곡 전환 한 번에 이 함수는 여러 번 불리는데(`_loadTrack` 직후·커버 확정 콜백·재획득 복원) 값이 안 바뀐 호출까지 카드를 다시 그리게 하지 않는다.
        - **죽은 변수 제거**: 무조건 갱신을 없애면서 `_lastForceRefreshAt`이 **쓰기 2곳·읽기 0곳**이 되어 함께 제거.
        - **검증**: metadata 정상 → 재설정 **0회**(깜빡임 없음), metadata 유실(null) → **1회**(복구), artwork만 유실 → **1회**(복구). 앞선 수정 회귀도 재확인 — 곡 전환 `metadata=null` 구간 없음(복원 1ms), 복귀 후 재생은 의도대로 세션 해제 후 250ms 복원, 연속 곡 전환 1ms.
      - **[주석 정정]** `_msArtBust`는 재획득 때 갱신되고 **해제되지 않으므로**, 첫 재획득 이후에는 모든 메타데이터 갱신에 같은 값이 계속 붙는다. 처음 적은 *"평상시에는 원본 URL 그대로"* 는 **첫 재획득 전까지만** 참이었다 → 주석을 실제 동작에 맞게 고쳤다(동작 코드는 **한 줄도 바꾸지 않음**, 실행 코드 3줄 그대로임을 대조 확인). 이 프로젝트에서 주석과 실제가 어긋나 헛짚은 전례가 있어(`--real-vh` 죽은 코드, 리페인트 주석) 같은 실수를 남기지 않기 위한 정리다.
      - **의도한 트레이드오프(기록)**: 커버 API는 `Cache-Control: public, max-age=300`(ID3)·`3600`(폴더 이미지)로 캐시되지만 `_ms` 값이 바뀌면 URL이 달라져 **HTTP 캐시를 우회해 다시 받는다**(ETag/304 처리 없음). 재획득은 재생 시작·외부 정지 때만 일어나 빈번하지 않고, 이 파라미터가 증상을 고친 유력 요인이므로 그대로 둔다. 서버 `audio_cover`는 `_ms`를 읽지 않아 무해하고, `_coverBlobCache`·`_lastGoodArtworkUrl`은 원본 URL을 쓰므로 영향받지 않는다.
      - **누적 방지 확인**: `_lastGoodArtworkUrl` 저장이 캐시버스팅 **앞**에 있어 기억되는 것은 항상 파라미터 없는 원본이다. 재획득을 반복해도 `_ms` 파라미터는 **항상 1개**임을 검증했다.
    - **검증**: `_reacquireMediaSession`을 파일에서 추출해 4경우 실행 — ①타이머·rAF 모두 동작 → `done via=timeout` ②타이머만 → `timeout` ③**rAF만(백그라운드 타이머 스로틀) → `raf`로 복원** ④둘 다 막힘(화면 잠김) → metadata `null`로 남지만 **복귀 시 `visible` 경로로 복원**. 모든 경우 `done`은 **정확히 1회**(중복 없음).
  - **검증**: artwork 결정부를 파일에서 추출해 5경우 실행 — ①커버 확정됨 → 그 커버 ②확정 전+폴더이미지 있음 → 폴더이미지 ③확정 전+폴더이미지 없음+기억 없음(첫 재생) → **기본 아이콘** ④확정 전+기억 있음(**이번 증상**) → **직전 커버** ⑤커버가 명시적으로 없는 곡 → **기본 아이콘**(부작용 없음).

- **[수정] 커버 로드 타임아웃 추가 — 기본 아이콘 영구 고정 경로 차단 (2026-08-17)**: 계측만 넣고 로그를 기다리려 했으나, 코드를 따라가다 **로그 없이도 확정되는 결함**을 찾았다.
    - **경로**: 잠금화면 아트워크는 `_updateMediaSession`이 확정해 `_currentMetadata`에 저장하고, 1초마다 도는 `_forceRefreshArtwork()`가 **그 저장본을 그대로 다시 찍는다**. 커버가 늦게 도착하면 `applyCover` → `_updateMediaSession` 재호출로 회복되지만, **`applyCover`가 호출되지 않으면 회복 경로가 없다**.
    - **결함**: ID3 커버 확인용 `new Image()`에 **타임아웃이 없었다**. `onload`/`onerror`가 **둘 다 오지 않으면**(네트워크 정체, 죽은 연결 등) `applyCover`가 영영 호출되지 않고, 미디어세션은 커버 없이 확정된 채 남으며, 1초 갱신이 **기본 음표 아이콘을 계속 다시 찍는다**. 곡을 빠르게 넘길 때 요청이 몰려 이 상태에 빠지기 쉽다 — 펜닐님이 보고한 "간헐적으로 썸네일 안 나옴"과 증상이 일치한다.
    - **수정**: 8초 타임아웃을 걸어 응답이 없으면 **폴더 이미지로 넘어가** 최소한 커버가 뜨게 한다(`cover_load_timeout` 이벤트도 남긴다). 기존 `onload`/`onerror` 경로는 **그대로 두고**, 두 곳에서 타이머를 해제한다. 빠른 트랙 전환으로 이전 로드를 취소할 때도 `testImg._timeoutClear()`로 타이머를 함께 해제해 누적을 막는다.
    - **검증**: 타임아웃 블록을 파일에서 추출해 3경로 실행 — `onload` 정상 → `applyCover(커버)`, `onerror(404)` → `applyCoverCached(폴더이미지)`, **응답 없음 → `cover_load_timeout` 기록 후 `applyCoverCached(폴더이미지)`**. 세 경로 모두 커버 확정 함수가 반드시 한 번 호출되어 기본 아이콘 고정이 발생할 수 없다.
  - **[후속] 한 단계 아래의 같은 함정까지 차단 + 공유 페이지 동기화 (2026-08-17 자체 감사)**: Image 타임아웃만으로는 반쪽이었다. 폴백 경로인 `applyCoverCached(url)` → `_getCachedCoverUrl(url)` 안의 **커버 `fetch`에도 타임아웃이 없어**, 그 요청이 멈추면 Promise가 끝나지 않아 결국 같은 결과(기본 아이콘 영구 고정)에 도달한다. 커버 확정 함수로 가는 경로 3곳이 모두 `applyCoverCached`로 모이므로 하나만 멈춰도 갇힌다. → 공용 헬퍼 `_coverTimeoutSignal(ms)`를 만들어 커버 `fetch`에 **10초** 타임아웃을 걸었다. 타임아웃 시 기존 `catch`가 원본 URL을 반환하므로 흐름이 끊기지 않는다(검증: 응답 없음 → 252ms에 원본 URL 반환, 정상 → blob, 404 → 원본 URL). `AbortSignal.timeout` → `AbortController` → `undefined`(기존 동작) 3단 폴백.
    - **룰4 적용**: 공유 페이지(`fs-audio-player.js`)에도 같은 함수가 있으나 구조가 달라(`_coverFetchQueued`로 iOS 큐를 거침) **각 파일 구조에 맞게** 적용했다 — 공유는 PC 즉시 fetch·iOS 큐 fetch **두 곳 모두**. Image 로드 타임아웃과 전환 취소 시 타이머 해제도 공유에 동일하게 이식했다. 대조 — 양쪽 모두 `setTimeout(커버) 1` · `_timeoutClear 2` · `_coverTimeoutSignal` 적용.
    - **무관하다고 확인한 것**: `app.js`의 나머지 `new Image()` 2곳은 **파일 목록 썸네일**과 **모바일 드래그용 빈 이미지**라 커버 확정과 무관해 손대지 않았다.
  - **읽는 법**: 증상 발생 시 `data/debug_logs/`에서 `ms_track_skip_fast`가 이어지는 구간을 찾고, 그 직후의 `ms_set`이 `art:"default"`인지, 같은 시각의 `ms_art_blob_lookup`이 `found:false`인지 본다. 셋이 맞물리면 **캐시 역조회 실패 → 기본 아이콘**이 확정된다.

- **[수정] 탭 복귀 시 간헐적으로 "로딩 중..." 화면에서 멈추는 증상 (2026-08-17, 펜닐님 제보 — 이전 세션부터 추적)**
  - **증상**: 다른 탭에서 작업하다 한참 뒤 웹하드 탭으로 돌아오면 간헐적으로 목록이 안 뜨고 "로딩 중..."만 보인다. 새로고침하면 정상.
  - **화면이 그렇게 보이는 구조**: `index.php`의 `#file-list` 초기 내용이 정확히 `<div class="empty-msg">로딩 중...</div>`이고, 로그인 상태면 PHP가 `#main-screen`을 이미 `active`로 렌더한다. 즉 **앱 껍데기는 보이는데 JS가 목록을 못 채우면 딱 이 화면**이 된다.
  - **원인 — 시작 체인 한 곳이 멈추면 전체가 멈춤**: `init()` → `await checkAuth()` → `api('me')` → `showMain()` → `loadStorages()` → `loadFiles()`. 확인한 구멍 3개:
    - **① API 요청에 타임아웃이 전혀 없다** — 전 코드에서 abort 타이머는 단 1곳(멀티오디오 info 5초)뿐이었다. `await fetch(...)`가 끝나지 않으면 `await checkAuth()`도 끝나지 않아 **그 뒤 코드가 하나도 실행되지 않는다**. 탭을 오래 두면 죽은 keep-alive 소켓 재사용 등으로 응답이 수 분간 안 오는 경우가 있고, 새로고침이 통하는 것도 새 연결을 열기 때문이다.
    - **② `checkAuth`가 null 가드 없이 `res.success`를 읽었다** — `api()`는 요청이 취소되면 `null`을 반환하므로 TypeError가 나고 `init()`이 통째로 중단됐다(이후 SSO 처리·postMessage 수신 등도 전부 미실행).
    - **③ `init()`에 try/catch가 없어** 한 단계의 예외가 초기화 전체를 죽였다.
  - **수정**
    - `api()`에 **선택적 `timeoutMs` 파라미터**를 추가했다. 기본값 0이면 기존과 완전히 동일(무한 대기)이라 **300여 곳의 기존 호출은 동작이 바뀌지 않는다**. 지정 시 자체 `AbortController`로 감싸고 외부 `signal`도 함께 연결하며, 타임아웃 사유는 `TimeoutError`로 구분해 **'취소(AbortError→null)'와 다르게** 에러 객체(`{error, timeout:true}`)로 반환한다. 타이머는 `finally`에서 정리(재귀 재시도 포함).
    - **일괄 적용은 하지 않았다** — `move`·`copy`·`delete`·`index_rebuild_storage`·`index_sync`·`bulk_search`가 같은 `api()`를 쓰고 대용량에서 정상적으로 수 분 걸린다. 시작 경로 3곳에만 적용: `me` **20초**, `storages` **30초**(기존 3회 백오프 재시도 스케줄러가 이어받음), `files` **60초**(FTP/SMB 대용량 폴더는 정상적으로도 느릴 수 있어 넉넉히). `files`는 실패 시 이미 있는 '⚠️ 에러 표시' 경로를 타므로 모래시계가 남지 않는다.
    - `checkAuth`: null 가드 + 20초 타임아웃 + **타임아웃/연결 실패 시 1회 재시도**(죽은 소켓이면 새 연결로 대개 복구). 세션 만료는 재시도하지 않는다.
    - `init()`: 인증 단계를 try/catch로 감싸고, 실패 시 목록 자리에 **⚠️ + 다시 시도 버튼**을 띄워 무한 로딩에서 빠져나올 수 있게 했다.
  - **검증**: `api()` 본문을 파일에서 그대로 추출해 가짜 fetch로 4개 시나리오 실행 — 정상 응답은 즉시 반환(타임아웃 유무 무관), **응답 없음은 타임아웃 미지정 시 무한 대기 / 2초 지정 시 정확히 2000ms 후 `{timeout:true}` 반환**, 연결 실패는 기존 에러 객체, **외부 signal 취소는 여전히 `null`**(기존 동작 보존). `checkAuth`도 추출해 5개 경로 확인 — 정상→main, 타임아웃 후 재시도 성공→main(2회 호출), 2회 실패→login, 세션만료→login(1회만), 취소(null)→화면 변경 없이 조용히 종료.
  - **[후속] 같은 체인의 남은 구멍 2곳 (2026-08-17, 펜닐님 재확인)**: `api()`에만 타임아웃을 넣어서는 부족했다 — 시작 체인에는 **`api()`를 거치지 않는 생 `fetch`** 가 두 개 더 있었다.
    - **`refreshCsrfToken()`** — `checkAuth`가 **`showMain()` 직전에 `await`** 하는데 `fetch('api.php?action=csrf_token')`에 타임아웃도 signal도 없었다. 여기서 멈추면 `me`가 성공해도 화면은 그대로 "로딩 중..."이다. **`me` 타임아웃으로는 절대 못 막히는 경로였다.**
    - **`api()` 내부 401 복구용 `fetch('api.php?action=me')`** — `api()`의 `timeoutMs` 컨트롤러 밖이라 별도 타임아웃이 필요했다.
    - 둘 다 15초 타임아웃을 걸었다. 공용 헬퍼 `_timeoutSignal(ms)`를 만들어 `AbortSignal.timeout`이 있으면 그것을, 없으면 `AbortController`+`setTimeout`을, 둘 다 없으면 `undefined`를 반환해 **구형 환경에서는 기존 동작(무한 대기)으로 안전하게 되돌아간다**. `refreshCsrfToken`은 실패 시 `false`를 반환하고 호출부는 그 값을 무시하므로 **화면은 정상적으로 뜬다**(토큰만 이전 값 유지 → 이후 POST에서 CSRF 재시도 로직이 처리).
    - 검증: `_timeoutSignal`+`refreshCsrfToken`을 파일에서 추출해 실행 — 정상 1ms/true, **응답 없음 251ms/false**(기존에는 영구 대기), 연결 실패 0ms/false, 구형 환경 폴백 `undefined`. 시작 체인의 `await` 요청 전수 재점검 — `me`(20초)·`csrf_token`(15초)·`storages`(30초)·`files`(60초)·401 복구 `me`(15초) **모두 타임아웃 적용됨**.
  - **[후속] 진단 계측 보강 + 스위치 통일 (2026-08-17)**: 기존 계측은 이전 세션의 진단 대상(**"스토리지를 선택하세요" 멈춤**)에 맞춰 `loadStorages`에만 촘촘했고(18곳), 이번 증상의 핵심 구간인 `me` → `csrf_token` → `showMain`은 사실상 빈칸이었다(`refreshCsrfToken` 0곳, `api()` 0곳, `showMain` 0곳). → 시작 체인에 10개 이벤트를 추가했다: `checkAuth_start`·`checkAuth_me_result`·`checkAuth_me_retry_result`(null/timeout/session_expired 구분), `csrf_start`·`csrf_ok`·`csrf_bad_response`·`csrf_failed`(경과 ms·abort 여부), `showMain`(메인 도달), `api_timeout`(**어느 action이 몇 초에서 끊겼는지**), `api_error`(연결 실패 — 타임아웃과 구분). 이제 `init_start`는 있는데 `init_end`가 없을 때 **어느 단계에서 멈췄는지 로그로 확정**된다.
    - **스위치 통일**: 스위치가 두 개라 놓치기 쉬웠다 — 클라이언트 이벤트는 `data/debug_logs/` **폴더**, 서버 단계·**pending 요청 추적(`client_state`)** 은 `data/session_debug.log` **파일**. 폴더만 만들면 pending 추적이 켜지지 않았다. → `api.php`에서 **폴더가 있고 로그 파일이 없으면 파일을 만들어** 둘을 함께 켠다. **폴더가 없으면 조건이 기존과 완전히 동일**(파일 존재 여부만)이라 평상시 동작은 변하지 않는다. 검증(임시 디렉터리로 로직 재현): 폴더X·파일X → OFF, 폴더X·파일O → ON(기존 방식), **폴더O·파일X → ON + 파일 자동 생성**, 폴더 삭제 후에도 파일이 남아 ON 유지(**끌 때는 파일까지 삭제**해야 함).
    - **[후속] 자체 감사에서 잡은 3건 (2026-08-17)**
    - **① 비로그인 방문자가 `me`를 2회 호출하던 문제(실제 문제, 내가 만든 회귀)**: 재시도 조건을 `(res.timeout || res.error)`로 넓게 잡았는데, `api.php`의 `me`는 **비로그인 시 `{success:false, error:'로그인이 필요합니다'}`** 를 돌려준다(`session_expired` 없음). 그래서 로그아웃 상태로 접속하는 모든 방문자가 `me`를 두 번 호출하고 로그인 화면 표시가 그만큼 늦어졌다. → `api()`의 반환 모양으로 '서버가 답했는가'를 구분해 조건을 좁혔다: **서버 정상 응답은 `success` 키가 있고, 타임아웃은 `timeout:true`, 연결 실패는 둘 다 없다** → `(res.timeout || res.success === undefined)`. 검증 7경우 전부 기대와 일치 — 로그인됨 main/1회, **비로그인 login/1회(2→1회로 복귀)**, 타임아웃→재시도 성공 main/2회, 타임아웃 2회 login/2회, 연결 실패→재시도 성공 main/2회, 세션만료 login/1회, 취소(null) 화면 변경 없이 1회.
    - **② 계측 요청이 자기 자신을 stuck으로 신고하던 문제**: pending 추적 모듈이 `client_state`만 제외하고 **`debug_log`는 추적**했다. 진단을 켠 상태에서 서버가 느리면 계측 요청들이 pending에 쌓여 `stuck_many`(10초+ 5개)를 스스로 유발해 원인 판별에 노이즈가 된다 → `debug_log`도 제외.
    - **③ 잘못 짚었던 수정을 되돌림(정직한 기록)**: `_timeoutSignal` 폴백의 `setTimeout`을 "요청이 끝나면 해제"하려고 abort 리스너에서 `clearTimeout`을 걸었는데, **abort 이벤트는 '취소' 시에만 발생하고 '정상 완료' 시에는 발생하지 않으므로 그 코드는 의도한 일을 하지 않는다**. 주석이 실제 동작과 다른 코드는 없는 것보다 나쁘므로 원복하고, 왜 해제하지 않는지를 주석으로 남겼다(요청이 먼저 끝나면 타이머가 ms 뒤에 이미 끝난 요청을 abort → 무동작, 페이지당 최대 2개라 영향 없음).
  - **평상시 부하(정확한 표현)**: `_debugLog`는 첫 호출에서 서버가 `disabled:true`를 돌려주면 이후 호출을 아예 하지 않는다(폴더가 없으면 그 경로). 추가한 10개도 같은 함수를 쓰므로 **진단을 끈 상태에서는 네트워크 요청이 발생하지 않는다**. 단 "오버헤드 0"은 아니다 — pending 추적 모듈의 두 타이머(10초 heartbeat·3초 stuck 감지)와 `window.fetch` 래핑은 **진단 여부와 무관하게 상시 동작**한다(전송은 pending이 있을 때만, 서버는 로그 OFF면 `session_write_close()` 후 즉시 종료). 또 스위치 통일로 진단이 꺼진 평상시 `api.php` 요청당 stat이 1회 늘었다(`is_file` → `is_file`+`is_dir`).
  - **참고**: 이 증상 추적용으로 이전 세션에서 넣은 계측이 살아 있다 — `app.js` 상단의 pending fetch 추적 모듈(3초마다 5건 이상 지연 시 `stuck_many` 보고, `visibilitychange` 때 pending 보고)과 `loadFiles`의 `show_select_storage_msg` 디버그. 서버 `data/session_debug.log`에 `visibility_visible` 직후 어떤 요청이 pending이었는지 남아 있다면 어느 요청이 멈추는지 확정할 수 있다.
  - **언어키**: `err_request_timeout`, `err_init_failed` ko·en 신설(`retry`는 기존 키 재사용).

- **[기능] 모달로 파일을 열면 브라우저 탭 제목에 파일명 표시 (2026-08-17, 펜닐님 요청)**
  - **증상**: 새 탭으로 파일을 열면 탭에 파일명이 보이는데, **미리보기 모달**(음악·동영상·이미지 등)로 열면 탭에는 사이트명만 떴다.
  - **원인**: `document.title`을 설정하는 곳이 **시스템 설정 저장 시 사이트명 1곳뿐**이었다. 모달은 제목을 건드리지 않는다.
  - **수정**: `_setDocTitle(name)` / `_restoreDocTitle()` 헬퍼를 추가하고 네 지점에 연결했다.
    - `_showPreviewImpl(item)` — **모든 파일 종류의 단일 진입점**이라 이미지·동영상·음악·문서가 한 번에 처리된다. 동영상 사이드 패널의 트랙 전환도 이 함수를 다시 타므로 자동 갱신된다.
    - `hideModal('modal-preview')` — 닫을 때 원래 제목으로 복원. 단 **`keepOpen`(트랙 전환 중)일 때는 복원하지 않는다** — 복원했다가 곧바로 다음 곡 제목으로 덮이면 깜빡이기 때문.
    - 음악 플레이어의 기존 `onTrackChange` 콜백 — 음악은 플레이어 자체 목록에서 곡을 바꿔 `showPreview`를 다시 타지 않으므로 여기서 갱신해야 곡을 넘겨도 탭 제목이 따라온다. **콜백이 이미 있어 FSAudioPlayer 클래스는 건드리지 않았다.**
    - 사이트명 저장 시 `_baseDocTitle`도 함께 갱신 — 안 하면 파일명 뒤에 옛 사이트명이 접미사로 남는다.
  - **표시 형식**: `파일명 - 사이트명`. 파일명이 없으면 사이트명만 표시(방어).
  - **검증**: 파일에서 두 헬퍼 본문을 그대로 추출해 시나리오 실행 — 미리보기 열기 → `휴가영상.mp4 - 내 웹하드`, 목록에서 다음 영상 → `다음편.mp4 - 내 웹하드`, 모달 닫기 → `내 웹하드`, 음악 곡 전환 → `02 본편.mp3 - 내 웹하드`, 이름 없음 → 사이트명만, 특수문자 파일명(`It's "quoted" & <odd>.mp4`) 그대로 표시, 사이트명 변경 후 → 새 이름이 접미사로 반영됨. 모두 통과.
  - **[제거] 재생 중 탭 제목 흐르기(마퀴) — 실기기에서 버벅여 철회 (2026-08-18, 펜닐님 확인)**
    - `document.title`을 350ms마다 갈아끼워 오른쪽→왼쪽으로 흐르게 했으나 **실기기에서 버벅여 오히려 읽기 불편했다**. 백그라운드 탭 스로틀링 대응으로 Web Worker 타이머까지 동원했지만 개선되지 않았다 — **브라우저가 탭 제목 텍스트를 갱신하는 비용 자체가 부드러운 애니메이션에 맞지 않는다**.
    - 제거 범위: 마퀴 본체(`_startTitleMarquee`/`_stopTitleMarquee`, Worker 타이머 + setInterval 폴백), 재생 상태 훅(`_setTitlePlaying`·`_bindVideoTitleMarquee`·`_bindAudioTitleMarquee`), 동영상 2곳·음악 3곳의 호출부, 곡 전환 시 흐르기 재개 코드. 잔존 심볼 **0건**(주석만 남김).
    - **정적 표시는 그대로 유지** — 파일을 열면 `파일명 - 사이트명`, 닫으면 사이트명. 제거 후 대조 확인: 제목 설정 5곳(`_showPreviewImpl`·`vaultPreviewFile`·`showGalleryImage`·`loadBoardPosts`·`viewBoardPost`)·복원 4곳(`closeModal`·`hideModal`·`goHome`·`exitBoardView`) **전부 정상**.
    - **재도전한다면**: 갱신 주기를 1초 이상으로 늘리는 방향뿐인데 그건 '흐른다'기보다 '끊겨 보인다'에 가까워 실익이 없다.
    - **[후속] 죽은 변수 정리 (2026-08-18 자체 감사)**: 마퀴를 걷어내면서 `_docTitleName`의 **유일한 소비자가 사라졌는데 변수는 남았다** — 할당 2곳뿐이고 **읽는 곳 0곳**(원래 `_setTitlePlaying`이 이 값을 읽어 마퀴를 시작했다). 동작에는 영향이 없지만, 이 프로젝트에는 **같은 종류의 죽은 코드로 헛수고한 전례**가 있다(`--real-vh`를 만들어놓고 CSS에서 참조하지 않아 무의미했던 건). 전 파일에서 동적 접근·문자열 조합 참조까지 확인해 할당 2건 외에 참조가 없음을 확정한 뒤 2줄 제거. `_baseDocTitle`은 읽기 3곳 + 사이트명 저장 시 갱신 1곳으로 **정상 사용 중이라 유지**. 검증: 제거 후 13개 시나리오(동영상·음악·게시판·vault·이름 없음·사이트명 변경) 전부 정상, 남은 인스턴스 속성은 `_baseDocTitle` 하나뿐, 제목 설정 5곳·복원 4곳 **9곳 전부 정상**.

  - **[기능·철회됨] 재생 중인 파일은 탭 제목이 오른쪽 → 왼쪽으로 흐른다 (2026-08-17, 펜닐님 요청 → 2026-08-18 제거)**: `document.title`을 350ms마다 갈아끼워 회전시킨다. **파일명 부분만 돌리고 `" - 사이트명"`은 고정**(전체를 돌리면 읽기 어렵다), 이름 끝과 시작 사이에 `·` 여백을 넣어 경계를 구분한다. **18자 이하 짧은 이름은 회전하지 않는다**(어차피 다 보이고, 타이머도 만들지 않는다). 재생 중일 때만 돌고 일시정지·종료·트랙 전환·모달 닫기에서 반드시 멈춘다 — 멈출 때는 잘린 상태로 남지 않게 원래 제목으로 되돌린다.
    - **연결 지점**: 동영상은 이미 있던 `play`/`pause`/`ended` 리스너(`wrap.classList` 토글)에 **한 줄씩만** 얹었다. 음악은 플레이어 생성 직후 `audio` 요소에 1회 바인딩(`_titleMarqueeBound` 플래그로 중복 방지, 요소가 플레이어와 함께 폐기되므로 누수 없음). 곡 전환 시 `_setDocTitle`이 이전 곡의 흐르기를 멈추므로, `onTrackChange`에서 **이미 재생 중이면 다시 시작**한다.
    - **누수 방지**: `_setDocTitle`·`_restoreDocTitle` 진입 시에도 흐르기를 멈춘다(정지 호출 4곳). 검증 — 긴 이름 재생 시 한 글자씩 좌측 이동 확인, 일시정지 시 **잘리지 않은 원래 제목으로 복귀 + 타이머 해제**, 짧은 이름은 타이머 자체를 만들지 않음, 모달 닫기 시 사이트명 복원 + 타이머 정리.
    - **백그라운드 탭에서도 부드럽게 (2026-08-17 펜닐님 지적)**: 처음에는 "브라우저 정책이라 우회 불가"라고 했는데 **그 판단이 과했다**. 두 가지가 있다. ㉠**소리를 내며 재생 중인 탭은 크롬이 타이머 스로틀링 대상에서 제외**한다 — 이 기능이 쓰이는 상황(음악·동영상 재생 중)이 정확히 그 예외라 `setInterval`만으로도 원래 간격이 유지될 가능성이 높다. ㉡그래도 음소거 재생 등 예외가 안 걸리는 경우를 위해 **Web Worker 안의 타이머를 1순위로** 쓴다(워커는 페이지 타이머와 별도로 동작). 워커는 Blob URL로 즉석 생성하고 URL은 바로 revoke하며, 생성이 막히는 환경(CSP 등)에서는 `try/catch`로 **기존 `setInterval`로 자동 폴백**한다. `index.php` 페이지에는 CSP가 걸려 있지 않다(`api.php` 응답 전용)。
    - **[후속] 트랜스코딩 전환 경로 누락 수정 (2026-08-17 자체 감사)**: `playing` 클래스를 다루는 play/pause/ended 리스너가 **두 벌** 있는데(`_showPreviewImpl` 안 · `_bindVideoPlayerEvents()`), 처음에는 앞의 한 벌에만 흐르기 훅을 얹었다. `_bindVideoPlayerEvents()`는 **트랜스코딩/HLS로 video 요소가 교체된 뒤 재바인딩되는 함수**라, 그대로 두면 변환 재생 중에는 제목이 흐르지 않고 전환 직전에 흐르던 것이 pause 신호를 못 받아 모달을 닫을 때까지 계속 도는 상태로 남는다(A-B 구간반복 때 확인했던 '요소 교체로 리스너 소실'과 같은 함정을 다시 밟은 것). → 공용 바인더 `_bindVideoTitleMarquee(vid)`를 만들어 **양쪽에서 호출**한다. 이 함수는 `_bindVideoPlayerEvents`가 같은 요소에 최대 3번 불리므로(readyState 확인 · `loadedmetadata` · 3초 폴백) **요소당 1회만 걸리도록 `vid._titleMarqueeBound` 가드**를 둔다. 동시에 앞서 원본 리스너 3줄에 인라인으로 얹었던 코드를 **원상 복구**해 기존 동작 코드를 전혀 건드리지 않는 형태로 정리했다(대조 확인: 원본 `playing` 리스너 삭제 **0건**).
    - **워커 누수 방지**: 정지 시 `postMessage({stop:true})` + `terminate()` + 참조 `null`. 검증 — 워커 경로로 동작(폴백 미사용), 정지 후 `terminate: true`·참조 정리 확인, **Worker 생성이 막힌 환경에서는 setInterval 폴백이 동작하고 정지 시 타이머도 `null`**. 바인더 검증 — 같은 요소에 3번 재바인딩해도 리스너 **1개 유지**(play 1회에 흐르기 시작도 1회), 요소 교체 시 새 요소에 정상 바인딩되어 재생→정지가 `start`→`stop`으로 이어짐.
  - **적용 범위 확인(전 파일 종류)**: 미리보기 모달의 종류 분기는 `_showPreviewImpl` 안의 `switch(type)` 하나뿐이고(`image`/`video`/`audio`/`document`(pdf·txt·md·html)/`code`/`archive`), 제목 설정을 **분기보다 앞**에 뒀으므로 **여섯 종류 모두 자동 적용**된다. 압축파일 이미지 갤러리도 같은 함수 안이라 포함. `hwp`·`office`는 `rhwp_viewer`·OnlyOffice로 **별도 탭/페이지**를 열어 원래부터 파일명이 표시된다.
  - **추가로 막은 빈틈 — vault(암호화 폴더)**: `vaultPreviewFile()`은 복호화 진행 화면을 위해 `showPreview`를 거치지 않고 **직접 `modal-preview`를 여는 경로**가 있어 제목이 안 걸렸다. 그 지점(`#preview-title`을 세우는 줄) 옆에 `_setDocTitle('🔐 ' + item.name)`을 함께 넣었다. 복호화 후 `showPreview`로 넘어가는 경로에서는 그쪽이 다시 덮으므로 충돌하지 않는다. 점검 결과 — 모달로 파일을 여는 함수는 `_showPreviewImpl`·`vaultPreviewFile` 둘뿐이고 **양쪽 모두 제목 설정 O**, `hideModal`은 복원 O(`keepOpen` 제외).
  - **공유 페이지는 미변경**: `share.php`는 서버가 `<title>`에 파일/폴더명을 넣어 이미 정상이다(펜닐님이 "새 탭은 파일명이 나온다"고 한 그 경로).
  - **[후속] 갱신·복원이 빠진 경로 3곳 보완 (2026-08-17, 펜닐님 확인)**
    - **① 이미지·압축파일 갤러리에서 다음/이전으로 넘겨도 제목이 그대로**: 음악·동영상은 트랙 전환이 `showPreview`를 다시 타지만, 갤러리 이동은 `showGalleryImage()`만 거친다. 이 함수에 제목 갱신을 추가했다(이전/다음·vault 이미지·압축 내부 이미지가 모두 이 한 함수를 지난다).
    - **② 모달을 닫아도 파일명이 남음**: `closeModal()`은 `App.hideModal()`을 **거치지 않고 모달을 직접 숨긴다**. `hideModal`에만 복원을 넣어둬서 실제 닫기 경로에서는 걸리지 않았다. 모달을 감추는 지점 바로 뒤에 복원을 추가. **(자체 감사 후속)** 처음에는 모달 종류를 가리지 않고 복원해, 게시판을 보는 중에 설정·공유관리 같은 **다른 모달을 닫는 것만으로 게시판 제목이 사이트명으로 되돌아가는** 불일치가 있었다 → `hideModal`과 같은 기준(`currentModal.id === 'modal-preview'`)으로 좁혔다. `closeModal`은 이미 `currentModal.id`로 모달을 식별하고 있어(vault 잠금 해제 분기) 추가 조회가 필요 없었다. 검증: 설정·공유관리·vault·알림 모달을 닫아도 게시판/글 제목 유지, 미리보기 모달만 복원.
    - **③ 사이트 홈(로고) 클릭 시 복원 없음**: `goHome()` 맨 앞에 복원 추가.
    - **④ 게시판(2026-08-17 추가 제보)**: 게시판은 **모달이 아니라 인라인 화면**(`_showBoardInlineView`가 파일 목록·툴바를 숨기고 그 자리를 차지)이라 미리보기 제목 로직이 전혀 걸리지 않았다. 세 곳에 연결 — 목록은 `loadBoardPosts()`에서 **게시판 이름**, 글 상세는 `viewBoardPost()`에서 **글 제목**, 나갈 때는 `exitBoardView()`에서 복원. 목록으로 돌아가면 `loadBoardPosts`가 다시 게시판 이름으로 덮으므로 별도 처리가 필요 없다.
    - **경로 전수 점검** — 제목 설정 `_showPreviewImpl`·`vaultPreviewFile`·`showGalleryImage`·`loadBoardPosts`·`viewBoardPost` **5곳 모두 O**, 제목 복원 `closeModal`·`hideModal`·`goHome`·`exitBoardView` **4곳 모두 O**.

#### v5.8.3d (2026-08-05~08-15)

- **[수정] 남아 있던 `{count}` 노출 5곳 정리 + 전수 재검증 (2026-08-15, 펜닐님 확인 요청)**
  - **대상**: vault(암호화 폴더) 쪽 `vault_zip_complete`·`vault_migrate_confirm`·`vault_migrate_partial`·`vault_migrate_errors`·`vault_migrate_complete`. **ko 값에 자리표시자가 없어 한국어 화면에서는 정상**이었고 영어 UI에서만 `7{count} file(s) …`처럼 깨졌다. 앞서 보고만 하고 두었던 항목이다.
  - **수정**: 공유 쪽과 동일하게 `tf(key, vars, fallback)`로 바꾸고 숫자 접두를 제거했다. ko 값이 접미사형(`'개 파일 ZIP 다운로드 완료'`)이라 **언어 값도 완성문으로 함께 고쳐야** 했다 — ko 5개 값 수정, en은 비율 표기 1개(`vault_migrate_partial`)만 `{count}/{total}`로 맞췄다(기존에는 `3/10 개 마이그레이션 완료`처럼 비율 뒤에 "개"가 붙어 어색했다). 각 키가 코드에서 **정확히 한 곳에서만** 쓰이는 것을 먼저 확인해 다른 화면에 영향이 없음을 보장했다.
  - **검증**: 파일에서 `t`/`tf` 구현을 그대로 추출해 ko·en·폴백(키 없음) 세 경로로 실행 — ko `✅ 7개 파일 ZIP 다운로드 완료` / `3/10 마이그레이션 완료 (2개 실패)`, en `✅ 7 file(s) ZIP download complete` / `3/10 migrations complete (2 failed)`, 자리표시자 잔존 0.
  - **[후속] `escapeHtml` 강화의 부작용 3곳 수정 (2026-08-15 자체 감사)**: 따옴표까지 이스케이프하도록 바꾼 뒤 전 파일을 다시 훑다가, **`escapeHtml` 결과를 CSS 선택자에 쓰는 곳 3군데**를 발견했다(`.file-item[data-path="${escapeHtml(path)}"]`). DOM 속성값은 파싱 시 엔티티가 **디코드**되므로(`It&#39;s` → `It's`) HTML 이스케이프한 문자열로는 매칭되지 않는다. 실측 — 경로 `It's Fine.mp3`가 변경 전에는 우연히 일치했으나 변경 후 불일치(`A&B.mp4`는 **변경 전부터 이미** 불일치였다). → 다른 4곳이 이미 쓰던 `CSS.escape(원본경로)`로 통일해 **기존 `&` 버그까지 함께 해소**. 마크업 속성을 *쓰는* 3곳(`data-path="…"` 출력)은 `escapeHtml`이 맞으므로 그대로 뒀다. 확인: 선택자 문맥의 `escapeHtml` 사용 **0건**, `CSS.escape` 11곳.
  - **전수 재검증**: 자리표시자를 가진 언어키 96개를 기준으로 JS·PHP 전 파일을 훑어 ㉠치환 없이 `t()`/`__()`로만 쓰는 곳 **0건** ㉡`tf()`/`__f()` 105건 중 변수 누락 **0건**을 확인했다. ※ 이 과정에서 스캐너 오탐이 세 번 나왔다 — `tf(key, 기본값, 변수)` 인자 순서, JS ES6 축약 속성(`{total, count}`), PHP 중첩 대괄호(`$sigCheck['expected']`). 셋 다 파서를 고쳐 최종 0건을 확인했으므로, **"검사에서 0건"이 아니라 "검사기가 그 형태를 읽을 수 있는 상태에서 0건"**임을 남겨둔다.
  - **언어 키 무결성**: ko/en 모두 **삭제된 키 0개**, 값 변경은 위 5개(ko)·1개(en)뿐임을 사전/사후 대조로 확인.

- **[수정] 공유를 지워도(또는 만료돼도) 파일 목록의 🔗 배지가 안 사라짐 (2026-08-15, 펜닐님 제보)**
  - **증상**: 내 공유 링크 / 공유 관리에서 공유를 삭제하거나 만료로 없어졌는데, 로그인 상태로 있으면 파일에 붙은 공유 배지가 그대로 남고 새로고침해야 사라진다.
  - **원인 3가지 — 경로마다 처리가 달랐다**
    - ㉠ **선택삭제만 동기화 누락**: 목록의 단일 🗑️(`deleteShare`)는 `_reloadFilesKeepScroll()`을 부르는데, **체크박스 선택삭제(`_deleteSelectedShares`)에는 그 호출이 빠져 있었다**.
    - ㉡ **만료 정리 시점이 모달 열기**: 서버는 만료·다운로드 초과 공유를 `ShareManager::cleanupExpiredShares()`에서 실제로 **삭제**하는데, 이 함수는 `getShares()` 안에서만 호출된다. 즉 **'내 공유 링크' 모달을 여는 순간** 삭제가 일어나는데 파일 목록은 그대로라 배지가 남았다.
    - ㉢ **목록 API가 만료를 무시**: `files` 응답에서 공유 여부를 `is_active`만으로 판정해, **만료된 공유도 `shared=true`로 내려왔다**. 그래서 모달을 한 번도 안 열었다면 새로고침해도 유령 배지가 유지되는 구조였다(같은 파일의 `share_count`는 만료·한도를 제외하고 있어 상단 개수와 배지가 서로 어긋났다).
  - **수정**: ㉠ 선택삭제에 `_reloadFilesKeepScroll()` 추가(단일 삭제와 동작 일치) ㉡ `showSharesModal()` 끝에서 목록을 한 번 다시 읽어(스크롤 유지) 모달을 여는 순간 정리된 만료분을 배지에 반영 ㉢ `api.php`의 `files` 공유 판정에 **`share_count`와 동일한 조건**(만료 지남 / 다운로드 한도 소진 제외)을 추가.
  - **검증**: 새 판정 블록을 그대로 떼어 PHP로 실행 — 무제한 공유·만료 전 → 배지 표시, 만료됨·다운로드 한도 소진·비활성 → 배지 숨김. 삭제/갱신 경로 4개를 표로 재점검해 단일삭제·선택삭제·모달열기 모두 목록 재로드가 걸리고, 공유 모달 내 삭제는 기존대로 배지 DOM을 직접 제거함을 확인.
  - **부수 효과**: 상단 공유 개수(`share_count`)와 파일 배지의 판정 기준이 같아져 **둘이 어긋나던 문제도 함께 해소**된다.

- **[수정] 공유 관리 삭제 확인 문구에 `{count}`가 그대로 노출 (2026-08-15, 펜닐님 제보)**
  - **증상**: 내 공유 링크 / 공유 관리에서 여러 개 선택 후 삭제하면 `15{count}개 공유를 삭제하시겠습니까?`로 표시.
  - **원인**: 호출부가 `` `${ids.length}${t('confirm_delete_shares','개 공유를 …')}` ``처럼 **숫자를 앞에 붙이고 번역 문구를 그대로 이어붙였는데**, 실제 번역값은 ko `"{count}개 공유를 삭제하시겠습니까?"` · en `"Delete {count} share(s)?"`로 **자리표시자를 포함한 완성 문장**이었다. 폴백 문자열("개 공유를 …")만 보고 접미사형이라고 가정한 것이 어긋난 경우다.
  - **수정**: 자리표시자를 치환하는 기존 헬퍼 `tf(key, vars, default)`로 바꾸고 앞의 숫자 접두를 제거했다(어순이 다른 영어도 올바르게 나온다). 같은 화면의 삭제 완료 토스트(`shares_deleted`)도 동일 문제여서 함께 고쳤다.
  - **검증**: 파일에서 `t`/`tf` 구현을 그대로 추출해 실제 ko/en 사전으로 실행 — ko `15개 공유를 삭제하시겠습니까?`, en `Delete 15 share(s)?`, 완료 토스트 ko `15개 공유 삭제됨` / en `15 share(s) deleted`로 자리표시자 잔존 0. 사전에 키가 없는 폴백 경로도 정상 확인.
  - **미수정(보고)**: 같은 유형이 vault(암호화 폴더) 쪽에 5곳 더 있다 — `vault_zip_complete`(app.js 42814), `vault_migrate_confirm`(44716), `vault_migrate_partial`·`vault_migrate_errors`(44753), `vault_migrate_complete`(44755). **이들은 ko 번역값에 자리표시자가 없어 한국어 화면에서는 정상**이고 영어 UI에서만 `5{count} file(s) …`처럼 깨진다. 다른 기능 영역이라 지시 없이 손대지 않았다. (`remote_notice` 2곳은 `.replace('{type}', …)`로 직접 치환하고 있어 정상이다.)

- **[기능] 여러 항목 선택 시 공유 링크 일괄 생성 (2026-08-15, 펜닐님 요청)**
  - **증상**: 개별 항목의 공유(다운로드·스트리밍·파일드롭 설정 포함)는 정상인데, 체크박스로 여러 개를 고른 뒤 공유하면 동작하지 않았다.
  - **원인**: 컨텍스트 메뉴의 `share`가 `showShareModal(item)`으로 **첫 항목만** 넘기고, 모달이 `this.selectedItems = [item]`으로 선택을 덮어쓴 뒤 `createShare()`가 `items[0]`만 사용하고 있었다. 즉 여러 개를 골라도 첫 항목 하나만 공유됐다.
  - **수정**: 호출부를 `showShareModal(item, items)`로 바꿔 선택 목록 전체를 넘기고, 2개 이상이면 `_showShareModalBulk()` → `_createShareBulk()` 경로를 탄다. **1개일 때의 흐름·화면은 그대로**라 회귀가 없다(분기는 `_shareBulkItems`가 2개 이상일 때만).
  - **중복 공유 방지**: 서버 `ShareManager::createShare`는 중복 검사를 하지 않아 부를 때마다 새 토큰을 만든다. 단일 공유 화면은 `share_check`로 기존 링크가 있으면 그것을 보여주므로, 일괄도 **항목마다 `share_check`를 먼저 호출해 기존 링크는 재사용**(♻️ 표시)하고 없을 때만 생성한다.
  - **공유 유형**: 선택 항목들이 공통으로 지원하는 것만 노출한다 — 전부 폴더면 다운로드/스트리밍/파일드롭, 폴더·미디어만 섞였으면 다운로드/스트리밍, 그 외가 섞이면 다운로드만(선택지 숨김). 만료·비밀번호·최대 다운로드는 전 항목에 동일 적용.
  - **결과 화면**: 단일용 `#share-url`을 건드리지 않도록 `#share-bulk-result`를 새로 두고, 항목별 링크 + 개별 복사 버튼 + `전체 링크 복사`(줄바꿈 구분)를 보여준다. 진행 중에는 `n / N` 표시. 한 건이 실패해도 나머지는 계속 진행하고 끝에 `신규 n · 기존 m · 실패 k`로 요약한다.
  - **안전장치**: 한 번에 100건 상한(게시판 일괄삭제 500건과 같은 취지)으로 과도한 연속 요청을 막고, 상한 초과 시 **API를 한 번도 호출하지 않고** 중단한다. 서버 권한 검사는 기존 `share_create` 경로를 그대로 타므로(폴더 읽기 권한 + `can_share` + 경로 탐색 방지) 일괄이라고 우회되는 지점이 없다.
  - **언어키**: `items_count_unit`, `share_bulk_progress/new/exist/fail/done/copy_all`, `err_share_bulk_limit` ko·en 각 8개 신설.
  - **[후속] 자체 감사에서 잡은 회귀 1건 수정 (2026-08-15)**: `#btn-create-share`는 초기화 때 한 번만 바인딩되는 **같은 버튼을 단일/일괄이 공유**한다. 일괄 공유는 진행 중 `prop('disabled', true)`로 잠그고 끝나면 `hide()`만 하는데, 단일 공유 흐름은 `show()`만 하고 `disabled`를 풀지 않아 **일괄 → 단일 순서로 열면 버튼이 보여도 눌리지 않았다**(새로고침하면 풀림, 일괄→일괄은 `_showShareModalBulk`가 풀어줘서 무증상). → 모달을 여는 유일한 경로인 `showShareModal` **최상단**(일괄/단일 분기보다 앞)에서 `prop('disabled', false)`로 한 번 초기화한다. 어느 경로로 열어도 같은 시작 상태가 보장되고, 앞으로 추가되는 경로에도 자동 적용된다. 검증: 실제 `_createShareBulk`를 돌려 버튼이 `disabled=true, visible=false`가 된 뒤 초기화 줄을 태우면 `disabled=false, visible=true`로 복구됨을 확인.
  - **[후속] 앞서 미뤘던 2건 조치 (2026-08-15)** — 미룬 근거 자체를 다시 검증한 결과 하나는 판단이 과했다.
    - **① 속성 문맥 이스케이프 — `escapeHtml` 자체를 강화**: 앞서 "`escapeHtml`을 바꾸면 텍스트 문맥 37곳에서 `&quot;`가 화면에 보일 수 있다"고 판단해 미뤘는데, **틀린 걱정이었다**. 결과는 전부 `innerHTML`로 삽입되고 **HTML 엔티티는 텍스트 노드에서도 디코드**되므로 화면 표기는 달라지지 않는다(실측 확인: 원본 `a"b<c>d&e'f` → 현재·변경 모두 화면에는 동일하게 표시). 실제로 전수 조사하니 **호출 341곳 중 `innerHTML`을 안 거치는 곳은 단 1곳**(`opt.textContent = …`)뿐이었다. → `escapeHtml`에 `" → &quot;`, `' → &#39;`를 추가해 **속성 문맥 37곳+를 한 번에 안전**하게 만들고, 유일한 예외였던 그 한 곳은 `textContent`가 대입만으로 안전하므로 이스케이프 호출 자체를 제거했다(그대로 두면 거기서만 `&quot;`가 보인다). 검증 후 비-`innerHTML` 경로 **0건**. 실측 — 파일명 `따옴표" onmouseover="x.mp4`가 `title="따옴표&quot; onmouseover=&quot;x.mp4"`로 막힌다.
    - **② 혼합 스토리지 — 일괄 공유에서 원격 항목 사전 판별**: 컨텍스트 메뉴의 원격 판정은 '현재 스토리지' 기준 1회라 검색 결과처럼 스토리지가 섞이면 원격 항목이 낄 수 있다. 서버도 거르지만 사유가 "잘못된 경로입니다"로 엉뚱하게 나온다. → `_createShareBulk`에서 **컨텍스트 메뉴와 동일한 기준**(`storage_type`이 home·shared·local이 아님)으로 미리 걸러 불필요한 요청을 없애고 `외부 스토리지는 공유 링크를 만들 수 없습니다`로 정확히 표시한다. **컨텍스트 메뉴의 판정 로직과 단일 공유 동작은 건드리지 않았다**(그쪽을 바꾸면 단일 공유 활성 조건까지 달라짐). 언어키 `err_share_remote_unsupported` ko·en 신설. 실측 — 로컬 2건은 API 호출, 원격 1건은 **호출 없이** 실패 행으로 표시.
  - **검증**: 파일에서 `_createShareBulk`·`copyBulkShareUrls` 본문을 그대로 추출해 가짜 API로 실행 — 3항목(신규 1·기존 1·실패 1) 시나리오에서 기존 항목은 `share_create`를 **호출하지 않고**(중복 미생성) 링크를 재사용, 실패 항목은 나머지 진행을 막지 않으며, 배지 갱신 1회·목록 UI 갱신 2건·전체 복사 문자열·결과 행 수(링크 2·실패 1·♻️ 1)까지 확인. 101건 투입 시 API 호출 0건으로 차단됨도 확인. `php -l`·`node --check`·JSON 통과.

- **[수정] 모바일 개방 후 드러난 겹침·폭 문제 3건 + 공유 헤더 정리 (2026-08-15, 펜닐님 화면 확인)**
  - **① 모바일 오버레이 겹침(세로·가로 공통) — 직전 개방 작업의 부작용**: 모바일 CSS에는 *"목록 버튼 회피용 `right:80px` 룰도 모바일에선 무효화 (세로 배치되니 좌우 충돌 없음)"* 라는 규칙이 있었는데, 이 전제는 **모바일에서 목록 버튼이 `display:none`이던 시절**의 것이다. 버튼이 보이게 되자 화질/재생방식 셀렉트(`right:6px`)와 목록 버튼(`right:8px`)이 같은 줄에서 겹쳤다. 모바일 오버레이는 좌우로 나누지 않고 **세로로 쌓는 설계**(오디오 `top:4px` → 화질 `top:44px`)이므로 목록 버튼도 같은 사다리에 얹었다 — 오디오가 있으면 `top:82px`, 오디오가 없거나 비었으면(일반재생 `.native` 배지 포함) `top:44px`. **화질·오디오 셀렉트 위치는 건드리지 않았다**(펜닐님이 여러 번 미세조정한 값이라 재파손 위험이 큼). 조건 셀렉터는 화질의 `top:6px` 규칙과 똑같이 맞춰 두 요소가 항상 같이 움직인다.
  - **② 재생 중 목록을 열면 패널 폭이 320px로 줄던 문제**: PC용 규칙이 재생 중 패널을 `width:0`, hover 시 `width:320px`로 잡는데 모바일 규칙은 `max-height`만 되돌리고 **폭을 되돌리지 않아** 그대로 상속됐다. 터치가 `:hover`를 트리거하므로 재생 중 목록을 열면 320px 규칙이 걸려 좁아진 것(재생 전에는 `.playing`이 없어 정상 폭). 모바일 재생 중 패널 규칙 3개에 `width:100% !important; flex-basis:auto !important` 추가.
  - **③ 공유 트랙 개수를 제목 아래로**: 공유 헤더가 `🎬 동영상 폴더 · 3개 트랙`으로 한 줄이었는데, 일반은 제목 아래에 개수가 붙는 2줄 구조다. 공유에 `.pl-header-main` 래퍼를 넣어 일반의 `.fs-vp-header-main`과 동일하게 세로 배치(`flex:1`은 래퍼로 이동).
  - **④ 배지가 길 때 화질 셀렉트와 겹침(모바일 세로, 추가 제보)**: 배지(좌)와 화질 셀렉트(우, `max-width:55%`)가 row 0을 좌우로 나눠 쓰는데 **배지에만 폭 제한이 없어** 해상도·용량이 길게 붙으면 우측을 침범했다(예: `▶ 일반 재생 (H264 1920x1080 3.5 GB)`). 배지 몫을 `max-width:40%`로 묶고 넘치면 말줄임(`ellipsis`). 화질 셀렉트가 없거나 비어 우측을 안 쓰면 `calc(100% - 12px)`로 풀어준다. **줄을 더 늘리지 않고 폭을 제한한 이유**: 세로 모드는 영상 자체가 낮아(360px 폭 16:9면 약 202px) 오버레이를 3~4줄로 쌓으면 화면을 과하게 덮는다. 폭 합계 검산 — 320/360/390/414px 어디서도 `40% + 55% + 여백 12px`가 100%를 넘지 않는다(남는 간격 4~9px). 대신 배지 뒤쪽 상세(해상도·용량)는 좁은 화면에서 잘릴 수 있고, 앞부분의 `일반 재생`/`실시간 변환` 구분은 항상 남는다.
  - **⑤ 모바일 목록 검색창 깨짐(추가 제보)**: 아이콘이 글자와 겹치고 입력창이 오른쪽으로 넘쳤다. 이 입력은 미리보기 모달 안(`.modal-body` → `#preview-content` → 패널)에 있어서 **모바일 전역 규칙 `.modal-body input[type="text"]`(특이도 0,2,1)** 이 `.fs-vp-search`(0,1,0)를 덮어쓰고 있었다 — `padding:10px 12px`로 좌측 30px 자리가 12px가 되어 배경 아이콘(10px 위치)과 글자가 겹쳤고, `width:100% !important`가 `margin:8px 16px`와 겹쳐 폭이 32px 넘쳤다. `width`는 상대도 `!important`라 `!important`만으로는 못 이기므로 **특이도를 (0,3,1)로 올려**(`.fs-vp-panel input[type="text"].fs-vp-search`) 값을 되돌렸다. 전역 규칙이 모바일 미디어쿼리 안에만 있어 **PC는 원래 정상이었고 이 수정으로도 값이 그대로**다. 같은 모달의 음악 검색창(`.fap-playlist-search`)은 이미 `!important`로 같은 문제를 막아둔 상태여서 진단이 교차 확인됐다.
  - **⑥ 모바일 목록에서 현재 재생 트랙 위치가 안 맞음(추가 제보)**: `_fsVpScrollToCurrent()`가 `cur.offsetTop - body.clientHeight/2 + cur.clientHeight/2`로 계산했는데, **`offsetTop`은 스크롤 컨테이너가 아니라 '가장 가까운 위치 지정 조상' 기준**이다. `.fs-vp-body`·`.fs-vp-item` 모두 `static`이라 기준이 `.modal-body`(인라인 `position:relative`)가 되어, 목록 위에 있는 것들의 높이가 그대로 더해진 값이 나왔다. → `- body.offsetTop`을 빼서 목록 안 실제 위치로 보정(둘의 `offsetParent`가 같아 뺄셈으로 상쇄). **PC/모바일 차이의 이유**: PC는 패널이 영상 *옆*(가로 배치)이라 오차가 헤더+검색창 높이(약 100px)뿐이라 600px 목록에서는 티가 안 났고, 모바일은 영상 *아래*(세로 배치)라 영상 높이(300~500px)만큼 부풀어 현재 트랙이 화면 위로 밀려 안 보였다. **공유 페이지(`share.php` 4385)는 처음부터 `cur.offsetTop - body.offsetTop` 형태로 되어 있어 정상**이었고, 일반 쪽만 이 항이 빠져 있었다 — 식을 공유와 동일하게 맞췄다(룰4). 검증: 수정한 함수 본문을 파일에서 추출해 PC(600px)·모바일 세로(230px)·모바일 가로(150px) 레이아웃으로 실행 — 첫 트랙은 맨 위, 중간 트랙은 정확히 가운데, 마지막 트랙은 최대 스크롤로 9개 경우 모두 통과.
  - **⑦ 가로모드 폰에서 목록이 한 줄도 안 보임 → 가로 배치로 전환(추가 제보, 펜닐님 A안 선택)**: 가로모드는 높이가 약 390px뿐인데 모바일 세로 배치의 패널 상한이 `35vh`(=137px)라, 패널 고정 요소(헤더 69 + 검색창 46 + body 여백 12 = **127px**)만으로 다 차서 **목록이 0개** 보였다. 스크롤 위치를 아무리 맞춰도 보일 칸이 없는 상태. 반면 가로모드는 **폭이 844px로 넉넉**하므로, `@media (orientation: landscape) and (max-width:1024px)`에서 `flex-direction: column`을 되돌려 PC와 같이 목록을 영상 **옆**에 둔다. 패널 폭은 `clamp(220px, 34%, 300px)`로 기기 폭에 따라 조절. 재생 중 접힘은 세로모드처럼 높이를 접는 대신 **PC처럼 폭을 접고**, 표시 여부는 세로모드와 동일하게 `.show-controls`가 결정한다(`opacity` 규칙은 세로용을 그대로 재사용). **영상 손해가 거의 없다**: 가로모드 영상은 어차피 높이에 먼저 걸려 최대 587x330인데, 패널 287px를 떼도 549x309다. 기기별 예상 — iPhone 14 가로 목록 5개/영상 549x309, SE 가로 5개/432x243, 15 Pro Max 가로 6개/624x351, 태블릿 가로 16개/716x403. **블록 배치 주의**: 셀렉터가 세로 배치 전환 블록과 동일해 특이도가 같으므로 **반드시 그 블록보다 뒤**에 와야 이긴다 — 파일 맨 끝에 붙였다.
  - **⑧ 음악 플레이어 목록도 가로모드에서 안 보임 → 2열 배치(추가 제보)**: 동영상 목록과 같은 원인이다. `.fap`은 세로 flex 스택이고 고정 요소가 상단패딩 40 + 커버줄 74 + 비주얼라이저 48 + 시킹 34 + 컨트롤 58 + 볼륨 36 + 목록헤더 38 + 하단패딩 12 = **약 340px**인데, 가로모드 모달 본문은 약 330px뿐이라 **재생목록이 0곡**이었다(세로모드는 784px라 10곡 정상). → 가로모드에서 왼쪽 재생부 / 오른쪽 재생목록 2열로 전환. **기본 + 리스타일 스킨(soundcloud·cassette·terminal·pixel)** 은 색만 바꾸고 레이아웃은 `.fap`의 세로 flex를 그대로 쓰므로 `display:grid` 2열로 전환하고 자식 6개를 1열에, 재생목록을 2열 전체 높이(`grid-row: 1 / -1`)에 배치했다. 비주얼라이저는 VU 모드에서 `.fap-vu-canvas-wrap`으로 감싸이므로 두 선택자를 같은 행에 함께 지정. `.fap-lyrics-modal`(position:fixed)·`.fap-skin-selector`(absolute)는 grid 항목이 아니라 무영향. **ap-fixed 스킨**은 이미 grid라 열만 2열로 되돌리고, 왼쪽 열이 세로로 넘치지 않게 커버 행을 240px → 110px로 줄였다 — 행 트랙이 고정값이라 커버 요소 높이만 줄여서는 행이 안 줄어들므로 **변형 4가지(`.fap-ios` / `.fap-no-inline-lyrics` / 조합)를 모두 지정**했고, 곡 정보 오버레이(`.fap-now`)는 `.fap` 기준 absolute라 모바일의 `width:100%`를 그대로 두면 오른쪽 목록까지 덮으므로 `60%`로 맞췄다. 결과 — iPhone 14 가로 5곡(ap-fixed 6곡), SE 가로 5곡, 15 Pro Max 가로 6곡, 왼쪽 열 필요 높이 290px(ap-fixed 250px)로 본문 330px 안에 들어감. **공유 페이지는 의도적으로 미적용**: 공유 오디오는 `.player-wrap.audio-wrap`에 `min-height: 480px`(모바일 420px) 바닥값이 있어 `max-height: calc(100vh-200px)`보다 우선하므로 **가로모드에서 0곡이 되는 게 아니라 박스가 화면보다 커지고 페이지가 스크롤되는 다른 동작**이다(iPhone 14 가로 3곡, SE 가로 1곡). 증상이 달라 같은 처방이 맞는지 화면 확인이 필요하므로 손대지 않았다.
  - **⑨ 음악 가로모드 2열 1차 적용 후 화면 확인 → 근본 충돌 1건 발견·수정(펜닐님 이미지 제보)**: 제보된 네 가지(목록이 잘게 보이거나 스킨별로 안 나옴 / 플레이어 아래 정체불명 검정 영역 / 플레이어가 위로 쏠림 / 재생부가 너무 넓음)는 대부분 **한 가지 원인**이었다 — 가로모드에는 원래부터 오디오 전용 블록이 있어 `.modal-body` → `#preview-content` → `.preview-audio-wrap` → `#fs-audio-player` → `.fap`까지 **전부 `height:auto !important; min-height:100%` + `align-items:flex-start`** 로 풀어두고 있었다(세로 스택 시절 "플레이어가 길어지면 페이지가 늘어나며 스크롤" 의도). 부모 체인이 전부 auto라 백분율 높이가 해소되지 않아 2열 grid에서 ㉠목록 행(`1fr`)이 내용 높이로 접히고 ㉡`.fap`이 컨테이너보다 짧아져 그 아래로 컨테이너 배경(검정)이 드러나고 ㉢`flex-start`로 카드가 맨 위에 붙는 증상이 한꺼번에 났다. → 가로모드 2열에서는 그 체인을 `height:100% !important; min-height:0`, `align-items:stretch`로 되돌리고, 세로 스택 시절의 `.fap-playlist-list { max-height:60vh }` 상한도 해제(확정 높이 안에서 내부 스크롤). **폭 조정**: 재생부가 넓다는 지적에 따라 60/40 → **55/45**(재생부 축소, 목록 확대). **세로 위치**: 기본·리스타일 스킨은 행 템플릿을 `1fr auto×5 1fr`로 바꿔 **위아래 스페이서 사이에 재생부를 끼워 가운데로 내렸고**, 재생목록은 `1 / -1`로 스페이서까지 포함해 전체 높이를 쓴다. **ap-fixed만 스페이서를 쓰지 않았다** — 곡 정보(`.fap-now`)가 `.fap` 기준 `position:absolute; top:0`으로 커버 위에 겹쳐 있어 커버를 내리면 글자만 위에 남아 어긋나기 때문이다. 대신 커버를 240px → **150px**로만 줄여 왼쪽 열(150+50+50+40=290)이 본문 330px을 거의 채우게 해 빈 공간을 줄였다. 결과 — iPhone 14 가로 5곡(ap-fixed 6곡)·재생부 464px, SE 가로 5곡, 15 Pro Max 가로 6곡.
  - **⑩ 가로모드 재생부를 재생목록과 같은 줄에서 시작하도록(펜닐님 요청)**: ⑨에서 기본·리스타일 스킨에 넣었던 **위쪽 `1fr` 스페이서를 제거**하고 행 템플릿을 `auto×6 1fr`로, 자식 행을 1~6으로 되돌렸다. 마지막 `1fr`은 남는 공간을 아래로 몰아주는 채움 행이고 재생목록은 `1 / -1`로 그 행까지 포함하므로, **두 열이 같은 줄에서 시작해 같은 높이로 끝난다**. ap-fixed는 커버가 이미 row 1이라 변경 없음. 앞서 "플레이어가 위로 쏠려 보인다"던 것은 스페이서 문제가 아니라 ⑨의 `height:auto` + `align-items:flex-start`가 원인이었고 그건 이미 해결됐으므로, 세로 가운데 정렬은 되돌려도 재발하지 않는다. 시작 y 좌표 — 기본·리스타일은 `.fap` padding-top 40px 안에서 좌열 `.fap-now`와 우열 목록이 모두 y=40, ap-fixed는 padding 0이라 둘 다 y=0.
  - **⑪ ap-fixed 커버 250px 조정 + terminal·cassette 장식 자리 확보(펜닐님 devtools 확인값·제보)**: ㉠ap-fixed 커버를 150px → **250px**로(펜닐님이 실기기 devtools에서 직접 맞춘 값 — 150px은 너무 작았다), 행 트랙이 고정값이라 변형 4가지 모두 반영. ㉡**terminal 스킨이 위로 붙어 보이던 원인**은 별개였다 — terminal(타이틀바 `prepend` + 프롬프트 `append`)과 cassette(카세트창)는 장식을 **`.fap`의 직계 자식**으로 주입하므로 grid에서 **자동배치 대상**이 되어 엉뚱한 칸에 떨어지고, 그 결과 재생부가 타이틀바 자리까지 올라와 붙었다. → 행 템플릿을 `auto×7 1fr auto`(9행)로 늘려 **1행=상단 장식 / 2~7행=재생부 / 8행=채움 / 9행=하단 장식**으로 나누고, 장식은 `grid-column: 1 / -1`로 좌우 열을 가로지르게 했다(스킨 창 테두리 성격이라 목록 위까지 덮는 게 맞다). 재생목록은 `grid-row: 2 / 9`로 장식 사이를 채운다. **장식이 없는 스킨(기본·soundcloud·pixel)은 1·9행이 0 높이로 접혀 영향이 없고**, 재생부는 여전히 목록과 같은 줄에서 시작한다. 이후 장식이 추가돼도 오른쪽 목록 칸으로 새지 않도록 `> .fap-skin-decoration { grid-column: 1 / -1 }` 안전망도 넣었다. ㉢**커버 높이를 고정 대신 반응형으로**(펜닐님 지적 — "해상도가 작은 것에서는 잘린다는 거지?"): 맞다. 고정 250px이면 세로가 낮은 기기에서 왼쪽 열이 본문을 넘어 `overflow:hidden`에 잘린다. → CSS 변수 `--fap-ap-cover-h: min(250px, calc(100vh - 차감))`으로 **상한만 250px**로 두고 모자라면 자동으로 줄인다. 행 템플릿 4종과 커버 요소가 모두 같은 변수를 참조하므로 한 곳만 고치면 된다. **차감값이 두 가지인 이유**: 볼륨 영역은 화면 크기가 아니라 **iOS 판별(`_isIOS` → `.fap-ios`)** 로 숨긴다(iOS는 정책상 `audio.volume` 변경 불가). 즉 iOS 기기는 볼륨 높이만큼 커버에 더 쓸 수 있어 `100vh - 175px`, 안드로이드·PC는 볼륨이 보이므로 `100vh - 215px`. 결과 — SE 가로 커버 200px(iOS)/160px, 13·14 가로 215/175, 15 Pro Max 가로 250/215, 태블릿 250/250으로 **모든 기기에서 좌열이 본문 안에 들어간다**. ※ 판별이 화면 크기가 아니라 플랫폼이므로 **아이패드도 볼륨이 숨겨지고 안드로이드 폰은 볼륨이 보인다** — "모바일엔 볼륨이 없다"가 아니라 "iOS엔 없다"가 정확하다.
  - **⑫ 나머지 스킨 세로 넘침 점검·수정(펜닐님 질문 "나머지 스킨들도 문제없이 반응형인가?")**: 점검해보니 **아니었다**. ap-fixed와 달리 나머지는 행이 전부 `auto`라 잘리는 대신 밀려나지만, 조상 `.preview-audio-wrap`이 `overflow:hidden`이라 **결과적으로 아래가 안 보인다**. 실측 재계산(왼쪽 열 필요 높이, 볼륨 포함) — 기본 302 · soundcloud 294 · terminal 315 · pixel 326 · **cassette 422**, 본문은 375높이 기기에서 315px. ※ **앞선 계산의 오류를 정정**: 카세트창을 74px로 추정했으나 실측은 릴 60 + 패딩 30 + 테두리 6 + 마진 16 = **112px**이었다. 조치 — ㉠**pixel**: 커버가 다른 스킨의 56px 대신 100px 정사각형이라, ap-fixed와 같은 방식으로 `min(100px, calc(100vh - 300px))`(width·height 동시)로 상한만 두고 낮은 화면에서 줄인다. ㉡**cassette**: 카세트창 112px이 가장 무겁고 줄이는 것만으로는 필요한 107px을 못 만들어(릴을 20px대로 뭉개야 함) **가로모드에서만 장식을 숨기고** 스킨 자체 패딩 30/25 → 16px로 줄였다. 색·글꼴 등 나머지 정체성은 유지되고 세로모드·PC는 무영향이며, 되살리려면 그 규칙 한 줄만 지우면 된다. ㉢**terminal**: 315px로 여유가 0이라 장식 패딩 8 → 5px만 줄여 12px 여유를 만들었다. 결과 — **다섯 스킨 모두 375·390·430 높이에서 볼륨이 보이는 최악 조건까지 통과**(기본 302 · soundcloud 294 · cassette 282 · terminal 303 · pixel 301~326). 캐스케이드 확인 결과 세로모드·PC에서는 다섯 규칙 모두 원래 값 그대로다(pixel 100px, cassette 30px 25px, 카세트창 표시, terminal 8px).
  - **⑬ cassette 가로모드에서 카세트창 되살림(펜닐님 세로/가로 비교 이미지)**: ⑫에서 세로 넘침을 막으려 가로모드에서 카세트창을 통째로 숨겼는데, 그러면 **스킨의 상징이 사라지고 왼쪽 열에 빈 공간만 남는다**. 창 제외 왼쪽 열이 282px이므로 남는 공간은 본문(100vh-60) - 282 — 375높이에서 33px, 390에서 48px, 430에서 88px, 태블릿에서 426px이다. → **화면 높이로 나눴다**: `min-height: 400px` 미만이면 자리가 없어 계속 숨기고, 400px 이상이면 남는 공간에 맞춰 릴 지름을 `min(60px, calc(100vh - 378px))`로 줄여 되살린다(400→22px · 430→52px · 768→60px 상한). 창 패딩 15→8px, 마진 16→6px, 가운데 글자 10→9px로 함께 줄여 창이 글자 높이에 끌려 커지지 않게 했다. 결과 좌열 — 375·390 282px(숨김) · 400 334 · 430 362 · 768 370으로 **모든 구간에서 본문 안에 8px 안팎 여유를 두고 들어간다**. 되살리는 블록은 숨기는 블록보다 **뒤**에 둬야 이긴다(같은 셀렉터·같은 `!important` → 순서). 캐스케이드 확인 — 세로모드·PC는 릴 60px·패딩 30/25·글자 10px로 **원래 값 그대로**이고, 창 표시 여부 규칙 자체가 가로모드에서만 존재한다.
  - **⑭ cassette 창을 재생부 열에만 배치(펜닐님 요청)**: ⑬에서 되살린 카세트창이 `grid-column: 1 / -1`로 **두 열을 가로질러** 재생목록 위까지 덮고 있었다. 카세트창은 창 테두리가 아니라 **재생 장치 자체를 표현하는 요소**이므로 `grid-column: 1`(재생부 열)로 바꾸고, 그러면 오른쪽 목록 위에 빈 칸이 생기므로 카세트 한정으로 목록을 `grid-row: 1 / 9`(맨 윗행부터)로 올렸다. terminal 타이틀바·프롬프트는 macOS 창 테두리 성격이라 `1 / -1` 유지. **여기서 실제로 안 먹을 뻔한 함정을 잡았다**: 장식 안전망 `> .fap-skin-decoration { grid-column: 1 / -1 }`이 카세트창 규칙과 **특이도가 (0,3)로 동일**한데 소스상 **뒤**에 있어, 카세트창의 `grid-column: 1`을 그대로 덮어쓰고 있었다(카세트창 요소는 두 클래스를 모두 가진다). → 안전망을 '기본값'답게 개별 규칙보다 **앞**으로 옮겼다. 검증도 셀렉터 단위가 아니라 **요소 단위**(클래스 집합으로 매칭해 후보 규칙을 모두 모아 정렬)로 바꿔 최종 적용값이 `1`임을 확인했다 — 셀렉터별로만 보면 둘 다 "이긴다"고 나와 놓치는 종류의 문제다.
  - **검증**: 특이도 계산으로 실제 적용 여부까지 확인 — 목록 버튼 `top`은 기본 `.fs-vp-toggle{top:8px}`(0,1,비!important)보다 모바일 규칙(0,2 !important)이 이기고, 조건부 `top:44px`(0,3~0,4 !important)가 그보다 이긴다. 패널 `width:100% !important`는 PC의 `width:320px`(비!important)를 이긴다. `php -l`·`node --check`·JSON·CSS 균형 통과. 사라진 선언은 의도한 교체분뿐. ⑦은 **미디어쿼리 조건까지 반영한 캐스케이드 해석기**로 세로모드·가로모드·PC 세 경우를 각각 판정해 확인했다 — 세로모드는 `column / width:100% / max-height:35vh`로 **변화 없음**, 가로모드만 `row / clamp(220px,34%,300px) / max-height:100%`, PC는 `320px / 재생 중 width:0`으로 **변화 없음**.

- **[기능] 모바일 일반 동영상에 재생목록·자동 다음 재생 개방 + 공유 토글 표기 통일 (2026-08-15, 펜닐님 요청)**
  - **증상**: 모바일에서 **일반(미리보기 모달)** 동영상은 폴더 안 영상을 열어도 목록이 없고 자동 다음 재생도 안 됐다. 같은 상황에서 **공유** 페이지는 목록과 자동 다음 재생이 정상 동작해 두 경로의 동작이 어긋나 있었다.
  - **원인 2개**: ① `style.css`의 `@media (max-width: 1024px)` 안에서 `.fs-vp-toggle`과 `.fs-vp-panel`을 `display: none !important`로 **완전히 숨기고** 있었다(v5.8.1g에서 "모바일 UX 단순화"로 추가). ② `_fsVpBindAutoNext`의 `ended` 핸들러 맨 앞에 `if (window.innerWidth <= 1024) return;`이 있었다 — 그 근거가 "모바일은 패널이 숨겨져 토글 버튼이 안 보이니 사용자가 끌 방법이 없다"였으므로, ①이 풀리면 성립하지 않는 가드다.
  - **수정**: 위 두 가지를 제거. 모바일 레이아웃 자체는 이미 갖춰져 있었다 — 같은 미디어쿼리 위쪽에 세로 배치(`flex-direction: column`), 패널 `width:100% / max-height:35vh`, 그리고 재생 중에는 패널이 접히고 컨트롤을 부르면 다시 펼쳐지는 규칙(`.playing` / `.show-controls`)이 그대로 있어서, **숨김 규칙만 걷어내면 설계대로 동작**한다. 새로 만든 레이아웃은 없다.
  - **PC 영향 없음**: 제거한 CSS는 1024px 이하 미디어쿼리 안에만 있었고, 제거한 JS 가드도 1024px 이하에서만 동작하던 것이다.
  - **공유 토글 표기 통일**: 공유의 자동 다음 재생 토글이 `ON/OFF`였는데 일반은 `수동/자동`이라 같은 기능이 다르게 보였다. 공유를 일반(`.fs-vp-autonext`)과 같은 규격으로 맞춤 — 표기 `OFF/ON → 수동/자동`, 폭 `44→52px`, 글자 `8→9px`, 노브 이동 `28→36px`, 그리고 일반처럼 토글 왼쪽에 **"다음재생" 라벨**을 붙였다(`.pl-autonext-wrap` / `.pl-toggle-label` 신설). 저장 키·기본값(ON)·동작은 그대로다.
  - **언어키**: 일반이 폴백 문자열로만 쓰던 `autonext_label`을 ko/en에 정식 추가(ko `다음재생`, en `Next`). 같은 키를 공유도 쓰므로 두 화면 표기가 한 곳에서 관리된다. 다만 토글 안의 `수동/자동` 글자는 일반과 동일하게 **CSS `content:`에 한국어로 박혀 있어** 영어 환경에서도 한글로 나온다(기존 일반의 방식을 그대로 따른 것 — 바꾸려면 양쪽 동시 작업 필요).
  - **의도적 미변경**: 패널 최초 열림 기본값이 일반은 닫힘(`fs_vp_panel_open === '1'`), 공유는 열림(`!== '0'`)으로 서로 다르다. 일반의 기본값을 바꾸면 PC 동작까지 함께 바뀌므로 지시 없이 손대지 않았다 — 모바일에서도 `📋 목록` 버튼이 이제 보이므로 열 수 있다.
  - **검증**: `php -l` 7파일·`node --check`·JSON·CSS 균형 통과. 삭제된 것은 의도한 4가지뿐 — JS 가드 1줄, CSS `display:none !important` 2건, 공유 토글의 교체된 값 7개.

- **[기능] 동영상 플레이어 반복 재생 + 구간 반복(A-B) 추가 (2026-08-15, 펜닐님 요청)**
  - **적용 범위**: 일반(미리보기 모달 `.vec-container`) + 공유(`share.php`의 `.player-controls`) **양쪽**(룰4).
  - **음악과 다른 점(구조적 제약)**: 동영상은 진행바가 **브라우저 기본 컨트롤**(`<video controls>`)이라 음악처럼 시크바에 A/B 눈금을 그릴 수 없다. 대신 컨트롤 줄에 `0:30 ~ 1:30` 형태의 시간 텍스트로 구간을 표시한다(1시간 넘는 영상은 `1:01:01` 형식).
  - **반복 재생**: `video.loop`를 쓴다. 명세상 `loop=true`면 `ended` 이벤트가 발생하지 않으므로 기존 '자동 다음 재생'이 자연히 멈춘다 — **반복 ON이면 다음 트랙으로 안 넘어가고, OFF면 기존대로 넘어간다**(펜닐님 결정). 자동 다음 재생 토글이 이미 OFF면 원래대로 아무 영향 없음. 브라우저 편차 대비로 양쪽 `ended` 핸들러(`_fsVpBindAutoNext`, 공유 폴더 다음 페이지 이동)에도 `if (loop) return` 명시 가드를 뒀다. 저장은 일반 `localStorage 'fs_vp_loop'`, 공유 `sessionStorage 'share_video_loop'`(각각 자동 다음 재생 토글과 같은 저장 범위), **기본 OFF**로 기존 "반복 재생 X" 동작을 유지.
  - **구간 반복(A-B)**: 음악과 동일한 버튼 1개 3단계 순환(A 지정 → B 지정 → 해제)이고, A만 찍힌 상태에서 A보다 앞이나 0.3초 이내를 누르면 A를 그 자리로 재지정하는 규칙도 같다. 되돌림은 기존 `timeupdate`에 얹어 새 타이머가 없다. 구간이 영상 끝에 붙은 경우 대비로 `ended`에서 A로 되감기.
  - **트랜스코딩·HLS에서는 비활성**(펜닐님 결정): 실시간 변환은 MSE라 `duration`이 Infinity/NaN이고 되감기가 불안정하다. 일반은 `dataset.transcodeBase`·`_hlsInstance`·유한 `duration`을 함께 보고 판정해 **네이티브 재생 중 변환으로 넘어가도 자동으로 버튼이 사라지고 구간이 해제**된다(`durationchange`/`loadedmetadata`에서 재평가). 공유는 `$needsTranscode`면 PHP가 아예 렌더하지 않고, 재생 중 `startTranscode()`로 넘어가면 `window._shareAbHide()`로 숨긴다.
  - **상태 보관**: 일반은 A/B를 video 요소에 두어(`video._abA/_abB`) 영상이 바뀌면(요소 교체) 자동 초기화된다. 반복 설정은 저장값이라 트랜스코딩 요소 교체 후에도 `_bindVideoExtraControls` 재호출 시 복원된다.
  - **언어키 신설**: `video_loop`, `video_loop_on`, `video_loop_off`, `video_ab`, `video_ab_set_a`, `video_ab_set_b`, `video_ab_clear` (ko/en 각 7개).
  - **[후속 개선] 자체 감사에서 잡은 결함 3건 조치 (2026-08-15) — 모두 이번에 추가한 코드의 결함**
    - **① 공유: 변환·HLS 전환 후에도 A-B가 살아남는 경로 2개**. ㉠ `startTranscode()`만 `_shareAbHide()`를 부르고 있었는데, **네이티브 재생 실패 → 변환 자동 전환**(`_triggerTranscodeFallback`)은 그 함수를 타지 않는다. 이 경로는 `$needsTranscode`가 false여서 A-B 버튼이 정상 렌더된 상태이므로 그대로 두면 HLS에서 구간 반복이 동작한다 → 이 경로에도 `_shareAbHide()` 호출 추가. ㉡ 더 근본적으로, 숨기기만 해서는 **이후 HLS가 유한한 `duration`을 보고하는 순간 `durationchange` → `_abUpdate`에서 버튼이 다시 나타난다**. `_abDisabled` 플래그를 둬 변환·HLS 진입은 되돌릴 수 없는 비활성으로 고정.
    - **② 리스너 중복 등록(누수)**: `timeupdate`/`ended`는 `_abBound`로 막았으나 `loadedmetadata`/`durationchange`는 가드가 없었다. `_bindVideoPlayerEvents()`가 `loadedmetadata`(once)와 **3초 fallback `setTimeout`** 양쪽에서 불려 같은 요소에 두 번 호출되므로 실제로 2개씩 쌓였다(실측 확인). 네 리스너 모두 요소당 1회 등록으로 바꾸고, 컨트롤 줄은 재바인딩마다 새로 만들어지므로 최신 갱신 함수를 `video._abUpdate`에 걸어 리스너가 **옛 버튼을 붙잡지 않게** 했다.
    - **③ B가 영상 끝에 붙으면 되감기와 자동 다음 이동이 동시 실행**: `ended` 핸들러가 두 개인데 자동 다음 쪽이 `loop`만 보고 A-B 상태를 몰랐다. B가 끝에서 약 250ms(=`timeupdate` 주기) 안쪽일 때 도달한다. 양쪽 자동 다음 핸들러에 A-B 가드 추가(공유는 `window._shareAbActive()` 경유). 결과 진리표: `loop`나 A-B 중 **하나라도 켜져 있으면 다음 트랙으로 이동하지 않고**, 둘 다 꺼져 있을 때만 기존대로 이동한다.
    - **①을 고치다 만든 회귀를 테스트가 잡아 즉시 수정**: `_abDisabled`를 `_shareAbHide()`에 넣었는데 `_abUpdate`가 "아직 길이 미확정"일 때도 그 함수를 부르고 있어, **`preload="none"`인 단일 영상 공유는 로드 시 `duration`이 NaN이라 영구 비활성이 되어 버튼이 영영 안 나오는** 상태가 됐다. "아직 준비 안 됨(일시적 숨김, 나중에 복귀)"과 "변환·HLS라 영구 비활성"을 `_abHideOnly()` / `_shareAbHide()`로 분리해 해결. 일반쪽도 같은 계열 확인 — NaN → 메타 도착 후 정상 표시, 변환 전환 후에는 HLS가 유한 길이를 보고해도 계속 숨김.
  - **검증**: `php -l` 7파일 통과, `node --check` 통과, JSON 유효, CSS 중괄호 균형. 삭제된 줄 **0**(share.php의 3줄은 캐시버스팅 교체분). 검증은 **파일에서 A-B 블록과 반복 블록을 그대로 추출해 가짜 video 요소에 얹어 실행**하는 방식으로 했다 — 양쪽 모두 A/B 지정·되돌림·구간 중 ended·해제·역순 클릭·1시간+ 표시·Infinity/NaN 숨김·재생 중 변환 전환 시 자동 해제, 반복 토글 저장/복원까지 통과. **실브라우저·실기기 미테스트**.

- **[기능] 음악 플레이어 구간 반복(A-B) 추가 (2026-08-15, 펜닐님 요청)**
  - **방식**: 웹에서 널리 쓰이는 **버튼 한 개 3단계 순환** — 누르면 현재 위치를 **A**로 지정(라벨 `A`), 다시 누르면 **B**로 지정하고 반복 시작(라벨 `A-B`), 한 번 더 누르면 해제. 별도 A/B 버튼이나 롱프레스 없음.
  - **잘못된 구간 방지**: A만 찍힌 상태에서 **A보다 앞(또는 0.3초 이내)** 위치를 누르면 B로 잡지 않고 **A를 그 자리로 다시 찍는다**. 따라서 B ≤ A인 구간이 만들어질 수 없다.
  - **반복 처리**: 기존 `timeupdate` 리스너 안에서 `currentTime >= B`이면 A로 되돌린다(새 타이머·새 리스너 없음). `timeupdate` 주기가 약 250ms이므로 B를 넘는 오차가 그만큼 생긴다 — 음악 구간반복 용도에는 충분하며, 더 정밀하게 하려면 `requestAnimationFrame` 루프가 하나 더 돌아야 해서 배터리 손해가 크다고 판단해 채택하지 않았다.
  - **곡 끝 처리**: B가 곡 끝에 붙어 있으면 되돌림보다 `ended`가 먼저 올 수 있어, `_onEnded()` **맨 앞**에서 구간 반복이 켜져 있으면 A로 되감아 재생하고 반환한다(한 곡 반복·다음 곡보다 우선).
  - **곡 단위 임시 상태**: `_loadTrack`에서 해제한다. 곡이 바뀌면 이전 곡의 A/B가 남지 않으며, localStorage에 저장하지 않는다(셔플·반복·볼륨과 다른 점).
  - **시크바 표시**: A/B 지점에 세로 눈금, 그 사이에 구간 밴드를 그린다. 스킨 강조색(보라·주황·금색·초록·하늘색)과 겹치지 않게 **앰버(#ffb300) 고정**이며, 재생바(z-index 2) 위에 얹어 진행 중에도 구간이 보인다. `duration`이 없거나 무한대(스트리밍)면 표시하지 않는다.
  - **스킨 대응**: 새 `.fap-btn-ab`를 기본 + 5개 스킨(soundcloud·cassette·terminal·pixel·ap-fixed)의 셔플/반복 버튼 셀렉터 그룹에 함께 넣어 비활성 투명도·활성 강조색·hover 규칙을 동일하게 따르도록 했다.
  - **적용 범위**: app.js(본체) + fs-audio-player.js(공유) **양쪽 동일**(룰4). CSS도 `style.css`·`fs-audio-player.css` 양쪽에 동일 규칙 추가. `share.php`·`index.php`는 자체 플레이어 클래스를 갖지 않으므로 미수정.
  - **[후속 개선] 자체 감사에서 발견한 4건 조치 (2026-08-15)**
    - **① 공유 페이지 캐시버스팅 불일치(실제 문제)**: `index.php`는 자산을 `md5_file()` 해시로 캐시버스팅하는데 `share.php`만 `?v=APP_VERSION`이었다. 버전을 올리지 않는 이번 같은 릴리즈에서는 **쿼리가 그대로여서 브라우저 캐시의 옛 `fs-audio-player.js/css`가 계속 쓰인다** — 공유 페이지에서만 새 기능이 안 보이는 원인이 된다. `share.php` 3곳(CSS 2·JS 1)을 `index.php`와 동일한 `substr(md5_file(__DIR__ . '...'), 0, 10)` 패턴으로 통일. 대상은 이번에 수정한 두 파일뿐이며 다른 자산 참조는 건드리지 않았다.
    - **② duration 확정 전 A 지정 시 눈금 미표시**: `_updateAbUI()`가 `_cycleAb`/`_clearAb`에서만 호출돼, 메타데이터 로드 전에 A를 찍으면 그 눈금이 B를 찍을 때까지 안 보였다(상태값은 정상, 표시만 지연). `loadedmetadata` 핸들러에 `_updateAbUI()` 호출 1줄 추가.
    - **③ `ab-set` 클래스가 CSS 미사용**: JS는 붙이는데 스타일 규칙이 없어 'A만 지정(대기)'과 'A-B 반복 중'이 작은 라벨 글자로만 구분됐다. `.fap-btn-ab.ab-set::after`로 앰버 밑줄을 추가해 시크바 눈금과 같은 색으로 연결지었다(`.fap-btn-ab { position: relative; }` 동반).
    - **④ 눈금 대비**: 앰버(#ffb300)가 cassette 금색(#ffd700)·soundcloud 주황(#f50)과 가깝고 ap-fixed는 배경이 흰색이라 묻힐 수 있어, 눈금에 `box-shadow: 0 0 0 1px rgba(0,0,0,0.45)` 외곽선을 둘러 형태로도 구분되게 했다.
    - **의도적으로 손대지 않은 것**: ㉠ ap-fixed 스킨의 모바일 컨트롤 간격(`gap:16px !important`) — 버튼이 하나 늘어 빡빡해지지만 `flex-shrink` 기본값 1이라 넘치지 않고 눌리기만 하며, 이 값은 v5.8.1c에서 의도적으로 조정된 것이라 임의 변경하지 않음. ㉡ 버튼 라벨 SVG 글자 크기(환산 약 9.7px) — 실제 렌더를 못 본 상태에서 조정하면 넘침 위험이 있어 유지.
  - **검증**: JS 양쪽 **삭제된 실행문 0줄**(각 순수 추가 84줄), A-B 관련 코드 42줄이 두 파일에서 **완전 동일**함을 대조로 확인. CSS는 셀렉터 줄에 콤마만 붙은 재작성이며 **사라진 선언 0건**(각 신규 19건), 중괄호 균형 통과. `node --check` 양쪽 통과. `share.php`는 PHP 태그 균형(`<?php`/`<?=` 284, `?>` 286) 원본과 동일, 변경은 해당 3줄뿐. 검증 방식은 **파일에서 `_cycleAb`/`_clearAb`/`_updateAbUI` 본문을 그대로 추출해 가짜 DOM에 얹고 실행**하는 방식으로 했다(직접 다시 쓴 코드로 검증하면 실제 파일 내용을 보증하지 못하므로). 상태 전이·되돌림·곡 끝·곡 전환·마커 계산·`A=0`(맨 앞) falsy 함정·duration 미확정/무한·역순 클릭·destroy 후 호출까지 통과하며 양쪽 파일 결과 동일. **실브라우저·실기기 테스트는 미실시** — SVG 라벨 실제 가독성, 스킨별 색 충돌, iOS 터치, 실제 `timeupdate` 주기는 육안 확인 필요. 버전은 명시 지시가 없어 **5.8.3d 유지**.

- **[결론] 모바일 크롬 상단 밀림 — 브라우저 이슈로 확정, 관련 시도 전량 원복 (2026-08-13)**
  - **증상**: 모바일에서 브라우저를 종료했다 다시 들어오면 상단 헤더·스토리지바·툴바 첫 줄이 화면 위로 밀려 보이고, 스크롤하거나 새로고침하면 정상으로 돌아옴.
  - **확정 근거 3가지**: ①**같은 아이폰에서 사파리·브레이브는 정상, 크롬에서만 발생**(펜닐님 교차 확인) — iOS는 셋 다 같은 WebKit 엔진이므로 엔진 공통 문제가 아니다. ②**밀려 보이는 바로 그 순간의 측정값이 전부 정상**(진단 로그: `header rect.top=0`/`h=48`, 경로바·툴바의 지정 top과 실제 위치 일치, 문서·컨테이너 스크롤 0, 문서 높이=창 높이, `position: sticky` 정상). ③**다시 그리기(49회)와 강제 재배치(reflow)를 실행했는데도 화면이 그대로**(`lay_hardrelayout`이 `pageshow` 시점에 정상 실행된 것까지 로그로 확인). → 크롬 iOS가 주소창을 접을 때 **웹뷰 자체를 옮기는 독자 처리**에서 생기는 어긋남이며, JS·CSS가 닿을 수 없는 층이다. 같은 시기 다른 웹사이트에서도 동일 증상이 관찰된 것과도 일치한다(모바일 Chrome 152.0.7977.40).
  - **원복 내역**: 이 증상을 쫓으며 넣었던 것을 **전부 제거**했다 — `.file-list` 최소 높이 `svh`, `#main-screen` `max-height:100svh`(2곳), `history.scrollRestoration='manual'`, `resetDocScroll`, `forceRepaint`(opacity), `hardRelayout`(크롬 전용 reflow), 레이아웃 진단 로그 일체(`_layoutLog`·연속 촬영·워치독·localStorage 버퍼/복구 전송). 모두 원인이 아니었고 효과도 없었으므로 남길 이유가 없다. 되돌리기는 추적 시작 직전 백업에서 `app.js`·`style.css`를 복원하는 방식으로 처리해 부분 삭제로 인한 누락 위험을 없앴다.
  - **유지한 것**: 펜닐님이 요청하신 **죽은 코드 제거**(`--real-vh`: `window.innerHeight`를 CSS 변수에 넣고 `resize`마다 갱신했으나 CSS에서 전혀 참조하지 않던 코드 — 실행문 5줄 제거)는 그대로 적용해 두었다. 음악 세션 수정 4종, 동영상 좌우 탐색 버튼, 게시판 작업, rhwp 0.8.4 자산은 모두 무변경으로 확인.
  - **대응 방안**: 앱 코드로는 회피가 불가능하므로 **크롬 업데이트를 기다리거나, 그동안 사파리·브레이브를 사용**하는 것이 현실적이다.
  - **검증**: 되돌린 뒤 음악 세션 4종·탐색 버튼(일반 2·공유 3)·탐색 CSS·게시판 일괄삭제/검색조건 전부 존재 확인, 추적 흔적(`_layoutLog`·`hardRelayout`·`forceRepaint`·`resetDocScroll`·`scrollRestoration`·`max-height:100svh`·`calc(100svh - 200px)`) **전부 0건**, PHP 6파일·JS 3파일·JSON·CSS 중괄호 균형 통과.
- **[업그레이드] rhwp 0.8.3 → 0.8.4 (2026-08-13)** — 아래 "rhwp 업그레이드 이력" 참조. 배포 채널 정리 전용 릴리즈로 **엔진 기능 변경 없음**. 커스텀 로직 변경 없음(`rhwp_editor.php` diff 2줄: studio JS 파일명 + `@rhwp_version`).

- **[업그레이드] rhwp 0.8.2 → 0.8.3 (2026-08-12)** — 아래 "rhwp 업그레이드 이력" 참조. 커스텀 로직 변경 없음(`rhwp_editor.php` diff 3줄: studio JS/CSS 파일명 + `@rhwp_version`). FileStation 버전은 명시 지시가 없어 **5.8.3d 유지**.

- **[방어] 외부 요인에 의한 재생 중단 감지 → 세션 자동 재획득 (2026-08-06)**
  - **전제**: 다른 탭의 동영상 플레이어가 미디어 세션을 가져가는 것 자체는 **우리가 막을 수 없다**(브라우저의 탭 격리로, 다른 출처의 세션을 감지하거나 차단하는 API가 없음). 따라서 "차단"이 아니라 **"뺏긴 것을 감지해 되찾는"** 방향으로 접근.
  - **감지 근거**: 로그에서 백그라운드 중 우리가 시키지 않은 `ms_pause`가 확인됨(08-06 00:13:02, `vis=hidden`·`state=playing` 상태에서 정지). 우리 플레이어가 `audio.pause()`를 부르는 곳은 `togglePlay`와 `destroy` **두 곳뿐**이므로, 그 외의 정지는 외부 요인(오디오 포커스 상실)으로 판별 가능하다.
  - **구현**: 자체 정지 시 `_selfPause` 플래그를 세우고, `pause` 리스너에서 플래그가 없으면 **외부 정지**로 판정 → `ms_pause_external`로 기록하고 `_reacquireMediaSession(track, 'pause_external')`로 세션을 다시 잡는다. 플래그는 판정 직후 초기화.
  - **로그 활용**: 이제 `ms_pause`(우리가 멈춤)와 `ms_pause_external`(뺏김)이 구분되어 기록된다. **다른 탭 동영상 사용 후 `ms_pause_external`이 찍히는지**가 세션 탈취의 직접 증거가 되고, 뒤따르는 `ms_reacquire_*`로 복구 시도까지 추적된다.
  - **한계(명시)**: 되찾기 시도일 뿐 iOS가 카드를 우리에게 줄지는 최종적으로 브라우저가 결정한다. 실패하면 남은 수단은 오디오 요소 재생성(위 미채택안).
  - **[후속 보완] 곡 전환 오탐 방지 (2026-08-06 감사에서 발견)**: HTML5 미디어 로드 알고리즘상 **재생 중에 `audio.src`를 바꾸면 `pause` 이벤트가 발생**한다(`_loadTrack`). 이를 '외부 요인 정지'로 오판하면 곡을 넘길 때마다 불필요한 세션 재획득이 돌아 metadata가 잠깐 비는 구간이 생긴다(앞서 제거한 해로운 패턴의 재발). → `src` 교체 직전에도 `_selfPause` 표시를 세워 오탐을 차단. 순수 추가 1줄.
  - **플래그 고착 위험 없음(확인)**: `togglePlay`은 **재생 중일 때만** `pause()`를 호출하므로 `pause` 이벤트가 반드시 발생해 플래그가 반드시 초기화된다. `destroy()`는 `_destroyed`를 먼저 세우고 리스너에도 `!this._destroyed` 가드가 있어 이중으로 안전. 클래스 밖에서 음악 오디오를 멈추는 경로는 없음(전부 `destroy()` 호출).
  - **검증**: 삭제된 실행문 1줄(`this._msLog('pause')`가 분기형으로 재작성), 새 9줄 + 오탐 방지 1줄. 캐시·데이터 절감 심볼 개수 전부 동일. 새 타이머·리스너 추가 0건(`setInterval` 19→19, `addEventListener` 438→438). `node --check` 통과.
  - **로그 내용(보안)**: 이벤트별로 재생 상태·metadata 유무·artwork 개수와 **곡 제목 40자**, **artwork URL 앞 70자**(blob이면 `blob:`으로만 표기)를 남긴다. 자체 서버의 `data/debug_logs`에만 저장되고 `data/.htaccess`로 외부 직접 접근이 차단되어 있다. 평소에는 **폴더를 지워두면 기록이 완전히 멈춘다**(폴더 존재 여부가 ON/OFF).

- **[수정 시도] 잠금화면 세션 다툼 — 곡 전환 시 미디어 세션 해제 후 재획득 (2026-08-05)**
  - **증상 확정(펜닐님 관찰)**: 문제 발생 시 잠금화면 제목이 **우리 곡 ↔ 동영상 사이트 URL로 번갈아** 표시되고, 곡이 바뀔 때 썸네일이 안 나옴. → 이미지 문제가 아니라 **iOS가 미디어 세션 주인을 확정하지 못하는 소유권 다툼**으로 확정.
  - **진단 로그가 배제한 것들**: `ms_*` 55건 분석 — 전 구간 `hasMeta=true`/`artCount=6` 유지, `ms_artwork_lost` 0건, 백그라운드 진입 시점에도 `state=playing`/`art=6` 정상. 또한 **`ctx=na`** — iOS는 비주얼라이저가 의도적으로 비활성(2550: MediaElementSource 연결 시 백그라운드 재생 끊김 이슈)이라 **AudioContext 자체가 없어** 직전의 `interrupted` 가설도 반증됨. 즉 우리 metadata 설정은 이미 1초마다 재설정까지 하며 완벽하게 동작 중이고, **metadata를 더 성실히 설정하는 방향의 수정은 한계에 도달**.
  - **근거**: 유일하게 확인된 해결 조건이 "플레이어를 닫았다 다시 열면 정상". `destroy()`를 보면 `metadata = null` + `playbackState = 'none'`으로 **세션을 놓아준다.** 우리는 지금까지 metadata를 **덮어쓰기만** 했지 놓았다가 다시 잡은 적이 없다.
  - **수정**: `_reacquireMediaSession(track, reason)` 신설 — 세션을 놓고(`metadata=null`, `playbackState='none'`) **다음 틱**에 다시 설정한다(같은 틱이면 브라우저가 동일 세션으로 볼 수 있음). **곡 전환 시점**에 호출. 액션 핸들러는 건드리지 않음(해제하면 잠금화면 버튼이 잠깐 죽음).
  - **로그 보강**: `ms_reacquire_release`(놓기 직전 상태)와 `ms_reacquire_done`(재획득 후) 추가, 모든 `ms_*`에 `rs`(readyState)·`hasSrc` 필드 추가. 다음 재현 시 세션을 놓고 다시 잡는 과정이 그대로 기록된다.
  - **검토했으나 채택하지 않은 안**: 오디오 요소 자체를 재생성(`new Audio()`)하는 방식. 닫았다 열기와 가장 가깝지만 `_bind()` 안에서 오디오 리스너 10개가 UI 바인딩과 뒤섞여 있고 `this.audio` 참조가 37곳이라 위험이 크다. 또한 PC는 비주얼라이저가 `createMediaElementSource`로 **옛 요소에 영구 연결**돼 있어 요소를 갈아치우면 비주얼라이저가 죽는다(iOS 한정 적용이 필요). 이번 수정이 실패하면 그때 iOS 한정으로 진행.
  - **검증**: 삭제된 실행문 1줄뿐(`ctx` 줄이 필드 추가로 재작성), 나머지 순수 추가 24줄. 캐시·데이터 절감 심볼(`createObjectURL`·`_coverBlobCache`·`_getCachedCoverUrl`·`FSCoverCacheDB`) 개수 전부 동일. `node --check` 통과.
  - **[후속 수정] 재획득 타이밍 이동 (2026-08-06, 로그로 자체 결함 확인)**: 08-06 로그에서 재획득 직후 `ms_play`가 **`meta=false`, `art=0`**으로 찍혔다. 곡 전환 시점에 세션을 비우자 그 빈틈에 `play` 이벤트가 끼어들어 **metadata 없는 상태로 재생이 시작**된 것 — 잠금화면 표시에 오히려 해로운 상태이며, 이전 수정이 만든 결함이다. → 재획득 호출을 곡 전환에서 **`playing` 이벤트(재생 확립 후, 트랙당 1회)**로 옮겨 그 빈틈을 제거했다.
  - **로그가 잡은 또 하나의 사실**: 백그라운드 상태(`vis=hidden`)에서 `ms_pause` → 1초 뒤 `ms_play`가 기록됐다(00:13:02→00:13:03). 우리 코드가 일시정지시키는 경로가 아니므로 **외부 요인에 의한 재생 중단 후 재개**로 보이며, 다른 탭과의 오디오 포커스 경합을 뒷받침한다.
  - **[검증] 타이밍 수정 효과 확인 (2026-08-06 로그)**: 새 빌드 구간의 `ms_play` 3건 모두 `meta=true`/`art=6`으로, 직전 빌드에서 나타났던 `meta=false`/`art=0` 결함이 **재발하지 않음**. 재획득이 재생 확립 뒤에 실행되는 것도 순서상 확인(`ms_play` → `ms_reacquire_release` → `ms_reacquire_done`).
  - **[후속 수정] 탭 복귀 후 첫 재생에도 재획득 (2026-08-06)**: 같은 로그에서 `ms_vis_hidden` → `ms_vis_visible` → `ms_play` 흐름에 **재획득이 없는** 구간이 확인됐다(00:31:15~20). 재획득 가드가 **트랙당 1회**라 같은 곡을 이어 재생하면 건너뛰는데, 그 사이 다른 탭이 세션을 가져갔을 수 있어 재현 절차와 정확히 겹친다. → 탭을 벗어날 때(`document.hidden`) 가드(`_reacqDoneFor`)를 풀어 **복귀 후 첫 재생에서 다시 세션을 잡도록** 함. 순수 추가 1줄, 캐시 심볼 개수 전부 동일.
  - ⚠️ **실기기 확인 필요**: 재현 절차 후 ①잠금화면 제목이 우리 곡으로 **고정**되는지(번갈아 안 나오는지) ②썸네일이 나오는지 ③평소 곡 전환 시 잠금화면 컨트롤(이전/다음·재생)이 정상인지(회귀 확인). 로그 판정 기준: `ms_play` 시점의 `meta`가 항상 `true`이고, **다른 탭을 다녀온 뒤 첫 재생에 `ms_reacquire_*`가 찍혀야** 한다.

- **[수정] 잠금화면 썸네일 미표시 — AudioContext `interrupted` 미복구 (2026-08-05, 진단 로그로 확인)**
  - **로그가 밝힌 사실**: 추가한 진단 로그(`ms_*` 30건)를 보니 재현 구간 전 시점에서 `hasMeta=true`, `artCount=6`이 유지되고 **`ms_artwork_lost`는 0건**이었다. 백그라운드 진입(`ms_vis_hidden`) 순간에도 `state=playing`, `art=6`으로 정상. → **우리가 metadata를 잃는 문제가 아니며**, 앞서 의심했던 "백그라운드 진입 시 처리 없음" 공백도 원인이 아니었다.
  - **결정적 단서(펜닐님)**: 플레이어를 **닫았다 다시 열면 증상이 없음**. 즉 오래 살아 있는 플레이어 인스턴스가 다른 탭 동영상 재생을 겪은 뒤에만 문제가 생김.
  - **원인**: 비주얼라이저가 `createMediaElementSource(this.audio)`로 오디오를 **Web Audio에 영구 연결**한다(요소당 1회만 가능, 되돌릴 수 없음). 그래서 오디오 출력이 항상 AudioContext를 거치는데, **iOS/WebKit은 다른 탭·앱이 오디오 포커스를 가져가면 컨텍스트를 `'interrupted'` 상태로 만든다** — 표준 `'suspended'`가 아닌 WebKit 전용 상태다. 기존 복구 코드는 `state === 'suspended'`만 확인했고, 그마저 `_startVisualizer`가 `_visRunning`이면 조기 반환하여 복구 기회가 없었다. 플레이어를 닫았다 열면 **새 AudioContext + 새 오디오 요소**가 생성되므로 정상으로 돌아오던 것과 일치한다.
  - **수정**(`assets/js/app.js`): `_resumeAudioCtx(reason)` 헬퍼 신설 — `'suspended'`와 **`'interrupted'` 모두** 복구 대상으로 삼고, 복구 결과를 로그로 남긴다(`ms_ctx_resumed` / `ms_ctx_resume_fail`). 호출 지점 3곳: **재생 시작 시**, **탭 복귀 시**, 그리고 기존 `_startVisualizer`의 복구 코드를 이 헬퍼로 대체(조기 반환 경로에서도 복구되도록 호출 위치 유지).
  - **진단 로그 보강**: 모든 `ms_*` 기록에 **`ctx`(AudioContext 상태)** 필드 추가. 다음 재현 시 `ms_play` 시점의 `ctx`가 `interrupted`로 찍히면 원인이 확정되고, `ms_ctx_resumed`로 복구 성공 여부까지 확인된다.
  - **검증**: 삭제된 실행문 3줄(기존 `suspended` 전용 resume 2줄 → 헬퍼 호출로 대체, `idx` 줄 → `ctx` 필드 추가로 재작성)뿐이고 나머지는 순수 추가 20줄. 캐시·데이터 절감 심볼(`createObjectURL`·`_coverBlobCache`·`_getCachedCoverUrl`·`FSCoverCacheDB`) 개수 전부 동일, `createMediaElementSource` **실제 호출 1건 유지**(증가분 2건은 주석). `node --check` 통과.
  - ⚠️ **실기기 확인 필요**: 재현 절차(음악 재생→일시정지→다른 탭 동영상→복귀→다른 음악 재생) 후 잠금화면 썸네일이 나오는지. 안 나오면 그 날짜 로그의 `ms_play`/`ms_ctx_*` 기록으로 다음 단서를 잡는다.

- **[진단] 잠금화면 썸네일 미표시 — 음악 플레이어 MediaSession 진단 로그 추가 (2026-08-05)**
  - **정확한 재현 절차(펜닐님)**: ①다른 탭에서 동영상 재생 후 일시정지/정지 → ②웹하드 탭에서 음악 재생 → ③다른 탭으로 이동하거나 브라우저를 백그라운드로 → ④아이폰 잠금화면·음악 위젯에 mp3 썸네일이 안 나옴. **다른 탭의 동영상을 쓰지 않으면 재현되지 않음.**
  - **앞선 두 수정이 모두 무효였고, 그 근거도 반증됨**:
    - `blob:` URL 이론 → 재현이 "다른 탭 사용 후"에만 발생하므로 성립하지 않음(항상 발생해야 함). 외부 사례에서도 iOS는 canvas 축소 후 **blob URL로 아트워크가 정상 동작**한다고 보고됨.
    - metadata를 `play()` 앞으로 옮긴 것 → 메타데이터는 실제 재생 중일 때만 반영되므로 순서 자체가 원인이 아님.
  - **코드에서 확인된 구조적 공백**: `_mediaSessionVisHandler`가 `if (!document.hidden)`로 **복귀할 때만** 재설정하고, **백그라운드로 들어가는 순간에는 아무 처리도 하지 않는다.** 재현 절차 ③과 정확히 겹치는 구간이라 유력한 후보지만, 확정 전이라 이번에는 **로그만** 넣고 동작은 바꾸지 않음.
  - **추가한 진단 로그**(`FSAudioPlayer._msLog`, `assets/js/app.js`): 재생/일시정지, `visibilitychange`의 **숨김 진입·복귀 양쪽**, 복귀 후 재설정, 메타데이터 설정 결과, artwork 유실 감지 시점. 각 기록에 `mediaSession.playbackState`, metadata 유무, artwork 개수, 첫 artwork URL 형태(blob/url/default), 오디오 일시정지 여부, 문서 가시성을 남긴다.
  - **부하·안전**: 기존 방식 그대로 **`data/debug_logs` 폴더가 있을 때만** 기록되고 없으면 즉시 종료(평소 부하 0). 1초 주기로 도는 artwork 유지 타이머가 로그를 폭주시키지 않도록 **유실이 실제 감지된 경우만, 5초 간격 제한**으로 기록.
  - **캐시(데이터 절감) 무영향 검증**: `createObjectURL`(11→11), `_coverBlobCache`(11→11), `_getCachedCoverUrl`(6→6), `FSCoverCacheDB`(10→10), `_coverBlobPending`(8→8) 모두 개수 동일. 실행문 다중집합 비교로 **삭제된 코드 0건(순수 추가 35줄)** 확인. `node --check` 통과.
  - **적용 범위**: 메인 사이트(`assets/js/app.js`)에만 적용. 공유 페이지(`fs-audio-player.js`)는 이번 증상과 무관해 미변경.

- **[버그 수정] 모바일 잠금화면 음악 카드에 커버가 안 뜨고 다른 사이트 정보가 남던 문제 (2026-08-05, 펜닐님 제보)**
  - **증상**: 동영상 스트리밍 사이트를 탭으로 열어둔 채(재생은 멈춤) 웹하드 음악을 재생하면, 잠금화면·알림바 미디어 카드에 **커버가 안 나오고**, 트랙이 넘어가거나 잠깐 멈출 때 **그 동영상 사이트의 URL 정보가 표시**됨. 웹하드 플레이어 화면 안의 커버 이미지는 정상.
  - **원인**: OS 미디어 카드(Media Session)는 **한 번에 한 세션만** 표시한다. 브라우저는 **재생이 시작되는 순간** 그 탭의 세션을 활성화하는데, 기존 코드는 `this.audio.play()`를 먼저 호출하고 **그 뒤에** `_updateMediaSession(track)`을 호출했다. 그래서 세션이 잡히는 시점에 metadata가 **비어 있거나(첫 재생) 이전 트랙 것**이어서, 우리 정보가 카드에 실리지 못하고 다른 탭(멈춰 있던 동영상 사이트)이 카드를 계속 쥐고 있었다. 커버가 안 보인 것도 카드 주인이 우리가 아니었기 때문 — 이미지 로드 실패가 아니다(화면 안 커버는 정상이었던 점이 이를 뒷받침).
  - **수정**: `_updateMediaSession(track)` 호출을 `play()` **앞으로 이동**. 메인 사이트(`assets/js/app.js`)와 공유 페이지(`assets/js/fs-audio-player.js`) **양쪽 동일 적용**.
  - **순수 재배치**: 실행 코드는 한 줄도 추가·삭제되지 않았다. 파일 전체에서 주석을 제외한 실행문 다중집합을 비교해 **app.js 36,985줄 / fs-audio-player.js 3,248줄이 완전히 동일**함을 확인(사라진 줄 0, 새 줄 0). `_updateMediaSession` 호출 5건, `audio.play()` 3건으로 개수도 불변.
  - **적용 범위**: MediaSession 코드는 **음악 플레이어(`FSAudioPlayer`) 안에만 존재**(30건)하고 동영상 플레이어는 이 API를 쓰지 않으므로, 동영상 재생 동작에는 영향이 없다. 코드에 모바일 전용 분기가 없어 PC도 같은 경로를 타지만, PC는 잠금화면이 없어 체감 영향이 거의 없다.
  - ⚠️ **한계**: 미디어 카드를 어느 탭에 줄지는 최종적으로 브라우저가 결정하므로 100% 차단은 보장할 수 없다. 다만 우리 쪽이 만든 빈틈(재생 시작 시점에 metadata 미설정)은 제거됐다.
  - **보류(B안)**: 일시정지·트랙 전환 구간에도 세션을 계속 유지하는 방안은 원치 않는 알림 잔류 등 부작용 가능성이 있어 이번에는 적용하지 않음. 위 수정 결과를 보고 판단.
  - ⚠️ **실기기 확인 필요**: 다른 사이트 탭을 열어둔 상태에서 ①음악 재생 시 잠금화면 커버가 바로 뜨는지 ②트랙 전환·일시정지 후에도 유지되는지 ③일반 재생(다른 탭 없을 때)이 기존과 동일한지.
- **[버그 수정] 잠금화면 음악 커버가 비어 있던 진짜 원인 — artwork에 `blob:` URL 사용 (2026-08-05, 로그·코드 재조사로 확인)**
  - **경과**: 위 순서 교체(재생 전 metadata 설정)를 적용했으나 **모바일에서 증상 그대로**. 제보받은 클라이언트 로그(`data/debug_logs/2026-08-05.log`, 210줄)를 분석했으나 **음악 플레이어·MediaSession 이벤트가 0건**이라(`FSAudioPlayer`는 `_debugLog`를 쓰지 않음) 로그로는 진단 불가. 대신 커버 처리 경로를 역추적해 원인을 찾음. 로그에서 확인된 사실: 기기는 **iPhone / iOS 26.6 / Chrome for iOS**.
  - **원인**: 커버 캐시(`_getCachedCoverUrl`)는 데이터 절감을 위해 `URL.createObjectURL(blob)`로 만든 **`blob:` URL**을 반환하고, 이 값이 `_activeCoverUrl`에 저장되어 **그대로 MediaSession artwork `src`로 사용**되고 있었다. 페이지 안 `<img>`는 `blob:`을 정상 표시하지만, **잠금화면·알림 카드의 커버는 브라우저 밖 OS 프로세스가 직접 받아가므로 문서 스코프인 `blob:` URL을 해석하지 못한다.** → 화면 안 커버는 멀쩡한데 잠금화면만 비는 증상과 정확히 일치. ID3 커버·폴더 이미지 **두 경로 모두** 캐시를 타므로 항상 `blob:`이 되어, 다른 탭이 없어도 재현되는 문제였다(초기의 "다른 탭 간섭" 추정은 커버 부분에 대해서는 빗나감).
  - **수정**: `_updateMediaSession`에서 artwork로 쓸 URL이 `blob:`이면 **커버 캐시(Map: 원본URL → blobURL)를 역조회해 원본 `http(s)` URL로 치환**. 원본을 못 찾으면 깨진 blob을 넘기지 않고 기본 음표 아이콘(정적 PNG)으로 넘어가게 함. **화면 표시 경로(`_activeCoverUrl` → `<img>`)는 blob 그대로 유지**되어 캐시 절감 효과에 영향 없음. 메인(`assets/js/app.js`)·공유(`assets/js/fs-audio-player.js`) 양쪽 동일 적용.
  - **검증**: 해석 로직을 추출해 6개 시나리오 시뮬레이션 통과(ID3 blob→원본 복원 / 폴더이미지 blob→원본 복원 / 캐시에 없는 blob→기본아이콘 / 이미 http면 그대로 / null→기본아이콘 / undefined→폴더커버). 실행문 다중집합 비교로 **사라진 코드는 `const resolved` 한 줄뿐**(`let resolved`로 대체)이고 `createObjectURL`·`cover.innerHTML` 등 캐시·표시 경로는 **개수·내용 불변** 확인. `node --check` 통과.
  - ⚠️ **실기기 확인 필요**: 잠금화면·알림 카드에 커버가 실제로 뜨는지(이번 수정의 핵심), 화면 안 커버가 기존처럼 정상인지(회귀 확인), 같은 폴더 연속 재생 시 커버 재요청이 늘지 않는지.
- 버전 번호 상향(5.8.3c → 5.8.3d)에 따라 `?v=APP_VERSION` 캐시 버스팅이 자동 무효화되어, 브라우저가 수정된 스크립트를 새로 받는다.
...
**[업그레이드] rhwp 0.8.3 → 0.8.4 (2026-08-13, 가이드 v6.6 준수)**

- **rhwp 0.8.4** (npm latest, shasum `07ca46da163c32f62f556b8b48acfcd274f855b2` 진본 확인) 적용.
- **업스트림 성격 — 배포 채널 복원 전용, 엔진 기능 변경 없음**. CHANGELOG의 0.8.4 항목은 전부 배포 범위 얘기다: 공식 배포 범위를 v0.8.2와 동일하게 복원하고(GitHub Pages·Release CLI·npm `@rhwp/core`/`@rhwp/editor`·VS Code Marketplace/Open VSX·브라우저 확장 유지), 메인테이너 채택과 안전 검증 없이 추가됐던 **PyPI, `@rhwp/node`, GHCR CLI 이미지, deb/rpm/MSI, 설치 스크립트·패키지 매니저 매니페스트·설치용 GitHub Action을 제거**했다(#4655). **조판·렌더·저장 관련 변경 항목은 0.8.4에 없다.**
  - FileStation은 `@rhwp/core`(npm)만 쓰므로 **철회된 채널과 무관**하다.
- **코어 대조로 무변경 교차 확인**: `rhwp.js`(wasm-bindgen 글루)와 `rhwp.d.ts`가 0.8.3과 **바이트 단위 완전 동일**(각 404,678B / 161,595B) → 공개 API 표면 무변경. `rhwp_bg.wasm`은 **크기가 정확히 같은데**(8,038,570B) 내용의 0.53%만 흩어져 달라 동일 소스의 재컴파일로 판단.
- **ABI 호환성**: `rhwp.d.ts`가 0.8.3과 동일하므로 `exportHwp`/`exportHwpx`/`exportHml`/`renderPageSvg`/`renderPageToCanvas`/`renderPageHtml`/`version` 전부 그대로. 커스텀 저장·뷰어 렌더 무영향.
- **호스트 API 존속 확인**: `window.rhwpStudio.notifySaved()`가 0.8.4 studio 번들에도 존재(1건) → 저장 후 이탈 경고 제거 기능 유지.
- **빌드 산출물**: `index-BsGOrzbg.js`, **`index-RCGbZ9bS.css`는 0.8.3과 동일 해시**(CSS 무변경), `rhwp_bg-B2jQnVzD.wasm`, `canvaskit-renderer-BSfYmKau.js`. **`canvaskit-DB1zH3nD.wasm`은 0.7.16 이후 계속 동일 해시**(md5 대조).
...
- **자산 배포 검증**: 빌드 산출물 50개 md5 대조 — **누락 0, 해시 불일치 0(패치분 제외), 이전 빌드 잔재 0**. `rhwp_editor.php` 참조 파일명, studio JS가 참조하는 `rhwp_bg`·`canvaskit-renderer`, renderer가 참조하는 `canvaskit wasm`까지 전부 존재 확인. 코어 2파일은 npm 0.8.4 원본과 md5 일치.
- **변경 범위**: `rhwp_editor.php` **2줄**(studio JS 파일명 + `@rhwp_version` — CSS 해시가 같아 교체 불필요), `rhwp_viewer.php` 1줄(`@rhwp_version`), `README.md`, `assets/rhwp/` 자산. **커스텀 로직·게시판·동영상 플레이어·음악 플레이어 코드 일절 미변경**(백업 대비 diff 확인).
- **STEP 10 검증 (v6.6)**: 1~10 커스텀 보존 전부 충족, 개정 항목 11/11b/12와 신설 18/18b 충족, 13~17(J1·J2 제거, CSS 정규화, `@rhwp_version 0.8.4`, CanvasKit 배치) ✅. PHP 6파일 `php -l` 통과.
- ⚠️ **실기기 확인**: 엔진 변경이 없으므로 0.8.3에서 정상이던 동작은 그대로일 가능성이 높다. 다만 wasm이 재컴파일되었으므로 **HWP 뷰어 렌더와 에디터 Ctrl+S 서버 저장**만 한 번 확인 권장.

**[업그레이드] rhwp 0.8.2 → 0.8.3 (2026-08-12, 가이드 v6.6 준수)**

- **rhwp 0.8.3** (npm latest, shasum `5f624bfcd99e0bb2e7b8a040605514df4b385af6` 진본 확인) 적용.
- **업스트림 성격 — 누적 PATCH (190개 PR)**. CHANGELOG 명시: JSON 봉투 major는 1.0 유지.
  - **암호 문서**: HWP5 EncryptVersion 4 암호 문서 열기와 Studio 비밀번호 입력, HWP3 암호 해제, 암호화 HWPX 열기, HWP3/HWP5/HWPX 암호화 저장·재열기 계약 추가.
  - **조판·렌더링**: 중첩 표의 페이지 분할·RowBreak·셀 내부 빈 문단·자식 표 흐름 연속 보정, PUA 글자 겹침으로 기록된 사각 번호·삼각형을 실제 글리프로 복원, **CanvasKit이 정확한 glyph run을 재생하도록 개선**(#3741 — FileStation 에디터 렌더 경로에 직접 관련).
  - **편집·Studio**: 중첩 표 선택·복사 보정, 다중 셀 서식, F5 표 셀 크기 조절 3모드, 표 분할·결합.
  - **CLI·MCP**: `mcp-serve` stdio JSON-RPC 서버, IR·capabilities·plan schema 정비, 공식 Node·Python 바인딩. **FileStation 사용 경로와 무관한 영역.**
  - **보안**: 압축 해제 크기 상한으로 **deflate bomb 입력의 메모리 고갈 차단**(#4263) — 뷰어/에디터가 신뢰할 수 없는 파일을 여는 경로에 유익.
- **ABI 호환성**: `exportHwp`/`exportHwpx`/`exportHml`/`renderPageSvg`/`renderPageToCanvas`/`renderPageHtml`/`version` 전부 `rhwp.d.ts`에 존재(누락 0) → 커스텀 저장·뷰어 렌더 무영향.
- **호스트 API 존속 확인**: 저장 후 이탈 경고 제거에 쓰는 `window.rhwpStudio.notifySaved()`가 0.8.3 studio 번들에도 존재(1건) → 해당 기능 유지.
- **빌드 산출물**: `index-Cvu55tJe.js`, `index-RCGbZ9bS.css`(0.8.2와 달리 **이번엔 CSS도 변경**), `rhwp_bg-CZ-q11ED.wasm`, `canvaskit-renderer-DkTGgisQ.js`. **`canvaskit-DB1zH3nD.wasm`은 0.7.16 이후 계속 동일 해시**(md5 대조 확인).
- **패치 재적용**: J1(`file:save` Ctrl+S 매핑) 1건 제거, J2(`file:print` Ctrl+P 매핑) 1건 제거 — `file:save` 메뉴 정의 유지. P1 0건 / P2(`../images/`) 1건 정규화. 패치 후 studio JS `node --check` 통과.
- **`vite.config.ts`**: PWA·커스텀 플러그인이 있어 **옵션 B**(`base: './'`만 안전 추가)로 처리 — PWA 생성 정상(`sw.js`, precache 57 entries).
- **자산 배포 검증**: 빌드 산출물 50개를 md5로 대조 — **누락 0, 해시 불일치 0(패치분 제외), 이전 빌드 잔재 0**. `rhwp_editor.php` 참조 파일명과 실제 파일, studio JS가 참조하는 `rhwp_bg`·`canvaskit-renderer`, renderer가 참조하는 `canvaskit wasm`까지 전부 존재 확인. 코어 2파일은 npm 0.8.3 원본과 md5 일치.
- **변경 범위**: `rhwp_editor.php` **3줄**(studio JS 파일명·CSS 파일명·`@rhwp_version`), `rhwp_viewer.php` 1줄(`@rhwp_version`), `README.md`, `assets/rhwp/` 자산. **커스텀 로직·게시판·동영상 플레이어·음악 플레이어 코드 일절 미변경**(백업 대비 diff 확인).
- **STEP 10 검증 (v6.6)**: 1~10 커스텀 보존 전부 기준 충족, 개정 항목 11/11b/12와 신설 18/18b 충족, 13~17(J1·J2 제거, CSS 정규화, `@rhwp_version 0.8.3`, CanvasKit 배치) ✅. PHP 6파일 `php -l` 통과.
- ⚠️ **실기기 확인 필요**: HWP 뷰어 렌더, 에디터 Ctrl+S 서버 저장, Ctrl+Shift+S 다른 이름으로 저장, HWPX 직접 저장 유지, 저장 후 이탈 경고 미표시, CanvasKit 렌더, **표(특히 중첩 표) 문서의 페이지 정합**(이번 릴리즈의 조판 보정 영향 확인).
...
*최종 업데이트: 2026-08-24 (v5.8.3f, rhwp 0.8.4 기준)*
